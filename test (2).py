...
class EnhancedProtectionSystem:
    ...
    def update(self, price: float, high: float, low: float, volume: float) -> None:
        if price <= 0 or high <= 0 or low <= 0 or volume < 0:
            logging.warning(f"Dữ liệu không hợp lệ, bỏ qua: price={price}, high={high}, low={low}, volume={volume}")
            return
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if current_time - self.last_price_update >= 5:
            if not await check_internet_connection():
                ...
                if self.current_price <= 0:
                    logging.error("Giá trị gần nhất không hợp lệ, không thể tiếp tục")
                    return
            else:
                new_price = None
                new_volume = None
                ...
                for attempt in range(config.api_retry_count):
                    try:
                        async with asyncio.timeout(30):  # Tăng timeout lên 30 giây để giảm lỗi timeout
                            ...
                            if new_price <= 0 or new_volume <= 0 or new_high_low[0] <= 0 or new_high_low[1] <= 0:
                                raise ValueError("Dữ liệu không hợp lệ từ API")
                    ...
                    if self.current_price <= 0:
                        logging.error("Không có dữ liệu hợp lệ để tiếp tục, dừng bot")
                        sys.exit(1)
                    error_msg = (
        ...
        if self.current_price <= 0 or self.current_volume <= 0 or self.current_high <= 0 or self.current_low <= 0:
            logging.error("Dữ liệu không hợp lệ, dừng bot")
            sys.exit(1)
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        ...
        self.paper_orders = []
        self.current_market_price = 60000.0  # Giá trị mặc định ban đầu
        self.current_volume = 100.0  # Khối lượng mặc định ban đầu
        self.current_high = 60500.0  # Giá cao mặc định ban đầu
        self.current_low = 59500.0  # Giá thấp mặc định ban đầu
        self.order_book = {"bids": [], "asks": []}
        self.last_api_update = 0.0
        self.api_update_interval = 10
        asyncio.run(self._fetch_initial_data())  # Lấy dữ liệu ban đầu khi khởi tạo
        ...
        for attempt in range(config.api_retry_count):
            try:
                ...
                if price > 0:
                    ...
                    raise ValueError("Giá ban đầu không hợp lệ")
            except Exception as e:
                logging.warning(f"Thử lấy dữ liệu ban đầu lần {attempt + 1}/{config.api_retry_count} thất bại: {str(e)}")
                if attempt < config.api_retry_count - 1:
                    await asyncio.sleep(config.api_retry_delay)
        if self.current_market_price <= 0:
            logging.error("Không thể lấy dữ liệu ban đầu hợp lệ, sử dụng giá trị mặc định")
            self.current_market_price = 60000.0
    ...
    async def get_price(self) -> float:
        ...
        if current_time - self.last_api_update < self.api_update_interval:
            if self.current_market_price <= 0:
                logging.warning("Giá hiện tại không hợp lệ, thử lấy dữ liệu mới")
            else:
        ...
        try:
            ...
            if self.current_volume <= 0 or self.current_high <= 0 or self.current_low <= 0:
                logging.warning("Dữ liệu không hợp lệ từ API, giữ giá trị cũ")
            else:
            ...
            return self.current_market_price if self.current_market_price > 0 else 60000.0
