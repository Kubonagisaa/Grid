...
async def send_telegram_alert(message: str, parse_mode: str = "Markdown") -> None:
    ...
    try:
        ...
        if e.status == 429:
            ...
            return await send_telegram_alert(message, parse_mode)
        elif e.status == 400:
...
# ======== HYBRID GRID STRATEGY (ĐÃ CẢI TIẾN) ========
class EnhancedSmartGridManager:
    def __init__(self):
        self.grid_levels: List[Tuple[float, float]] = []
        self.static_levels: int = 3  # Số lệnh static cố định
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        if len(self.high_history) == 0 or len(self.low_history) == 0 or len(self.price_history) == 0:
            logging.warning("Một trong các lịch sử giá rỗng, không thể đồng bộ")
            self.grid_levels = self._create_static_grid(current_price)
            return
                ...
                try:
                    ...
                    if not self.duplicate_order_exists("SELL", sell_price, open_orders):
                        ...
                        if 'orderId' not in sell_order:
                            logging.error(f"Lệnh BÁN static không trả về orderId: {sell_order}")
                            continue
                        self.order_ids.append(sell_order['orderId'])
                        self.static_order_ids.append(sell_order['orderId'])
                        logging.info(f"(Static) Đã đặt lệnh BÁN tại {sell_price:.2f}, số lượng: {quantity:.6f}, orderId: {sell_order['orderId']}")
                        await send_telegram_alert(f"(Static) SELL: {sell_price:.2f}, SL: {quantity:.6f}")
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        async with order_lock:
            ...
            # Đặt dynamic orders (bỏ qua static levels)
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                ...
                try:
                    ...
                    # Đặt lệnh BUY động nếu giá hiện tại thấp hơn mức lưới
                    ...
                    # Đặt lệnh SELL động nếu giá hiện tại cao hơn mức lưới
            ...
            # Fallback động với điều kiện chặt chẽ hơn
    ...
    async def rebalance_if_needed(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        if need_rebalance:
            async with order_lock:
                await self.cancel_all_orders(exchange, preserve_static=False)  # Hủy tất cả, kể cả static
                await self.calculate_adaptive_grid(current_price, exchange)
                await self.place_static_orders(current_price, exchange)  # Đặt lại static orders
                await self.place_grid_orders(current_price, exchange)  # Đặt dynamic orders
        ...
        try:
            ...
            for order in orders:
                ...
                if order['status'] == 'FILLED':
                    ...
                    if order['orderId'] in grid_manager.static_order_ids:
                        grid_manager.static_order_ids.remove(order['orderId'])
                        grid_manager.static_orders_placed = False  # Đặt lại để cho phép đặt static mới
                else:
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        try:
            ...
            if not signals and len(self.grid_manager.grid_levels) > 0:
                buy_price, sell_price = self.grid_levels[0]
                if data.price < buy_price and (rsi < 70 or rsi < 30):
                    signals.append(Signal(side="BUY", price=buy_price, quantity=config.min_quantity))
                elif data.price > sell_price and (rsi > 30 or rsi > 70):
                    signals.append(Signal(side="SELL", price=sell_price, quantity=config.min_quantity))
...
async def check_protections(state: BotState, exchange: 'ExchangeInterface', price: float, volume: float, high: float, low: float) -> bool:
    ...
    if state.protection.check_abnormal_activity(volume):
        logging.warning("Phát hiện hoạt động bất thường về khối lượng")
        return True
    return False
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def get_order_book(self) -> Dict:
        await self.get_price()  # Cập nhật order book
    ...
    async def run_paper_trading_live(self, state: BotState, interval: int = 1) -> None:
        ...
        while True:
            try:
                ...
                rsi_val = calculate_rsi(list(state.grid_manager.price_history), config.rsi_period) if len(state.grid_manager.price_history) >= config.rsi_period else 50.0
                logging.info(f"Giá thị trường tại {datetime.now()}: {price}, RSI: {rsi_val:.2f}")
                ...
                if int(datetime.now().timestamp()) % 60 == 0:
                    ...
                    await send_telegram_alert(
                        f"Thống kê paper trading trên {self.exchange_name}\nTotal Profit: {stats['Total Profit']:.2f} USDT\nTrade Count: {stats['Trade Count']}\nWin Rate: {stats['Win Rate']:.2f} percent\nMax Drawdown: {stats['Max Drawdown']:.2f} percent\nSharpe Ratio: {stats['Sharpe Ratio']:.2f}\nTotal Fees: {stats['Total Fees']:.2f} USDT"
                    )
                await asyncio.sleep(interval)
            except Exception as e:
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        try:
            ...
            if order_type == 'limit' and price is None:
                raise ValueError("Lệnh giới hạn yêu cầu giá")
        ...
        for attempt in range(config.api_retry_count):
            try:
                ...
                if self.symbol not in markets:
                    ...
                    raise ValueError(f"Cặp giao dịch {self.symbol} không tồn tại")
                ...
                logging.error(f"Lỗi khi lấy thông tin LOT_SIZE (lần thử {attempt+1}/{config.api_retry_count}): {str(e)}")
    ...
    def __init__(self):
        ...
        if config.paper_trading_enabled:
            ...
            for exchange_name in config.enabled_exchanges:
                if exchange_name not in config.exchange_credentials:
                    logging.error(f"Không tìm thấy thông tin xác thực cho {exchange_name}")
                    continue
                ...
                if exchange_name == "binance":
                    ...
                    logging.warning(f"Sàn {exchange_name} không được hỗ trợ")
    
    async def initialize(self) -> None:
        for name, exchange in self.exchanges.items():
            try:
                await exchange.get_price()  # Kiểm tra kết nối
                logging.info(f"Sàn {name} đã được khởi tạo thành công")
            except Exception as e:
                logging.error(f"Lỗi khi khởi tạo sàn {name}: {str(e)}")
                del self.exchanges[name]
    
    async def get_all_balances(self) -> Dict[str, Tuple[float, float]]:
        balances = {}
        for name, exchange in self.exchanges.items():
            try:
                usdt, base = await safe_api_call(exchange.get_balance)
                balances[name] = (usdt, base)
            except Exception as e:
                logging.error(f"Lỗi khi lấy số dư từ {name}: {str(e)}")
                balances[name] = (0.0, 0.0)
        return balances
    
    async def close_all(self) -> None:
        for exchange in self.exchanges.values():
            await exchange.close()
        logging.info("Đã đóng tất cả kết nối sàn")

# ======== DASHBOARD ========
app = dash.Dash(__name__, title="Grid Trading Bot Dashboard")
auth = dash_auth.BasicAuth(app, {config.dashboard_username: config.dashboard_password})

app.layout = html.Div([
    html.H1("Grid Trading Bot Dashboard"),
    dcc.Interval(id="interval-component", interval=60 * 1000, n_intervals=0),  # Cập nhật mỗi 60s
    html.Div(id="bot-status"),
    dcc.Graph(id="equity-curve"),
    dcc.Graph(id="price-chart"),
    dash_table.DataTable(id="orders-table", columns=[
    ...
    html.Button("Pause All Bots", id="pause-button", n_clicks=0),
    html.Button("Resume All Bots", id="resume-button", n_clicks=0),
])
...
def update_dashboard(n_intervals):
    ...
    for name, state in bot_states.items():
        stats = state.tracker.get_stats()
        balance = await state.exchange.get_balance()
        status_text.append(
            html.Div([
                html.H3(f"Bot {name}"),
                ...
                html.P(f"Balance: USDT {balance[0]:.2f}, {config.symbol.split('USDT')[0]} {balance[1]:.6f}"),
                html.P(f"Paused: {state.order_manager.is_paused}"),
            ])
        )

        equity_data.append(go.Scatter(
        ...
        price_data.append(go.Scatter(
            x=[datetime.now() - timedelta(minutes=i) for i in range(len(state.grid_manager.price_history))][::-1],
            y=list(state.grid_manager.price_history),
            mode="lines",
            name=f"{name} Price"
        ))

        open_orders = await state.exchange.get_open_orders()
        for order in open_orders:
            orders_data.append({
                "exchange": name,
                "side": order.get("side", "N/A"),
                "price": f"{order.get('price', 0):.2f}",
                "quantity": f"{order.get('origQty', 0):.6f}",
                "status": order.get("status", "N/A"),
            })

    equity_fig = {"data": equity_data, "layout": {"title": "Equity Curve"}}
    price_fig = {"data": price_data, "layout": {"title": f"{config.symbol} Price History"}}
    return status_text, equity_fig, price_fig, orders_data

@app.callback(
    Output("pause-button", "n_clicks"),
    [Input("pause-button", "n_clicks")]
)
async def pause_bots(n_clicks):
    if n_clicks > 0:
        ...
        logging.info("Tất cả bot đã được tạm dừng")
        await send_telegram_alert("Tất cả bot đã được tạm dừng")
    return 0
...
async def resume_bots(n_clicks):
    if n_clicks > 0:
        for state in bot_states.values():
            state.order_manager.is_paused = False
        logging.info("Tất cả bot đã được tiếp tục")
        await send_telegram_alert("Tất cả bot đã được tiếp tục")
    return 0
...
def calculate_rsi(prices: List[float], period: int) -> float:
    ...
    avg_gain = np.mean(gains[-period:]) if gains else 0.0
    avg_loss = np.mean(losses[-period:]) if losses else 0.0
...
async def safe_api_call(func, *args, **kwargs):
    for attempt in range(config.api_retry_count):
        try:
            ...
            logging.warning(f"Lỗi API (lần thử {attempt+1}/{config.api_retry_count}): {str(e)}")
            if attempt == config.api_retry_count - 1:
                raise
            await asyncio.sleep(config.api_retry_delay)
    raise Exception("Hết số lần thử API")
...
async def initialize_bots():
    ...
    for name, exchange in exchange_manager.exchanges.items():
        ...
        bot_states[name] = BotState(exchange, strategy)
        if config.websocket_enabled:
            await exchange.start_websocket(bot_states[name])
        logging.info(f"Bot cho {name} đã được khởi tạo")

    load_state(bot_states)

async def run_bots():
    while True:
        tasks = []
        for name, state in bot_states.items():
            if not state.order_manager.is_paused:
                tasks.append(state.process_events())
            health = await state.exchange.health_check()
            if health['status'] != 'healthy':
                logging.warning(f"Sàn {name} không ổn định: {health['status']}")
                state.order_manager.is_paused = True
            else:
                state.order_manager.is_paused = False
        if tasks:
        ...
        if int(time.time()) % 60 == 0:
            stats_summary = "\n".join(
                f"{name}: Profit {state.tracker.get_stats()['Total Profit']:.2f} USDT, "
                f"Trades {state.tracker.get_stats()['Trade Count']}"
                for name, state in bot_states.items()
            )
            await send_telegram_alert(f"Bot Status Update\n{stats_summary}")
        
        # Kiểm tra tài nguyên hệ thống
        cpu_usage = psutil.cpu_percent()
        memory_info = psutil.virtual_memory()
        if cpu_usage > 90 or memory_info.percent > 90:
            logging.warning(f"Tài nguyên hệ thống cao: CPU {cpu_usage}%, Memory {memory_info.percent}%")
            await send_telegram_alert(f"Cảnh báo tài nguyên: CPU {cpu_usage}%, Memory {memory_info.percent}%")
        
        await asyncio.sleep(5)

def signal_handler(sig, frame):
    logging.info("Đã nhận tín hiệu dừng, đang tắt bot...")
    for state in bot_states.values():
        asyncio.run(state.order_manager.cancel_all_orders(state.exchange))
        asyncio.run(state.exchange.close())
    save_state(bot_states)
    sys.exit(0)
...
async def main():
    ...
    # Chạy paper trading thời gian thực nếu được bật
    paper_trading_tasks = [
        bot_states[name].exchange.run_paper_trading_live(bot_states[name])
        for name, state in bot_states.items()
        if name == "paper_trading" and state.live_paper_trading
    ]

    # Chạy vòng lặp chính cho các bot
    bot_task = run_bots()
    ...
    # Chạy tất cả tasks
    await asyncio.gather(bot_task, *paper_trading_tasks, return_exceptions=True)
