...
class AdvancedConfig:
    def __init__(self):
        ...
        # Biến cấu hình cho paper trading (mô phỏng giao dịch)
...
class EnhancedSmartGridManager:
    ...
    async def rebalance_if_needed(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        # Hủy toàn bộ các order cũ trước khi rebuild grid
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        ...
        # Khi đặt lệnh, trạng thái ban đầu là OPEN để chờ khớp khi giá thật chạm limit
        order = {
...
# Lớp BinanceExchange và MEXCExchange giữ nguyên chức năng ban đầu
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        # Hủy hết lệnh mở ngay khi khởi động bot
        await state.order_manager.cancel_all_orders(exchange)
        ...
        # Hủy tất cả các lệnh mở khi dừng bot
        await state.order_manager.cancel_all_orders(exchange)
        logging.info(f"✅ Đã hủy hết lệnh khi dừng bot trên {exchange.exchange_name}")

# ======== PHẦN CHẠY BOT CHÍNH (MAIN) ========
if __name__ == "__main__":
    import sys

    async def main():
        # Giả sử bạn muốn chạy bot cho tất cả các sàn được cấu hình trong config.enabled_exchanges
        from collections import defaultdict
        states = defaultdict()
        exchange_manager = ExchangeManager()

        # Chạy bot cho từng sàn trong một event loop
        tasks = []
        for exch in exchange_manager.exchanges.values():
            tasks.append(run_bot_for_exchange(exch, states))
        try:
            await asyncio.gather(*tasks)
        except KeyboardInterrupt:
            logging.info("Đã nhận tín hiệu dừng bot (Ctrl+C)")
        finally:
    ...
    try:
        ...
        logging.error(f"Bot gặp lỗi: {str(e)}")
        sys.exit(1)