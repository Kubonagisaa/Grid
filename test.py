...
class AdvancedConfig:
    def __init__(self):
        self.enabled_exchanges: List[str] = os.getenv("ENABLED_EXCHANGES", 
"paper_trading").split(",")
        ...
        self.initial_investment: float = float(os.getenv("INITIAL_INVESTMENT", 
20))
        self.min_quantity: float = float(os.getenv("MIN_QUANTITY", 0.0005))
        self.max_position: float = float(os.getenv("MAX_POSITION", 0.01))
        self.base_stop_loss_percent: float = 
float(os.getenv("BASE_STOP_LOSS_PERCENT", 2.0))
        self.base_take_profit_percent: float = 
float(os.getenv("BASE_TAKE_PROFIT_PERCENT", 2.0))
        self.price_drop_threshold: float = 
float(os.getenv("PRICE_DROP_THRESHOLD", 1.0))
        self.adaptive_grid_enabled: bool = os.getenv("ADAPTIVE_GRID_ENABLED", 
"True").lower() == "true"
        self.min_grid_levels: int = int(os.getenv("MIN_GRID_LEVELS", 5))
        self.max_grid_levels: int = int(os.getenv("MAX_GRID_LEVELS", 15))
        self.base_grid_step_percent: float = 
float(os.getenv("BASE_GRID_STEP_PERCENT", 2.0))
        self.grid_rebalance_interval: int = 
int(os.getenv("GRID_REBALANCE_INTERVAL", 300))
        self.trailing_stop_enabled: bool = os.getenv("TRAILING_STOP_ENABLED", 
"True").lower() == "true"
        self.trailing_up_activation: float = 
float(os.getenv("TRAILING_UP_ACTIVATION", 90000))
        self.trailing_down_activation: float = 
float(os.getenv("TRAILING_DOWN_ACTIVATION", 80000))
        self.trailing_tp_enabled: bool = os.getenv("TRAILING_TP_ENABLED", 
"True").lower() == "true"
        self.trailing_buy_stop_enabled: bool = 
os.getenv("TRAILING_BUY_STOP_ENABLED", "True").lower() == "true"
        self.trailing_buy_activation_percent: float = 
float(os.getenv("TRAILING_BUY_ACTIVATION_PERCENT", 1.5))
        self.trailing_buy_distance_percent: float = 
float(os.getenv("TRAILING_BUY_DISTANCE_PERCENT", 1.0))
        self.pump_protection_threshold: float = 
float(os.getenv("PUMP_PROTECTION_THRESHOLD", 0.03))
        self.circuit_breaker_threshold: float = 
float(os.getenv("CIRCUIT_BREAKER_THRESHOLD", 0.07))
        self.circuit_breaker_duration: int = 
int(os.getenv("CIRCUIT_BREAKER_DURATION", 360))
        self.abnormal_activity_threshold: float = 
float(os.getenv("ABNORMAL_ACTIVITY_THRESHOLD", 3.0))
        self.maker_fee: float = float(os.getenv("MAKER_FEE", 0.0002))
        self.taker_fee: float = float(os.getenv("TAKER_FEE", 0.0004))
        self.telegram_enabled: bool = bool(os.getenv("TELEGRAM_BOT_TOKEN") and 
os.getenv("TELEGRAM_CHAT_ID"))
        ...
        self.status_update_interval: int = 
int(os.getenv("STATUS_UPDATE_INTERVAL", 1800))
        ...
        self.min_acceptable_volume: float = 
float(os.getenv("MIN_ACCEPTABLE_VOLUME", 100))
        self.fee_threshold: float = float(os.getenv("FEE_THRESHOLD", 0.1))
        self.websocket_enabled: bool = os.getenv("WEBSOCKET_ENABLED", 
"False").lower() == "true"
        self.websocket_reconnect_interval: int = 
int(os.getenv("WEBSOCKET_RECONNECT_INTERVAL", 60))
        self.order_cooldown_sec: int = int(os.getenv("ORDER_COOLDOWN_SECONDS", 
180))
        self.paper_trading_enabled: bool = os.getenv("PAPER_TRADING", 
"False").lower() == "true"
        self.live_paper_trading: bool = os.getenv("LIVE_PAPER_TRADING", 
"False").lower() == "true"
...
"admin123")
        ...
        self.wide_grid_step_percent_min: float = 
float(os.getenv("WIDE_GRID_STEP_PERCENT_MIN", 2.0))
        self.wide_grid_step_percent_max: float = 
float(os.getenv("WIDE_GRID_STEP_PERCENT_MAX", 3.0))
        self.dense_grid_step_percent_min: float = 
float(os.getenv("DENSE_GRID_STEP_PERCENT_MIN", 0.5))
        self.dense_grid_step_percent_max: float = 
float(os.getenv("DENSE_GRID_STEP_PERCENT_MAX", 1.0))
        self.capital_reserve_percent: float = 
float(os.getenv("CAPITAL_RESERVE_PERCENT", 0.0))
        self.min_profit_percent: float = float(os.getenv("MIN_PROFIT_PERCENT", 
0.0))
        self.stop_loss_capital_percent: float = 
float(os.getenv("STOP_LOSS_CAPITAL_PERCENT", 30.0))
        self.backtest_enabled: bool = os.getenv("BACKTEST_ENABLED", 
"False").lower() == "true"
        self.backtest_timeframe: str = os.getenv("BACKTEST_TIMEFRAME", "1h")
        self.backtest_limit: int = int(os.getenv("BACKTEST_LIMIT", 1000))
        self.telegram_report_interval: int = 
int(os.getenv("TELEGRAM_REPORT_INTERVAL", 3600))
        self.telegram_volatile_interval: int = 
int(os.getenv("TELEGRAM_VOLATILE_INTERVAL", 600))
        self.telegram_volatility_threshold: float = 
float(os.getenv("TELEGRAM_VOLATILITY_THRESHOLD", 1.0))
        ...
        if "DOGE" in self.symbol:
            self.trailing_up_activation = 
float(os.getenv("TRAILING_UP_ACTIVATION_DOGE", 0.0))
            self.trailing_down_activation = 
float(os.getenv("TRAILING_DOWN_ACTIVATION_DOGE", 0.0))

config = AdvancedConfig()

logging.info(f"Đã chọn chiến lược {config.strategy_mode}: {'Grid thưa (bước 
~2-3%, có RSI)' if config.strategy_mode == 1 else 'Grid dày (bước ~0.5%, không 
dùng RSI)'}")
logging.info(f"Bước lưới cơ bản: {config.base_grid_step_percent:.2f}%, Mức grid tối 
thiểu: {config.min_grid_levels}, tối đa: {config.max_grid_levels}")

# ======== HÀM TIỆN ÍCH (ATR, RSI, tính drawdown, Sharpe, tự điều chỉnh RSI) ========
def calculate_atr(highs: List[float], lows: List[float], prices: List[float], 
period: int) -> float:
...
def calculate_rsi(prices: List[float], period: int) -> float:
    ...
    downs = -1 * np.clip(deltas, a_max=0, a_min=None)
...
None:
        if price <= 0 or high <= 0 or low <= 0 or volume < 0:
            logging.warning(f"Dữ liệu không hợp lệ: price={price}, high={high}, 
...
class EnhancedOrderManager:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 
'ExchangeInterface',
            ...
            for order in orders:
                ...
                if order['status'] == 'FILLED':
                    ...
                    try:
                        ...
                        qty = float(order['executedQty']) if order['executedQty'] is not 
None else 0.0
                    except (ValueError, TypeError) as e:
                        logging.error(f"Giá hoặc số lượng không hợp lệ trong lệnh 
{order['orderId']}: {str(e)}")
                        continue
                    if price <= 0 or qty <= 0:
                        logging.warning(f"Giá hoặc số lượng không hợp lệ trong lệnh 
{order['orderId']}: price={price}, qty={qty}")
                        continue
                    if order['orderId'] in grid_manager.static_order_ids and 
side.upper() == 'BUY':
                        grid_manager.trailing_stops[("BUY", round(price, 
grid_manager.price_precision))] = [price, qty]
                        ...
                        await send_telegram_alert(f"Static BUY filled @ {price:.2f}, qty 
{qty:.6f}")
                        continue
                    fee = qty * price * (config.maker_fee if order['type'] == 'LIMIT' 
else config.taker_fee)
                    if side.upper() == 'BUY':
                        profit = (current_price - price) * qty
                    else:
                        profit = (price - current_price) * qty
                    ...
                    logging.info(f"[PAPER] Lệnh {side.upper()} đã khớp: Giá 
{price:.2f}, SL {qty:.6f}, Lợi nhuận {profit:.2f} USDT")
                    await send_telegram_alert(msg)
                    self.completed_orders.append({"side": side.upper(), "price": 
price, "quantity": qty})
                elif order['status'] == 'CANCELED':
                    if order.get('side', '').upper() == 'BUY' and order.get('orderId') in 
grid_manager.order_ids:
                        grid_manager.order_ids.remove(order['orderId'])
        except Exception as e:
    ...
    try:
        ...
        for name, state in states.items():
            if name in state_data:
                ...
                state.grid_manager.static_order_ids = 
data.get("static_order_ids", [])
...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: 
Optional['ExchangeInterface'] = None) -> None:
        ...
        atr = calculate_atr(list(self.high_history), list(self.low_history), 
list(self.price_history), config.atr_period)
...
min(config.dense_grid_step_percent_max / 100, raw_step))
        ...
        for i in range(num_levels):
            ...
            if buy_price <= 0 or sell_price <= 0:
                continue
            if abs(buy_price - current_price) / current_price > max_distance or 
abs(sell_price - current_price) / current_price > max_distance:
                continue
    ...
    def _create_static_grid(self, current_price: float) -> List[Tuple[float, float]]:
        ...
        for i in range(max(self.static_levels, config.min_grid_levels)):
            ...
            if buy_price <= 0 or sell_price <= 0:
                continue
            if abs(buy_price - current_price) / current_price > 0.05 or 
abs(sell_price - current_price) / current_price > 0.05:
                continue
            grid.append((buy_price, sell_price))
        return grid

    def duplicate_order_exists(self, side: str, price: float, open_orders: 
List[Dict]) -> bool:
    ...
    async def place_static_orders(self, current_price: float, exchange: 
'ExchangeInterface') -> None:
        ...
        rsi_val = calculate_rsi(list(self.price_history), config.rsi_period) if len(self.price_history) 
>= config.rsi_period else 50.0
        ...
        async with order_lock:
            ...
            allocated_funds_per_order = (usdt_balance * (1 - config.capital_reserve_percent / 100)) / 
(2 * num_levels)
            quantity_per_level = allocated_funds_per_order / current_price if 
current_price > 0 else 0.0
            ...
            for buy_price, sell_price in static_grid:
                try:
                    if buy_price <= 0 or sell_price <= 0:
                        continue
                    profit_percent = (sell_price - buy_price) / buy_price * 100 if buy_price > 
0 else 0
                    if profit_percent < config.min_profit_percent:
                        continue
                    if usdt_balance < buy_price * quantity:
                        logging.warning("USDT không đủ để đặt lệnh MUA tĩnh, dừng đặt 
lệnh tiếp theo")
                        break
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        ...
                        logging.info(f"(Static) Đã đặt lệnh MUA tại {buy_price:.2f}, SL: 
{quantity:.6f}")
                except Exception as e:
                    logging.error(f"(Static) Lỗi khi đặt lệnh: {str(e)}")
                    await send_telegram_alert(f"(Static) Lỗi khi đặt lệnh: 
{str(e)}")
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        async with order_lock:
            ...
            allocated_funds_per_order = (usdt_balance * (1 - 
config.capital_reserve_percent / 100)) / (2 * num_levels)
            quantity_per_level = allocated_funds_per_order / current_price if 
current_price > 0 else 0.0
            quantity = max(filters['minQty'], quantity_per_level)
            quantity = min(quantity, filters['maxQty'])
            ...
            for i, (buy_price, sell_price) in 
enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                usdt_balance, coin_balance = await 
safe_api_call(exchange.get_balance)
                ...
                try:
                    ...
                    if buy_price < current_price:
                        ...
                        if current_time < self.grid_cooldown.get(price_key, 0):
                            continue
                        if usdt_balance < buy_price * quantity:
                            logging.warning("USDT không đủ để đặt lệnh MUA, dừng 
vòng lặp đặt lệnh")
                            break
                        if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                            buy_order = await safe_api_call(
                            ...
                            self.order_ids.append(buy_order['orderId'])
                            open_count += 1
                            logging.info(f"(Dynamic) Đã đặt lệnh MUA tại {buy_price:.2f}, 
SL: {quantity:.6f}")
                    if sell_price > current_price:
                        price_key = ("SELL", round(sell_price, self.price_precision))
                        if current_time < self.grid_cooldown.get(price_key, 0):
                            continue
                        if not self.duplicate_order_exists("SELL", sell_price, 
open_orders):
                            if config.max_open_orders and open_count >= 
config.max_open_orders:
                                logging.warning(f"Đạt giới hạn lệnh tối đa 
({open_count}/{config.max_open_orders})")
                                break
                            ...
                            open_count += 1
                            logging.info(f"(Dynamic) Đã đặt lệnh BÁN tại {sell_price:.2f}, 
SL: {quantity:.6f}")
                except Exception as e:
                    logging.error(f"(Dynamic) Lỗi khi đặt lệnh: {str(e)}")
                    await send_telegram_alert(f"(Dynamic) Lỗi khi đặt lệnh: 
{str(e)}")

            has_buy_order = any(o['side'].upper() == 'BUY' for o in open_orders)

            if not has_buy_order and len([o for o in open_orders if 
o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > 
self.FALLBACK_COOLDOWN:
                buy_price = current_price * (1 - config.base_grid_step_percent / 
200)
                if buy_price > 0 and not self.duplicate_order_exists("BUY", buy_price, 
open_orders):
                    buy_order = await safe_api_call(
                        exchange.place_order,
                        side='BUY',
                    ...
                    if 'orderId' in buy_order:
                        ...
                        logging.info(f"(Fallback) Đã đặt lệnh MUA tại 
{buy_price:.2f}, SL: {quantity:.6f}")

    async def cancel_all_orders(self, exchange: 'ExchangeInterface', 
preserve_static: bool = True) -> None:
        async with order_lock:
            orders_to_cancel = self.order_ids.copy()
            if preserve_static:
                orders_to_cancel = [oid for oid in orders_to_cancel if oid not 
in self.static_order_ids]
...
class GridStrategy(ABC):
    ...
    @abstractmethod
    async def generate_signals(self, data: MarketData) -> List[MarketData]:
        pass

class AdaptiveGridStrategy(GridStrategy):
    async def generate_signals(self, data: MarketData) -> List[TradeSignal]:
        signals = []
        rsi = calculate_rsi(list(self.grid_manager.price_history), 
config.rsi_period) if len(self.grid_manager.price_history) >= config.rsi_period else 
50.0
        rsi_low, rsi_high = auto_adjust_rsi_thresholds(list(self.grid_manager.price_history))
        if rsi < rsi_low:
            signals.append(TradeSignal(side="BUY", price=data.price * 0.999, 
quantity=config.min_quantity))
        elif rsi > rsi_high:
            signals.append(TradeSignal(side="SELL", price=data.price * 1.001, 
quantity=config.min_quantity))
        low_grid, high_grid = self.grid_manager.get_grid_range()
        if low_grid and data.price < low_grid:
            signals.append(TradeSignal(side="BUY", price=data.price, 
quantity=config.min_quantity))
        if high_grid and data.price > high_grid:
            signals.append(TradeSignal(side="SELL", price=data.price, 
quantity=config.min_quantity))
        return signals
...
class BotState:
    def __init__(self, exchange: 'ExchangeInterface', strategy: GridStrategy):
        ...
        self.live_paper_trading: bool = config.live_paper_trading and 
self.exchange_name == "paper_trading"
    ...
    async def process_events(self) -> None:
                ...
                for attempt in range(config.api_retry_count):
                    try:
                        async with asyncio.timeout(20):
                            ...
                            if new_price > 0:  # Chỉ cập nhật nếu giá hợp lệ
        ...
        signals = await self.strategy.generate_signals(MarketData(price=self.current_price, 
high=self.current_high, low=self.current_low, volume=self.current_volume))
        async with order_lock:
            for signal in signals:
                try:
                    order = await self.exchange.place_order(signal.side, "LIMIT", 
signal.quantity, signal.price)
                    ...
                    logging.error(f"Lỗi khi đặt lệnh: {signal.side} tại {signal.price}: 
{str(e)}")
        ...
        if capital < config.initial_investment * (config.stop_loss_capital_percent / 
100):
            ...
            if highest_price >= target_price:
                ...
                if self.current_price <= stop_price and self.current_price >= entry_price:
                    ...
                    if self.exchange.simulated_balance[base_asset] < quantity:
                        logging.warning(f"Số dư {base_asset} không đủ: 
{self.exchange.simulated_balance[base_asset]} < {quantity}")
                        ...
                        logging.info(f"Chốt lời: Bán {quantity:.6f} {base_asset} @ 
{sell_price:.2f}, lợi nhuận {profit:.2f}")
                        await send_telegram_alert(f"Chốt lời: Bán {quantity:.6f} {base_asset} @ 
{sell_price:.2f}, lợi nhuận {profit:.2f}")
    ...
    async def send_hourly_report(self):
        if not config.telegram_enabled or not config.telegram_bot_token or not 
config.telegram_chat_id:
        ...
        base_asset = config.symbol.replace("USDT", "")
        report_msg = (
            f"📊 Báo cáo sau {int((time.time()-self.last_report_time)/60)} 
phút:\n"
            f"Giá hiện tại: {self.current_price:.2f} {config.symbol}\n"
            f"Số dư: {usdt_balance:.2f} USDT, {coin_balance:.6f} 
{base_asset}\n"
...
async def check_protections(state: BotState, exchange: 'ExchangeInterface', price: float, 
volume: float, high: float, low: float) -> bool:
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def get_price(self) -> float:
        ...
        try:
            ...
            ticker = await mexc.fetch_ticker(self.symbol if "/" in self.symbol else self.symbol.replace("USDT", "/USDT"))
            self.current_market_price = float(ticker['last'])
            self.current_volume = float(ticker['quoteVolume']) if 'quoteVolume' in ticker else 0.0
            self.current_high = float(ticker['high'])
            self.current_low = float(ticker['low'])
            self.last_api_update = current_time
            logging.info(f"[PAPER] Lấy giá từ MEXC: {self.current_market_price:.2f}")
            return self.current_market_price
        except ccxt.NetworkError as e:
            logging.error(f"Lỗi mạng khi lấy giá: {str(e)}")
            return self.current_market_price
        except ccxt.RateLimitExceeded as e:
            logging.error(f"Rate limit MEXC bị vượt quá: {str(e)}")
            return self.current_market_price
        except Exception as e:
            logging.error(f"Lỗi khi lấy giá: {str(e)}")
            return self.current_market_price
        finally:
            try:
                await mexc.close()
    ...
    async def get_open_orders(self) -> List[Dict]:
        return [order for order in self.paper_orders if order["status"] == "OPEN"]
