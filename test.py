...
# ======== HÀM TIỆN ÍCH (ATR, RSI, max drawdown, Sharpe Ratio, auto-adjust RSI) ========
...
def calculate_max_drawdown(equity_curve: List[float]) -> float:
    ...
    for x in equity_curve:
        ...
        if dd > max_dd:
            max_dd = dd
    return max_dd * 100  # trả về dưới dạng %
...
def auto_adjust_rsi_thresholds(prices: List[float], default_low=30, default_high=70) -> Tuple[float, float]:
    # Ví dụ đơn giản: nếu thị trường có biến động thấp, thu hẹp ngưỡng; nếu cao, mở rộng
...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        step_percent = min(volatility / num_levels if num_levels > 0 else config.base_grid_step_percent / 100, 0.02)
    ...
    async def place_static_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        current_time = time.time()
        if current_time - getattr(self, 'last_static_order_time', 0) < config.order_cooldown_sec:
            logging.info("Chưa hết thời gian cooldown cho static orders, bỏ qua.")
            return
        ...
        if not (calculate_rsi(list(self.price_history), config.rsi_period) >= 40 and calculate_rsi(list(self.price_history), config.rsi_period) <= 60):
            logging.info("Thị trường không đủ ổn định theo chỉ số RSI, bỏ qua lệnh static.")
            return
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        try:
            ...
            if not signals and len(self.grid_manager.grid_levels) > 0:
                # Fallback: sử dụng mức đầu tiên của grid
...
# Hàm safe_api_call: wrapper để gọi API an toàn (có thể bổ sung retry logic nếu cần)
...
if __name__ == "__main__":
    async def main():
        # Ví dụ: sử dụng PaperTradingExchange
