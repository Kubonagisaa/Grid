...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if config.trailing_stop_enabled:
            to_remove = []
            # Duyệt qua các vị thế static BUY đang chờ chốt lời
            for (side, entry_price), val in list(self.grid_manager.trailing_stops.items()):
                if side.upper() == "BUY" and isinstance(val, (list, tuple)) and len(val) >= 2:
                    highest_price = val[0] if isinstance(val[0], (int, float)) else 0.0
                    quantity = float(val[1])
                    if highest_price == 0.0:
                        highest_price = entry_price
                    highest_price = max(highest_price, self.current_high)
        ...
        if config.telegram_enabled:
            if current_time - self.last_report_time >= 3600:
                try:
                    usdt_balance, coin_balance = safe_api_call(self.exchange.get_balance)
                except Exception as e:
...
async def send_open_orders(state: BotState) -> None:
    try:
        open_orders = safe_api_call(state.exchange.get_open_orders)
    except Exception as e:
    ...
    async def get_price(self) -> float:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Lỗi khi đóng kết nối mexc: {str(e)}")
