...
# ======== HYBRID GRID STRATEGY (THAY ĐỔI CHÍNH) ========
class EnhancedSmartGridManager:
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        has_buy_order = False
        current_time = time.time()
        # --- PHẦN HYBRID: Đặt ngay N mức đầu (ví dụ: N=3) ---
        for i, (buy_price, sell_price) in enumerate(self.grid_levels):
            is_static_level = (i < 3)  # 3 lệnh đầu luôn đặt ngay
            ...
            try:
                ...
                # Đặt lệnh BUY: nếu đây là static level (đặt ngay) hoặc nếu giá hiện tại đã thấp hơn mức lưới
                if (is_static_level or buy_price < current_price) and current_time >= cooldown_buy:
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        ...
                        has_buy_order = True
                        logging.info(f"(Hybrid) Đã đặt lệnh MUA tại {buy_price:.2f}, số lượng: {quantity:.6f}, orderId: {buy_order['orderId']}")
                        await send_telegram_alert(f"(Hybrid) BUY: {buy_price:.2f}, SL: {quantity:.6f}")
                    else:
                        logging.info(f"(Hybrid) Bỏ qua lệnh BUY tại {buy_price:.2f} do đã tồn tại lệnh trùng.")
                else:
                    logging.info(f"(Hybrid) Bỏ qua lệnh BUY tại {buy_price:.2f} do cooldown hoặc giá không phù hợp.")

                # Đặt lệnh SELL: nếu đây là static level (đặt ngay) hoặc nếu giá hiện tại cao hơn mức lưới
                if (is_static_level or sell_price > current_price) and current_time >= cooldown_sell:
                    if not self.duplicate_order_exists("SELL", sell_price, open_orders):
                        ...
                        logging.info(f"(Hybrid) Đã đặt lệnh BÁN tại {sell_price:.2f}, số lượng: {quantity:.6f}, orderId: {sell_order['orderId']}")
                        await send_telegram_alert(f"(Hybrid) SELL: {sell_price:.2f}, SL: {quantity:.6f}")
                    else:
                        logging.info(f"(Hybrid) Bỏ qua lệnh SELL tại {sell_price:.2f} do đã tồn tại lệnh trùng.")
                else:
                    logging.info(f"(Hybrid) Bỏ qua lệnh SELL tại {sell_price:.2f} do cooldown hoặc giá không phù hợp.")
            except Exception as e:
                logging.error(f"(Hybrid) Lỗi khi đặt lệnh: {str(e)}")
                await send_telegram_alert(f"(Hybrid) Lỗi khi đặt lệnh: {str(e)}")
                ...
                if buy_price > 0:
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        ...
                        if 'orderId' not in buy_order:
                            ...
                            return
                        self.order_ids.append(buy_order['orderId'])
                        logging.info(f"(Hybrid) Đã đặt lệnh MUA động tại {buy_price:.2f}, số lượng: {quantity:.6f}, orderId: {buy_order['orderId']}")
                        await send_telegram_alert(f"(Hybrid) BUY động: {buy_price:.2f}, SL: {quantity:.6f}")
                    else:
                        logging.info(f"(Hybrid) Bỏ qua lệnh MUA động tại {buy_price:.2f} do đã tồn tại lệnh trùng.")
            except Exception as e:
                logging.error(f"(Hybrid) Lỗi khi đặt lệnh MUA động: {str(e)}")
                await send_telegram_alert(f"(Hybrid) Lỗi khi đặt lệnh MUA động: {str(e)}")
...
# Hàm dummy để kiểm tra bảo vệ, trả về False (bạn có thể bổ sung logic cụ thể)
...
class PaperTradingExchange(ExchangeInterface):
            ...
            if order["status"] == "OPEN":
                if (order["side"] == "BUY" and price <= order["price"]) or (order["side"] == "SELL" and price >= order["price"]):
                    ...
                    if order["side"] == "BUY":
                        ...
                        self.simulated_balance[base_asset] -= order["quantity"]
                        self.simulated_balance["USDT"] += cost
                    logging.info(f"Khớp lệnh thành công: {order}")
                    await send_telegram_alert(f"Khớp lệnh: {order['side']} tại {order['price']:.2f}, qty: {order['quantity']:.6f}")
                    ...
                    try:
                        ...
                        for o in self.paper_orders:
                            if o['orderId'] == order['orderId'] and o['status'] == 'FILLED':
                                ...
                                profit = ((executed_price * (1 + config.base_take_profit_percent / 100) - executed_price) * signal.quantity
                                          if signal.side == "BUY"
                                ...
                                await send_telegram_alert(
                                    f"Giao dịch hoàn tất\nSide: {signal.side}\nGiá: {executed_price:.2f}\nSố lượng: {signal.quantity:.6f}\nLợi nhuận: {profit:.2f} USDT\nPhí: {fee:.4f} USDT"
                                )
                    except Exception as e:
    ...
    async def run_paper_trading(self):
        # Nếu bạn muốn chạy chế độ paper trading theo event loop riêng
        state = BotState(self, AdaptiveGridStrategy(EnhancedSmartGridManager()))
        while True:
...
# ======== HÀM TIỆN ÍCH (DUMMY IMPLEMENTATIONS) ========
...
# ======== MAIN (KHỞI ĐỘNG PAPER TRADING LIVE) ========
if __name__ == "__main__":
    # Kiểm tra xem có bật PAPER_TRADING và LIVE_PAPER_TRADING không
    if not (config.paper_trading_enabled and config.live_paper_trading):
        logging.error("Chế độ paper trading live chưa được bật! Vui lòng bật PAPER_TRADING và LIVE_PAPER_TRADING trong file .env")
        exit(1)

    async def main():
        # Khởi tạo sàn giao dịch paper trading
        exchange = PaperTradingExchange(config.symbol)
        # Khởi tạo chiến lược grid với EnhancedSmartGridManager (Hybrid)
        ...
        # Khởi động paper trading live
        await exchange.run_paper_trading_live(state)

    try:
        asyncio.run(main())
    except Exception as e:
        logging.error(f"Bot paper trading gặp lỗi: {str(e)}")