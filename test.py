...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        _ = MarketData(price=current_price, high=current_price * 1.01, low=current_price * 0.99, volume=1000)
            ...
            for buy_price, sell_price in static_grid:
                try:
                    ...
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        buy_order = await exchange.place_order('BUY', 'LIMIT', quantity, buy_price)
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        async with order_lock:
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                ...
                try:
                    if buy_price <= 0 or sell_price <= 0:
                        logging.warning(f"Giá không hợp lệ: buy_price={buy_price}, sell_price={sell_price}. Bỏ qua mức lưới này.")
                        continue
                    if buy_price < current_price and current_time >= self.grid_cooldown.get(("BUY", round(buy_price, self.price_precision)), 0):
                        if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                            buy_order = await exchange.place_order('BUY', 'LIMIT', quantity, buy_price)
                            if 'orderId' not in buy_order:
                    ...
                    if sell_price > current_price and current_time >= self.grid_cooldown.get(("SELL", round(sell_price, self.price_precision)), 0):
                        if not self.duplicate_order_exists("SELL", sell_price, open_orders):
                            if config.max_open_orders and open_count >= config.max_open_orders:
                                break
                            sell_order = await exchange.place_order('SELL', 'LIMIT', quantity, sell_price)
            ...
            has_buy_order_current = has_buy_order or any(o['side'].upper() == 'BUY' for o in open_orders)
            if (not has_buy_order_current and len([o for o in open_orders if o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > self.FALLBACK_COOLDOWN):
                try:
                    fallback_buy_price = current_price * (1 - config.base_grid_step_percent / 200)
                    if fallback_buy_price > 0 and not self.duplicate_order_exists("BUY", fallback_buy_price, open_orders):
                        buy_order = await exchange.place_order('BUY', 'LIMIT', quantity, fallback_buy_price)
                        if 'orderId' not in buy_order:
                            logging.error(f"Lệnh MUA fallback không trả về orderId: {buy_order}")
                            return
                        self.order_ids.append(buy_order['orderId'])
                        self.last_fallback_time = current_time
                        logging.info(f"(Fallback) Đã đặt lệnh MUA tại {fallback_buy_price:.2f}, số lượng: {quantity:.6f}, orderId: {buy_order['orderId']}")
                        await send_telegram_alert(f"(Fallback) BUY: {fallback_buy_price:.2f}, SL: {quantity:.6f}")
                    else:
                        logging.info(f"(Fallback) Bỏ qua lệnh BUY tại {fallback_buy_price:.2f} do đã tồn tại lệnh trùng.")
                except Exception as e:
...
class EnhancedOrderManager:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface',
        ...
        """
        try:
            orders = await exchange.get_all_orders()
            for order in orders:
                if 'orderId' not in order:
                    logging.warning(f"Lệnh không có orderId: {order}")
                    continue
                if order['status'] == 'FILLED':
                    ...
                    if not any(o.get('orderId') == order.get('orderId') for o in self.open_orders):
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if current_time - self.last_price_update >= 5:
            try:
                self.current_price = await self.exchange.get_price()
                self.current_volume = await self.exchange.get_volume()
                self.current_high, self.current_low = await self.exchange.get_high_low()
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def run_paper_trading_live(self, state: BotState, interval: int = 1) -> None:
        """Chạy vòng lặp giao dịch giả lập (paper trading) liên tục với khoảng thời gian giữa các chu kỳ là interval giây."""
        try:
            while True:
                try:
            ...
            logging.info("run_paper_trading_live bị hủy.")
        finally:
            # Có thể thêm logic lưu trạng thái hoặc đóng kết nối tại đây nếu cần
            pass
