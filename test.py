...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            # Fix phần 1: Xử lý quantity None và filter
            atr_value = calculate_atr(list(self.high_history), list(self.low_history), list(self.price_history), config.atr_period)
            quantity = dynamic_position_sizing(atr_value)
            if quantity is None:
                logging.error("Lỗi tính quantity: dynamic_position_sizing trả về None")
                await send_telegram_alert("Lỗi nghiêm trọng: Không tính được quantity")
                return
                
            min_qty = filters.get('minQty', 0.0)
            max_qty = filters.get('maxQty', 10000.0)
            ...
            # Fix phần 4: Kiểm tra minQty/maxQty
            if min_qty is None or max_qty is None:
                logging.error("Lỗi filter: minQty/maxQty không hợp lệ")
                await send_telegram_alert("Lỗi filter: minQty/maxQty không hợp lệ")
                return
            ...
            for i, (buy_price, sell_price) in enumerate(static_grid, 1):
                ...
                if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                    ...
                    # Fix phần 2: Kiểm tra min_price và max_price
                    min_price = filters.get('minPrice')
                    max_price = filters.get('maxPrice')
                    if min_price is None or max_price is None:
                        logging.error("Lỗi filter giá: minPrice/maxPrice không hợp lệ")
                        continue
                    if buy_price < min_price or buy_price > max_price:
                        logging.warning(f"Lệnh {i} không đặt được: Giá {buy_price} ngoài khoảng [{min_price}, {max_price}]")
                        continue
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            # Fix phần 1: Xử lý quantity None
            atr_value = calculate_atr(list(self.high_history), list(self.low_history), list(self.price_history), config.atr_period)
            quantity = dynamic_position_sizing(atr_value)
            if quantity is None:
                logging.error("Lỗi tính quantity: dynamic_position_sizing trả về None")
            ...
            # Fix phần 4: Kiểm tra quantity hợp lệ
            if min_qty is None or max_qty is None:
                logging.error("Lỗi filter: minQty/maxQty không hợp lệ")
                await send_telegram_alert("Lỗi filter: minQty/maxQty không hợp lệ")
                return
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                ...
                if buy_price is not None and current_price is not None and buy_price < current_price:
                    ...
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        buy_price = round(buy_price / tick_size) * tick_size
                        # Fix phần 2: Kiểm tra min/max price
                        min_price = filters.get('minPrice')
                        max_price = filters.get('maxPrice')
                        if min_price is None or max_price is None:
                            logging.error("Lỗi filter giá: minPrice/maxPrice không hợp lệ")
                ...
                if sell_price is not None and current_price is not None and sell_price > current_price:
                    ...
                    # Fix phần 3: Kiểm tra cho sell price
                    min_price = filters.get('minPrice')
                    max_price = filters.get('maxPrice')
                    if min_price is None or max_price is None:
                        logging.error("Lỗi filter giá: minPrice/maxPrice không hợp lệ")
                        continue
                    if sell_price < min_price or sell_price > max_price:
                        continue
            ...
            if not has_buy_order and len([o for o in open_orders if isinstance(o, dict) and o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > self.FALLBACK_COOLDOWN:
                ...
                if (buy_price is not None and buy_price > 0 and 
                    ...
                    min_price is not None and max_price is not None and
                    buy_price >= min_price and buy_price <= max_price):
                    buy_order = await safe_api_call(exchange.place_order, side='BUY', order_type='LIMIT', quantity=quantity, price=buy_price, alert_on_failure=True)
                    if buy_order and 'orderId' in buy_order:
                        self.order_ids.append(buy_order['orderId'])
...
class TrendDetector:
    ...
    async def _activate_anti_trend_mode(self):
        ...
        await send_telegram_alert("Chuyển sang chiến lược Breakout do xu hướng mạnh")
...
class PaperTradingExchange(IExchange):
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        ...
        min_price = filters.get('minPrice', 0.0)
        max_price = filters.get('maxPrice', 0.0)
        if price < min_price or price > max_price:
            logging.warning(f"Giá {price} ngoài khoảng cho phép [{min_price}, {max_price}]")
            return {}
        order = {
    ...
    async def get_symbol_filters(self) -> Dict:
        ...
        try:
            ...
            return {
                "minQty": float(limits['amount'].get('min', 1.0)),
                "maxQty": float(limits['amount'].get('max', 10000.0)),
                "stepSize": market['precision'].get('amount', 0.00001),
                "minPrice": float(limits['price'].get('min', 0.00001)),
                "maxPrice": float(limits['price'].get('max', 10000.0)),
                "tickSize": market['precision'].get('price', 0.00001)
            }
        except Exception as e:
