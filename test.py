    ...
    for i in range(len(historical_data)):
        ...
        for signal in signals:
            try:
                ...
                # Giả lập khớp lệnh ngay lập tức trong mô phỏng
                order['status'] = 'FILLED'
                order['executedQty'] = order['quantity']
                profit = (price * (1 + config.base_take_profit_percent / 100) - price) * signal.quantity if signal.side == "BUY" else (price - price * (1 - config.base_stop_loss_percent / 100)) * signal.quantity
                fee = signal.quantity * price * config.maker_fee
                profit_tracker.record_trade(profit, fee)
            except Exception as e:
    ...
    if time.time() - last_status_update > config.status_update_interval:
        try:
            ...
            message = (
                ...
                f"**Độ Trễ API**: {latency_str}"
            )
            logging.info(f"Trạng thái bot: {message}")
            send_telegram_alert(message)
            return time.time()
        except Exception as e:
            logging.error(f"Lỗi khi cập nhật trạng thái: {str(e)}")
            send_telegram_alert(f"⚠️ Lỗi khi cập nhật trạng thái: {str(e)}")
...
def monitor_system_health() -> Dict[str, float]:
    ...
    if cpu_usage > 90 or memory_usage > 90 or disk_usage > 90:
        warning_message = (
            f"⚠️ Cảnh báo tài nguyên hệ thống:\n"
            f"CPU: {cpu_usage:.2f}%\n"
            f"Memory: {memory_usage:.2f}%\n"
            f"Disk: {disk_usage:.2f}%"
        )
        logging.warning(warning_message)
        send_telegram_alert(warning_message)
    return system_health

# ======== XỬ LÝ TÍN HIỆU DỪNG BOT ========
def signal_handler(sig: int, frame: Any) -> None:
    logging.info(f"Nhận tín hiệu dừng bot: {sig}")
    send_telegram_alert("🛑 Bot đang dừng...")
    raise KeyboardInterrupt

signal.signal(signal.SIGINT, signal_handler)
signal.signal(signal.SIGTERM, signal_handler)

# ======== GIAO DIỆN NGƯỜI DÙNG (DASH) ========
def create_dashboard(states: Dict[str, BotState]) -> dash.Dash:
    ...
    @app.callback(
        [Output('bot-stats', 'children'),
         ...
         Output('grid-levels', 'figure')],
        [Input('interval-component', 'n_intervals')]
    )
    def update_dashboard(n_intervals: int) -> Tuple[List[html.Div], go.Figure, go.Figure, go.Figure]:
        stats_display = []
        ...
        for state in states.values():
            stats = state.tracker.get_stats()
            stats_display.append(html.Div([
                html.H3(f"Bot on {state.exchange_name}"),
                ...
                html.P(f"Current Price: {state.current_price:.2f}"),
            ]))
            ...
            equity_fig.add_trace(go.Scatter(
                ...
                mode='lines',
                name=f'Equity Curve ({state.exchange_name})'
            ))
            ...
            price_fig.add_trace(go.Scatter(
                ...
                y=price_history,
                mode='lines',
                name=f'Price ({state.exchange_name})'
            ))

            low_grid, high_grid = state.grid_manager.get_grid_range()
            if low_grid and high_grid:
                grid_fig.add_trace(go.Scatter(
                    x=list(range(len(price_history))),
                    y=[low_grid] * len(price_history),
                    mode='lines',
                    name=f'Lower Grid ({state.exchange_name})',
                    line=dict(dash='dash')
                ))
                grid_fig.add_trace(go.Scatter(
                    x=list(range(len(price_history))),
                    y=[high_grid] * len(price_history),
                    mode='lines',
                    name=f'Upper Grid ({state.exchange_name})',
                    line=dict(dash='dash')
                ))
        ...
        grid_fig.update_layout(title="Grid Levels", xaxis_title="Time", yaxis_title="Price")

        return stats_display, equity_fig, price_fig, grid_fig

    return app

# ======== CHẠY BOT CHÍNH ========
async def run_bot_for_exchange(exchange: ExchangeInterface, state: BotState) -> None:
    logging.info(f"Starting bot for {exchange.exchange_name}")
    last_status_update = time.time()
    downtime_start: Optional[float] = None

    try:
        if not await check_api_connectivity(exchange):
            logging.error(f"Không thể kết nối với {exchange.exchange_name}")
            return

        if not await check_balance_before_trading(exchange):
            logging.error(f"Số dư không đủ để giao dịch trên {exchange.exchange_name}")
            return
        ...
        while True:
            try:
                if not await check_api_connectivity(exchange):
                    should_stop, downtime_start = await handle_exchange_downtime(state, downtime_start)
                    if should_stop:
                        break
                    continue
                downtime_start = None
                ...
                logging.info(f"Trạng thái hệ thống: {system_health}")
                ...
                logging.info(f"Bot trên {exchange.exchange_name} đang dừng...")
                send_telegram_alert(f"🛑 Bot trên {exchange.exchange_name} đang dừng...")
                break
            except Exception as e:
                logging.error(f"Lỗi trong vòng lặp chính trên {exchange.exchange_name}: {str(e)}")
                send_telegram_alert(f"⚠️ Lỗi trên {exchange.exchange_name}: {str(e)}")
                await asyncio.sleep(60)

    finally:
        await exchange.close()
        logging.info(f"Bot trên {exchange.exchange_name} đã dừng hoàn toàn")
...
async def main() -> None:
    ...
    try:
        ...
        dashboard_app = create_dashboard(states)
        dashboard_thread = Thread(target=lambda: dashboard_app.run_server(debug=False, port=8050, host='0.0.0.0'))
        dashboard_thread.start()

        tasks = [run_bot_for_exchange(exchange, states[name]) for name, exchange in exchange_manager.exchanges.items()]
        await asyncio.gather(*tasks)

    except KeyboardInterrupt:
        logging.info("Dừng bot do tín hiệu KeyboardInterrupt")
    except Exception as e:
        logging.error(f"Lỗi chính: {str(e)}")
        send_telegram_alert(f"❌ Lỗi chính: {str(e)}")
    finally:
        await exchange_manager.close_all()
        logging.info("Bot đã dừng hoàn toàn")
        send_telegram_alert("🛑 Bot đã dừng hoàn toàn")
