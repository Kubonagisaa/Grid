...
class EnhancedOrderManager:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface',
        ...
        try:
            ...
            for order in orders:
                if 'orderId' not in order or 'status' not in order:
                    continue
...
async def verify_order_state(exchange: 'ExchangeInterface', local_order_ids: List[str]) -> None:
    try:
        ...
        actual_ids = {order.get('orderId') for order in actual_orders if 'orderId' in order}
...
class NewsAnalyzer:
    ...
    async def update_sentiment(self) -> None:
        url = f"https://newsapi.org/v2/everything?q=Dogecoin&apiKey={self.news_api_key}"
        try:
            async with aiohttp.ClientSession() as session:
...
async def safe_api_call(func, *args, retries=3, delay=2, **kwargs):
    for attempt in range(retries):
        try:
            ...
            if result is None:
                raise ValueError(f"{func.__name__} trả về None")
            return result
        except Exception as e:
            logging.warning(f"API error on attempt {attempt+1}/{retries}: {str(e)}")
