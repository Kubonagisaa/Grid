...
class AdvancedConfig:
    def __init__(self):
        ...
        self.max_grid_levels = int(os.getenv("MAX_GRID_LEVELS", 10))
        ...
        self.volatility_window = int(os.getenv("VOLATILITY_WINDOW", 100))
        ...
        self.atr_sensitivity = float(os.getenv("ATR_SENSITIVITY", 1.0))
        self.drawdown_recovery_threshold = float(os.getenv("DRAWDOWN_RECOVERY_THRESHOLD", 5.0))
        self.use_multi_timeframe = os.getenv("USE_MULTI_TIMEFRAME_ANALYSIS", "True").lower() == "true"
        self.use_position_sizing = os.getenv("USE_POSITION_SIZING", "True").lower() == "true"
        self.max_risk_per_trade = float(os.getenv("MAX_RISK_PER_TRADE", 0.02))
        self.volume_spike_threshold = float(os.getenv("VOLUME_SPIKE_THRESHOLD", 3.0))
...
def send_telegram_chart(exchange_name: str, prices: List[float], indicators: Dict[str, List[float]]):
    if not config.telegram_enabled:
        return
    fig = go.Figure()
    fig.add_trace(go.Scatter(y=prices, name='Price'))
    for name, values in indicators.items():
        fig.add_trace(go.Scatter(y=values, name=name))
    img_path = f"{exchange_name}_chart.png"
...
class AdaptiveGridStrategy(GridStrategy):
    def __init__(self):
        self.price_history_1h = deque(maxlen=config.volatility_window)
        self.price_history_4h = deque(maxlen=config.volatility_window // 4)
        self.price_history_1d = deque(maxlen=config.volatility_window // 24)
        self.high_history = deque(maxlen=config.volatility_window)
        self.low_history = deque(maxlen=config.volatility_window)

    async def generate_signals(self, market_data: MarketData) -> List[TradeSignal]:
        self.price_history_1h.append(market_data.price)
        self.high_history.append(market_data.high)
        self.low_history.append(market_data.low)
        if len(self.price_history_1h) % 4 == 0:
            self.price_history_4h.append(market_data.price)
        if len(self.price_history_1h) % 24 == 0:
            self.price_history_1d.append(market_data.price)

        if len(self.price_history_1h) < config.rsi_period:
            return []
        
        signals = []
        rsi_1h = calculate_rsi(list(self.price_history_1h), config.rsi_period)
        rsi_4h = calculate_rsi(list(self.price_history_4h), config.rsi_period) if len(self.price_history_4h) >= config.rsi_period else 50
        rsi_1d = calculate_rsi(list(self.price_history_1d), config.rsi_period) if len(self.price_history_1d) >= config.rsi_period else 50
        ema = calculate_ema(list(self.price_history_1h), config.ema_period)
        sma, bb_upper, bb_lower = calculate_bollinger_bands(list(self.price_history_1h), config.bb_period, config.bb_std_dev)
        atr = calculate_atr(list(self.high_history), list(self.low_history), list(self.price_history_1h), config.atr_period)
        volatility = atr / market_data.price if market_data.price > 0 else 0.01

        quantity = config.min_quantity
        if volatility > config.base_grid_step_percent / 100 * 1.5:
            quantity *= 2.0
        elif volatility > config.base_grid_step_percent / 100:
            quantity *= 1.5

        if config.use_multi_timeframe:
            if rsi_1h < 30 and rsi_4h < 40 and rsi_1d < 50 and market_data.price < ema and market_data.price <= bb_lower:
                signals.append(TradeSignal(side="BUY", quantity=quantity, price=market_data.price * 0.995))
                if rsi_1h < 20:
                    send_telegram_alert(f"📉 Buy Signal: RSI 1h={rsi_1h:.2f}, 4h={rsi_4h:.2f}, 1d={rsi_1d:.2f}")
            elif rsi_1h > 70 and rsi_4h > 60 and rsi_1d > 50 and market_data.price > ema and market_data.price >= bb_upper:
                signals.append(TradeSignal(side="SELL", quantity=quantity, price=market_data.price * 1.005))
                if rsi_1h > 80:
                    send_telegram_alert(f"📈 Sell Signal: RSI 1h={rsi_1h:.2f}, 4h={rsi_4h:.2f}, 1d={rsi_1d:.2f}")
        else:
            if rsi_1h < 30 and market_data.price < ema and market_data.price <= bb_lower:
                signals.append(TradeSignal(side="BUY", quantity=quantity, price=market_data.price * 0.995))
            elif rsi_1h > 70 and market_data.price > ema and market_data.price >= bb_upper:
                signals.append(TradeSignal(side="SELL", quantity=quantity, price=market_data.price * 1.005))
...
class EnhancedProfitTracker:
    ...
    def send_daily_summary(self) -> None:
        ...
        if current_time - self.last_summary_time >= 86400:  # 24 giờ
...
class EnhancedProtectionSystem:
    ...
    def check_volume_spike(self, volume: float) -> bool:
        return self.volume_spike_detector.check(volume)

class VolumeSpikeDetector:
    def __init__(self):
        self.volume_history = deque(maxlen=100)

    def check(self, volume: float) -> bool:
        self.volume_history.append(volume)
        if len(self.volume_history) < 20:
            return False
        avg_volume = sum(self.volume_history) / len(self.volume_history)
        return volume > avg_volume * config.volume_spike_threshold
...
class SmartGridManager:
    ...
    def calculate_market_conditions(self) -> Tuple[float, float]:
        ...
        volatility = atr * config.atr_sensitivity / closes[-1] if closes[-1] > 0 else 0.01
        ema = calculate_ema(closes, config.ema_period)
        trend_strength = (closes[-1] - ema) / ema * 100 if ema > 0 else 0
        return volatility, trend_strength

    def auto_adjust_grid_params(self, current_price: float, balance: float) -> None:
        volatility, trend_strength = self.calculate_market_conditions()
        base_levels = min(int(balance / (current_price * config.min_quantity)), config.max_grid_levels)
        volatility_factor = max(0.5, min(2.0, volatility / 0.01))
        trend_factor = 1.0 + abs(trend_strength) / 50
        self.optimal_levels = max(config.min_grid_levels, min(config.max_grid_levels, int(base_levels / volatility_factor / trend_factor)))
        self.grid_spacing = max(config.base_grid_step_percent / 100, min(volatility * 1.5, 0.02))
        logging.info(f"Grid auto-adjusted: levels={self.optimal_levels}, spacing={self.grid_spacing:.2%}, volatility={volatility:.2%}, trend={trend_strength:.2f}%")

    def adjust_grid_levels(self, drawdown: float) -> None:
        if drawdown > config.drawdown_threshold:
            new_levels = max(config.min_grid_levels, int(self.optimal_levels * 0.75))
            if new_levels != self.optimal_levels:
                self.optimal_levels = new_levels
                send_telegram_alert(f"⚠️ Grid levels reduced to {self.optimal_levels} (Drawdown: {drawdown:.2f}%)")
        elif drawdown < config.drawdown_recovery_threshold and self.optimal_levels < config.max_grid_levels:
            new_levels = min(config.max_grid_levels, int(self.optimal_levels * 1.25))
            if new_levels != self.optimal_levels:
                self.optimal_levels = new_levels
                send_telegram_alert(f"📈 Grid levels increased to {self.optimal_levels} (Drawdown: {drawdown:.2f}%)")
    ...
    async def update_grid(self, current_price: float, exchange: 'ExchangeInterface', drawdown: float) -> None:
        ...
        balance = await OrderManager(exchange).get_usable_balance()
        self.adjust_grid_levels(drawdown)
        self.auto_adjust_grid_params(current_price, balance)

        if not self.trailing_enabled or (self.last_price != 0.0 and abs(current_price - self.last_price) < config.grid_adjust_threshold * self.last_price):
            return

        self.last_price = current_price
        new_levels = []
        for i in range(self.optimal_levels):
            buy = current_price * (1 - (i + 1) * self.grid_spacing)
            sell = current_price * (1 + (i + 1) * self.grid_spacing)
            if buy > 0 and sell > 0:
                new_levels.append((buy, sell))

        self.grid_levels = new_levels
        logging.info(f"Grid updated - price: {current_price:.2f}, spacing={self.grid_spacing:.2%}, levels={len(new_levels)}")
...
class DynamicRiskManager:
    def __init__(self):
        ...
        self.position_sizing_enabled = config.use_position_sizing
        self.price_history = deque(maxlen=config.volatility_window)
        self.high_history = deque(maxlen=config.volatility_window)
        self.low_history = deque(maxlen=config.volatility_window)

    async def calculate_position_size(self, current_price: float, balance: float) -> float:
        if not self.position_sizing_enabled or len(self.price_history) < config.atr_period:
        ...
        return max(config.min_quantity, min(position_size, config.max_position))
...
class BinanceExchange(ExchangeInterface):
    ...
    async def get_all_orders(self) -> List[Dict]:
        ...
        return [{'orderId': o['id'], 'side': o['side'].upper(), 'price': o['price'], 'origQty': o['amount'], 'executedQty': o['filled'], 'type': o['type'].upper(), 'status': 'FILLED' if o['status'] == 'closed' else 'OPEN'} for o in orders]
...
class BotState:
    def __init__(self, exchange: 'ExchangeInterface', strategy: GridStrategy):
        ...
        self.last_chart_time = time.time()
    ...
    async def process_events(self) -> None:
        while True:
            ...
            try:
                ...
                self.risk_manager.price_history.append(market_data.price)
                self.risk_manager.high_history.append(market_data.high)
                self.risk_manager.low_history.append(market_data.low)
                
                if self.protection.check_volume_spike(market_data.volume):
                    send_telegram_alert(f"⚠️ Volume Spike Detected: {market_data.volume:.2f}")
                    await self.order_manager.cancel_all_orders(self.exchange)
                    await asyncio.sleep(60)  # Tạm dừng 1 phút
                    continue
                ...
                balance = await self.order_manager.get_usable_balance()
                quantity = await self.risk_manager.calculate_position_size(market_data.price, balance)
                ...
                if time.time() - self.last_chart_time > 3600:  # Gửi chart mỗi giờ
                    indicators = {
                        "EMA": list(self.tracker.price_history)[-config.ema_period:] + [calculate_ema(list(self.tracker.price_history), config.ema_period)] * (len(self.tracker.price_history) - config.ema_period),
                        "BB Upper": list(self.tracker.price_history)[-config.bb_period:] + [calculate_bollinger_bands(list(self.tracker.price_history), config.bb_period, config.bb_std_dev)[1]] * (len(self.tracker.price_history) - config.bb_period),
                        "BB Lower": list(self.tracker.price_history)[-config.bb_period:] + [calculate_bollinger_bands(list(self.tracker.price_history), config.bb_period, config.bb_std_dev)[2]] * (len(self.tracker.price_history) - config.bb_period)
                    }
                    send_telegram_chart(self.exchange.exchange_name, list(self.tracker.price_history), indicators)
                    self.last_chart_time = time.time()
            except Exception as e:
    ...
    async def run_simulated_backtest(self, timeframe: str = '1h', limit: int = 1000) -> Dict[str, float]:
        ...
        for candle in historical_data:
            ...
            balance = config.initial_investment  # Giả lập số dư cho backtest
            quantity = await self.risk_manager.calculate_position_size(market_data.price, balance)
            signals = await self.strategy.generate_signals(market_data)
            for signal in signals:
                signal.quantity = quantity
