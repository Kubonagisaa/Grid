...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            quantity = min(quantity, filters['maxQty']) if filters.get('maxQty') is not None else quantity
            tick_size = filters.get('tickSize', 0.00001)
            quantity = round(quantity / tick_size) * tick_size

            if quantity is not None and (quantity < filters['minQty'] or (filters.get('maxQty') is not None and quantity > filters['maxQty'])):
