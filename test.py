...
class BaseExchange(ExchangeInterface):
    ...
    async def get_high_low(self) -> Tuple[float, float]:
        try:
            ticker = await self.client.fetch_ticker(self.symbol)
            if 'high' not in ticker or 'low' in ticker or ticker['high'] is None or ticker['low'] is None:
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        ...
        self.paper_orders = []
        self.current_market_price = 100.0  # Giá trị mặc định
    ...
    async def _match_orders(self):
        ...
        # Khởi tạo base_asset trong simulated_balance nếu chưa có
...
def calculate_rsi(prices: List[float], period: int) -> float:
    ...
    if len(prices) < period + 1:
        logging.warning("Dữ liệu giá không đủ hoặc không hợp lệ để tính RSI")
        return 50.0
    
    rsi_calculator = RSICalculator(period)
    # Khởi tạo giá đầu tiên
    for price in prices[:period]:
        rsi_calculator.gains.append(price)
...
async def check_protections(state: BotState, exchange: ExchangeInterface, price: float, volume: float, high: float, low: float) -> bool:
    if state.protection.check_pump_protection(price):
        ...
        await send_telegram_alert("Phát hiện bơm giá, tạm dừng giao dịch")
        await state.order_manager.cancel_all_orders(exchange)
        return True
    if await state.protection.check_circuit_breaker(price):
        logging.warning("Ngắt mạch được kích hoạt, tạm dừng giao dịch")
        await state.order_manager.cancel_all_orders(exchange)
        return True
    if await state.protection.check_circuit_breaker_status():
        logging.info("Ngắt mạch vẫn đang hoạt động")
        return True
    if state.protection.check_abnormal_activity(volume):
        ...
        await state.order_manager.cancel_all_orders(exchange)
        return True
    should_stop, stop_price = state.protection.update_trailing_stop(price)
    if should_stop:
        logging.warning(f"Trailing stop được kích hoạt tại {stop_price:.2f}, hủy tất cả lệnh")
        await send_telegram_alert(f"Trailing stop được kích hoạt tại {stop_price:.2f}, hủy tất cả lệnh")
        await state.order_manager.cancel_all_orders(exchange)
        return True
    should_buy, buy_price = state.protection.update_trailing_buy(price)
    if should_buy:
        logging.info(f"Trailing buy được kích hoạt tại {buy_price:.2f}")
        await send_telegram_alert(f"Trailing buy được kích hoạt tại {buy_price:.2f}")
...
app.layout = html.Div([
    ...
    dash_table.DataTable(id='orders-table', style_table={'overflowX': 'auto'}),
    dcc.Interval(id='interval-component', interval=10*1000, n_intervals=0),
    html.Button('Tạm dừng/tiếp tục', id='pause-button', n_clicks=0),
])
...
def toggle_pause(n_clicks: int) -> str:
    global is_paused
    is_paused = not is_paused
    return 'Tiếp tục' if is_paused else 'Tạm dừng'
...
def update_dashboard(tab: str, n_intervals: int) -> Tuple[html.Div, html.Div, html.Div, go.Figure, List[Dict]]:
    global states, is_paused, exchange_health
    if not states:
    ...
    if tab == 'tab-overview':
        ...
        for name, state in states.items():
            stats = state.tracker.get_stats()
            health = exchange_health.get(name, {'status': 'unknown', 'api_latency_ms': None})
...
async def update_exchange_health():
    global exchange_health, states
    while True:
        try:
            for name, state in states.items():
                health = await state.exchange.health_check()
                exchange_health[name] = health
            await asyncio.sleep(60)  # Cập nhật mỗi 60 giây
        except Exception as e:
            logging.error(f"Lỗi khi cập nhật trạng thái sức khỏe: {str(e)}")
            await asyncio.sleep(60)
...
async def run_bot():
    ...
    # Định kỳ kiểm tra chênh lệch giá giữa các sàn
    ...
    # Thêm task cập nhật sức khỏe
    tasks.append(asyncio.create_task(update_exchange_health()))
...
async def shutdown(loop, exchange_manager: ExchangeManager):
    logging.info("Đang tắt bot...")
    await send_telegram_alert("Bot đang tắt...")
    save_state(states)
...
def handle_shutdown(loop, exchange_manager: ExchangeManager):
    asyncio.run_coroutine_threadsafe(shutdown(loop, exchange_manager), loop)
...
if __name__ == "__main__":
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        loop.call_soon_threadsafe(signal.signal, sig, lambda s, f: handle_shutdown(loop, exchange_manager))

    # Khởi động Dash server
    try:
        run_dash()
    except KeyboardInterrupt:
        logging.info("Nhận được tín hiệu dừng từ người dùng")
    finally:
        handle_shutdown(loop, exchange_manager)