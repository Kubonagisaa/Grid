...
class AdvancedConfig:
    def __init__(self):
        ...
        # Chuyển đổi định dạng cặp giao dịch
        raw_symbol = os.getenv("TRADING_SYMBOL", "BTCUSDT")
        self.symbol: str = self._normalize_symbol(raw_symbol)
        ...
        self.status_update_interval: int = int(os.getenv("STATUS_UPDATE_INTERVAL", 60))  # Giảm xuống 60 giây để debug
    ...
    def _normalize_symbol(self, symbol: str) -> str:
        ...
        for quote in ['USDT', 'USD', 'BTC', 'ETH', 'BNB']:
            if symbol.endswith(quote):
                ...
                return f"{base}/{quote}"
        logging.warning(f"Không thể chuyển đổi định dạng cặp giao dịch: {symbol}. Sử dụng giá trị gốc.")
        return symbol
...
class BaseExchange(ExchangeInterface):
    async def get_price(self) -> float:
        try:
            ...
            if not ticker:
                raise ValueError("Không lấy được dữ liệu ticker từ sàn")
            
            # Thử lấy giá từ 'last', nếu không có thì lấy từ 'close' hoặc 'bid'
            ...
            if price is None:
                raise ValueError("Không lấy được giá từ sàn: Các trường last/close/bid đều không có")
    ...
    async def get_volume(self) -> float:
        try:
            ...
            volume = ticker.get('quoteVolume', 0.0)  # Mặc định là 0.0 nếu không có quoteVolume
            if volume is None:
                logging.warning(f"Không lấy được khối lượng từ sàn {self.exchange_name}. Sử dụng giá trị mặc định: 0.0")
                return 0.0
    ...
    async def get_high_low(self) -> Tuple[float, float]:
        try:
            ...
            if not ticker or 'high' not in ticker or 'low' not in ticker:
                raise ValueError("Không lấy được giá cao/thấp từ sàn: Dữ liệu ticker không đầy đủ")
            
            high = ticker.get('high')
            low = ticker.get('low')
            last = ticker.get('last')

            # Nếu high hoặc low là None, sử dụng giá hiện tại (last) làm giá trị mặc định
            if high is None or low is None:
                if last is None:
                    raise ValueError("Không lấy được giá cao/thấp và giá hiện tại từ sàn")
                high = last if high is None else high
                low = last if low is None else low
                logging.warning(f"Không lấy được giá cao/thấp từ sàn {self.exchange_name}. Sử dụng giá hiện tại: high={high}, low={low}")
...
class BinanceExchange(BaseExchange):
    ...
    async def get_balance(self) -> Tuple[float, float]:
        try:
            ...
            base_asset = self.symbol.split('/')[0]  # Sửa để lấy base asset từ symbol có định dạng BTC/USDT
...
async def update_status(state: BotState, exchange: ExchangeInterface, last_status_update: float) -> float:
    ...
    if time.time() - last_status_update < config.status_update_interval:
        logging.info(f"Chưa đến thời gian cập nhật trạng thái. Thời gian còn lại: {(config.status_update_interval - (time.time() - last_status_update)):.2f} giây")
    ...
    try:
        ...
        open_orders = await safe_api_call(exchange.get_open_orders)
        open_orders_info = "\n".join(
            [f"  • {order['side']} tại {order['price']:.2f}, số lượng: {order['origQty']:.6f}" for order in open_orders]
        ...
        logging.info(f"Chuẩn bị gửi thông báo trạng thái trên {state.exchange_name}: {message}")
        send_telegram_alert(message)
        logging.info(f"Đã gửi thông báo trạng thái trên {state.exchange_name}")
        return time.time()  # Cập nhật last_status_update
    except Exception as e:
        logging.error(f"Lỗi khi cập nhật trạng thái trên {state.exchange_name}: {str(e)}", exc_info=True)
        send_telegram_alert(f"⚠️ *Lỗi khi cập nhật trạng thái trên {state.exchange_name}*: {str(e)}")
        return last_status_update  # Giữ nguyên last_status_update nếu có lỗi
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        # Thử lấy giá ban đầu với nhiều lần thử
        max_attempts = 3
        for attempt in range(max_attempts):
            try:
                ...
                break  # Thoát vòng lặp nếu lấy giá thành công
            except Exception as e:
                logging.error(f"Lỗi khi lấy giá ban đầu (lần thử {attempt + 1}/{max_attempts}): {str(e)}")
                if attempt == max_attempts - 1:
                    logging.error(f"Không thể lấy giá ban đầu sau {max_attempts} lần thử, dừng bot")
                    send_telegram_alert(f"❌ *Không thể lấy giá ban đầu trên {exchange.exchange_name} sau {max_attempts} lần thử*: {str(e)}")
                    return
        ...
        while True:
            # Tách biệt việc xử lý sự kiện và cập nhật trạng thái
            try:
                ...
                try:
                    ...
                    logging.error(f"Lỗi trong process_events trên {state.exchange_name}: {str(e)}", exc_info=True)
                    send_telegram_alert(f"⚠️ *Lỗi trong process_events trên {state.exchange_name}*: {str(e)}")
                    # Tiếp tục vòng lặp ngay cả khi process_events gặp lỗi

            finally:
                # Luôn gọi update_status, bất kể process_events có lỗi hay không
