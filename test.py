...
class EnhancedProtectionSystem:
    ...
    def update(self, price: float, high: float, low: float, volume: float) -> None:
        if price is None or high is None or low is None or volume is None:
            logging.warning("Dữ liệu không hợp lệ: một hoặc nhiều giá trị là None")
            return
    ...
    def check_pump_protection(self, price: float) -> bool:
        if len(self.price_history) < 2 or price is None:
            return False
    ...
    def check_abnormal_activity(self, volume: float) -> bool:
        if len(self.volume_history) < 2 or volume is None:
            return False
    ...
    def update_trailing_stop(self, price: float) -> Tuple[bool, float]:
        if not config.trailing_stop_enabled or price is None:
    ...
    def update_trailing_buy(self, price: float) -> Tuple[bool, float]:
        if not config.trailing_buy_stop_enabled or price is None:
            ...
            for order in orders:
                ...
                if order['status'] == 'FILLED':
                    ...
                    if order['orderId'] in grid_manager.static_order_ids and side == 'BUY':
                        ...
                        # Update ML labels based on profit outcome
                        profit = (current_price - price) * qty
                        profit_tracker.record_trade(profit, 0)  # Fee handled below
                        continue
        ...
        async with order_lock:
            ...
            for i, (buy_price, sell_price) in enumerate(static_grid, 1):
                ...
                if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                    ...
                    if buy_price < filters['minPrice'] or buy_price > filters['maxPrice']:
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'IExchange') -> None:
        async with order_lock:
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                if buy_price is None or sell_price is None or current_price is None:
                    logging.warning(f"Mức lưới {i} không hợp lệ: buy_price={buy_price}, sell_price={sell_price}, current_price={current_price}")
                    continue
                if buy_price <= 0 or sell_price <= 0:
                ...
                if sell_price > current_price:
                    ...
                    if sell_price < filters['minPrice'] or sell_price > filters['maxPrice']:
                        continue
            ...
            if not has_buy_order and len([o for o in open_orders if o is not None and o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > self.FALLBACK_COOLDOWN:
                ...
                buy_price = round(buy_price / tick_size) * tick_size
                if buy_price > 0 and not self.duplicate_order_exists("BUY", buy_price, open_orders) and buy_price >= filters['minPrice'] and buy_price <= filters['maxPrice']:
                    buy_order = await safe_api_call(exchange.place_order, side='BUY', order_type='LIMIT', quantity=quantity, price=buy_price, alert_on_failure=True)
                    if buy_order and 'orderId' in buy_order:
                        self.order_ids.append(buy_order['orderId'])
...
class BlackSwanProtection:
    ...
    async def monitor(self) -> None:
        ...
        if liquidity_ratio < 0.5:
            ...
            if (price_drop > config.price_drop_threshold / 100) or (spread > 0.05 and abnormal_vol):  # Adjusted threshold
                if current_time - self.last_black_swan_time > 60:
                    config.min_profit_percent = 1.5  # Adjust min profit dynamically
    ...
    async def process_events(self) -> None:
        try:
            ...
            if current_time - self.last_price_update >= 3:
                if not await check_internet_connection():
                    ...
                    if price is not None and price > 0:
                        ...
                        self.current_high, self.current_low = high_low if high_low[0] is not None else (self.current_high, self.current_low)
            ...
            if len(self.ml_features) > 50:
                # Use profit-based labels for reinforcement learning
            ...
            await send_telegram_alert(f"Lỗi nghiêm trọng trong vòng lặp chính: {str(e)}")
...
if __name__ == "__main__":
    async def main():
        ...
        symbols = ["DOGEUSDT", "BTCUSDT"]  # Multi-bot support
        states = {}
