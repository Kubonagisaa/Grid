...
# ======== TRAILING STOP VÀ CƠ CHẾ TỰ ĐỘNG ========
@dataclass
class TradeSignal:
    side: str
    price: float
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def get_price(self) -> float:
        ...
        try:
            ...
            price = float(ticker.get("last", ticker.get("close", self.current_market_price)))
            if price <= 0:
                logging.warning("Giá từ API không hợp lệ, sử dụng giá hiện tại")
                return self.current_market_price
            self.current_market_price = price
            self.current_volume = float(ticker.get("quoteVolume", ticker.get("baseVolume", self.current_volume)))
            self.current_high = float(ticker.get("high", ticker.get("highPrice", self.current_high)))
            self.current_low = float(ticker.get("low", ticker.get("lowPrice", self.current_low)))
