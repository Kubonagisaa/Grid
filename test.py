...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        atr = calculate_atr(list(self.high_history), list(self.low_history), list(self.price_history), config.atr_period)
        atr = atr if atr is not None and atr > 0 else current_price * 0.001  # Sửa lỗi: kiểm tra None
...
class AdaptiveGridStrategy(IGridStrategy):
    ...
    async def generate_signals(self, data: MarketData) -> List[TradeSignal]:
        price = data.price if data.price is not None and data.price > 0 else 0.0  # Sửa lỗi: kiểm tra None
        if price == 0.0:
            return []
...
class BlackSwanProtection:
    ...
    async def monitor(self) -> None:
        ...
        if liquidity_ratio < 0.5:
            ...
            if (price_drop > config.price_drop_threshold / 100) or (spread > 0.05 and abnormal_vol):
...
class BotState:
        ...
        try:
            ...
            if current_time - self.last_price_update >= 3:
                if not await check_internet_connection():
                    ...
                    price = price if price is not None and price > 0 else self.current_price
    ...
    async def handle_trailing_stops(self):
        ...
        for (side, entry_price), data in self.grid_manager.trailing_stops.items():
            ...
            entry_price = entry_price if entry_price is not None and entry_price > 0 else 0.0  # Sửa lỗi: kiểm tra None
            highest_price = highest_price if highest_price is not None and highest_price > 0 else entry_price  # Sửa lỗi: kiểm tra None
