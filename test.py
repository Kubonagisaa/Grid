...
# ======== GỬI CẢNH BÁO TELEGRAM (ĐÃ SỬA) ========
async def send_telegram_alert(message: str, parse_mode: str = "Markdown", max_retries: int = 3) -> None:
    if not config.telegram_enabled:
        logging.warning("Telegram không được bật. Bỏ qua gửi thông báo.")
        return

    if not config.telegram_bot_token or not config.telegram_chat_id:
        logging.error("Thiếu cấu hình Telegram (TELEGRAM_BOT_TOKEN hoặc TELEGRAM_CHAT_ID). Bỏ qua gửi thông báo.")
        return
    ...
    for attempt in range(max_retries):
        try:
            ...
            return  # Gửi thành công, thoát khỏi hàm
        except requests.exceptions.RequestException as e:
            logging.error(f"Lỗi khi gửi thông báo Telegram (lần thử {attempt + 1}/{max_retries}): {str(e)}")
            if attempt == max_retries - 1:
                logging.error(f"Không thể gửi thông báo Telegram sau {max_retries} lần thử: {message}")
                return
            await asyncio.sleep(2 ** attempt)  # Chờ theo cấp số nhân trước khi thử lại
...
class EnhancedSmartGridManager:
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        if quantity < filters['minQty']:
            ...
            await send_telegram_alert(f"⚠️ Số lượng {quantity} nhỏ hơn mức tối thiểu {filters['minQty']}")
            return
        ...
        for buy_price, sell_price in self.grid_levels:
            try:
                ...
                await send_telegram_alert(f"⚠️ Lỗi khi đặt lệnh: {str(e)}")
...
class EnhancedOrderManager:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface', profit_tracker: 'EnhancedProfitTracker') -> None:
        try:
            ...
            await send_telegram_alert(f"⚠️ Lỗi khi kiểm tra lệnh: {str(e)}")
...
class ExchangeManager:
    ...
    async def check_price_disparity(self) -> None:
        ...
        if max_diff / min(prices.values()) > 0.01:
            ...
            await send_telegram_alert(f"⚠️ Chênh lệch giá lớn: {max_diff:.2f} giữa {prices}")
...
async def safe_api_call(func, *args, max_retries: int = 3, **kwargs) -> Any:
    for attempt in range(max_retries):
        try:
            ...
            await send_telegram_alert(f"⚠️ Sàn giao dịch không khả dụng: {str(e)}")
...
async def check_balance_before_trading(exchange: ExchangeInterface) -> bool:
    try:
        ...
        await send_telegram_alert(f"⚠️ Lỗi kiểm tra số dư: {str(e)}")
        return False
...
async def check_api_connectivity(exchange: ExchangeInterface) -> bool:
    try:
        ...
        await send_telegram_alert(f"🔴 Mất kết nối với {exchange.exchange_name}")
        return False
...
async def handle_exchange_downtime(state: BotState, downtime_start: Optional[float]) -> Tuple[bool, Optional[float]]:
    if downtime_start is None:
        ...
        await send_telegram_alert(f"⚠️ Sàn {state.exchange_name} không khả dụng, chuyển sang chế độ an toàn")
    elapsed = time.time() - downtime_start
    if elapsed > 300:
        logging.error(f"Sàn {state.exchange_name} không khả dụng quá lâu, dừng bot")
        await send_telegram_alert(f"❌ Sàn {state.exchange_name} không khả dụng quá lâu, dừng bot")
...
async def check_protections(state: BotState, exchange: ExchangeInterface, current_price: float, current_volume: float, high: float, low: float) -> bool:
    if state.protection.check_pump_protection(current_price):
        ...
        await send_telegram_alert(f"⚠️ Bảo vệ tăng giá đột biến được kích hoạt trên {state.exchange_name}")
        state.order_manager.is_paused = True
        return False

    if state.protection.check_circuit_breaker(current_price):
        await send_telegram_alert(f"⚠ Ngắt mạch được kích hoạt: Giá thay đổi {abs(current_price - state.protection.price_history[-2]) / state.protection.price_history[-2]:.2%}")
        await state.order_manager.cancel_all_orders(exchange)
        return True

    if state.protection.check_circuit_breaker_status():
        await state.order_manager.cancel_all_orders(exchange)
        return True
    ...
    if state.protection.check_abnormal_activity(current_volume):
        ...
        await send_telegram_alert(f"⚠️ Phát hiện hoạt động bất thường trên {state.exchange_name}: Khối lượng {current_volume}")
        state.order_manager.is_paused = True
        return False
    ...
    if should_stop:
        ...
        await send_telegram_alert(f"✅ Trailing stop được kích hoạt trên {state.exchange_name} tại {trailing_stop_price:.2f}")
        await state.order_manager.cancel_all_orders(exchange)
        return True
    ...
    if should_buy:
        ...
        await send_telegram_alert(f"✅ Trailing buy được kích hoạt trên {state.exchange_name} tại {trailing_buy_price:.2f}")
...
async def update_status(state: BotState, exchange: ExchangeInterface, last_status_update: float) -> float:
    ...
    if time.time() - last_status_update > config.status_update_interval:
        try:
            ...
            await send_telegram_alert(f"⚠️ Lỗi khi cập nhật trạng thái trên {state.exchange_name}: {str(e)}")
    return last_status_update

# ======== CHẠY BOT CHO TỪNG SÀN GIAO DỊCH (ĐÃ SỬA) ========
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        try:
            ...
            await send_telegram_alert(f"❌ Lỗi khi lấy giá ban đầu trên {exchange.exchange_name}: {str(e)}")
            return

        health = await exchange.health_check()
        logging.info(f"Kiểm tra sức khỏe ban đầu: {health}")
        if health['status'] != 'healthy':
            await send_telegram_alert(f"⚠️ Kiểm tra sức khỏe thất bại trên {exchange.exchange_name}: {health['status']}")
            return
        ...
        # Gửi thông báo trạng thái ngay sau khi khởi động thành công
        last_status_update = 0  # Đặt về 0 để đảm bảo gửi thông báo trạng thái ngay lần đầu tiên
        last_status_update = await update_status(state, exchange, last_status_update)
        logging.info(f"Bot đã khởi động thành công trên {exchange.exchange_name}. Đã gửi thông báo trạng thái ban đầu.")
        ...
        await send_telegram_alert(f"🤖 Bot trên {exchange.exchange_name} đã bị dừng bởi người dùng")
        await state.order_manager.cancel_all_orders(exchange)
    except Exception as e:
        logging.critical(f"🚨 Lỗi nghiêm trọng trên {exchange.exchange_name}: {str(e)}", exc_info=True)
        await send_telegram_alert(f"🚨 Bot dừng hoạt động do lỗi trên {exchange.exchange_name}: {str(e)}")
        await state.order_manager.cancel_all_orders(exchange)
    finally:
...
async def main():
    ...
    # Chạy Dash app trong một thread riêng
