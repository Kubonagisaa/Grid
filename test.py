...
class AdvancedConfig:
    def __init__(self):
        ...
        self.initial_investment: float = float(os.getenv("INITIAL_INVESTMENT", 500))  # Tăng từ 200 lên 500
        self.min_quantity: float = float(os.getenv("MIN_QUANTITY", 0.001))  # Tăng từ 0.0005 lên 0.001
        ...
        self.base_grid_step_percent: float = float(os.getenv("BASE_GRID_STEP_PERCENT", 2.0))  # Tăng từ 1.0 lên 2.0
        self.grid_rebalance_interval: int = int(os.getenv("GRID_REBALANCE_INTERVAL", 30))  # Giảm từ 60 xuống 30 giây
...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        volatility = min(volatility, 0.05)  # Giới hạn volatility tối đa 5%
        num_levels = max(config.min_grid_levels, min(config.max_grid_levels, int(volatility / (config.base_grid_step_percent / 100))))
        step_percent = config.base_grid_step_percent / 100  # Sử dụng base_grid_step_percent trực tiếp
...
class EnhancedOrderManager:
    def __init__(self):
        ...
        self.last_trade_time: Dict[float, float] = {}  # Lưu thời gian giao dịch cuối cùng tại mỗi mức giá
        self.cooldown_period: int = 60  # 60 giây cooldown
        ...
        try:
            ...
            for order in orders:
                if order['status'] == 'FILLED':
                    ...
                    if fee > qty * price * 0.01:  # Nếu phí vượt quá 1% giá trị giao dịch, có thể là lỗi
                        ...
                        fee = qty * price * config.maker_fee  # Sử dụng phí mặc định
                    ...
                    # Kiểm tra cooldown tại mức giá này
                    current_time = time.time()
                    if price in self.last_trade_time:
                        time_since_last_trade = current_time - self.last_trade_time[price]
                        if time_since_last_trade < self.cooldown_period:
                            logging.info(f"Đang trong thời gian cooldown tại giá {price:.2f}. Bỏ qua đặt lệnh đối ứng.")
                            send_telegram_alert(f"⏳ Đang trong thời gian cooldown tại giá {price:.2f}. Bỏ qua đặt lệnh đối ứng.")
                            continue
                    
                    # Cập nhật thời gian giao dịch cuối cùng tại mức giá này
                    self.last_trade_time[price] = current_time
                    
                    # Đặt lại lệnh đối ứng với chênh lệch giá tối thiểu
                    if side == 'SELL':
                        # Đặt lệnh BUY thấp hơn 0.5% so với giá SELL vừa khớp
                        buy_price = price * 0.995  # Giảm 0.5%
                        ...
                        # Đặt lệnh SELL cao hơn 0.5% so với giá BUY vừa khớp
                        sell_price = price * 1.005  # Tăng 0.5%
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        while True:
            ...
            # Kiểm tra hiệu suất thực tế của bot
            try:
                await backtest_strategy(exchange, state)
            except Exception as e:
                logging.error(f"Lỗi khi chạy backtest trên {exchange.exchange_name}: {str(e)}")
                send_telegram_alert(f"⚠️ Lỗi khi chạy backtest trên {exchange.exchange_name}: {str(e)}")

            # Kiểm tra tài nguyên hệ thống (CPU, RAM) để tránh quá tải
            ...
            if cpu_usage > 90 or memory_usage > 90:
                ...
                await asyncio.sleep(300)  # Tạm dừng 5 phút để giảm tải

            # Kiểm tra xem bot có nên dừng không (ví dụ: người dùng yêu cầu dừng hoặc lỗi nghiêm trọng)
            # (Bạn có thể thêm logic dừng bot tại đây nếu cần)
        ...
        # Đảm bảo đóng kết nối với sàn giao dịch khi bot dừng
