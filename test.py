...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def get_price(self) -> float:
        # Lấy giá live từ API công cộng của Binance
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        ...
        order = {
            ...
            "status": "OPEN",  # Đặt trạng thái ban đầu là OPEN để chờ khớp khi giá live chạm limit
            "timestamp": time.time(),
            "executedQty": 0.0
        }
...
class ExchangeManager:
    def __init__(self):
        ...
        # Nếu bật paper trading thì chỉ khởi tạo sàn mô phỏng paper_trading
