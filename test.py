...
# Cấu hình logging với QueueHandler để đồng bộ hóa
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.handlers.QueueHandler(logging.handlers.QueueListener(
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.order_cooldown: float = 5.0  # 5 giây cooldown giữa các lần đặt lệnh
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        # Kiểm tra giá thay đổi trước khi tính toán lại lưới
        if len(self.price_history) > 1:
            price_change = abs(current_price - self.price_history[-2]) / self.price_history[-2]
            if price_change < 0.001:  # Giá thay đổi < 0.1%
                logging.info(f"Giá hiện tại: {current_price:.2f}, không đủ biến động ({price_change:.2%}) để cập nhật lưới")
                return
        ...
        logging.info(f"Đã tính toán lưới thích ứng: {num_levels} mức, bước {step_percent:.2%}, giá hiện tại: {current_price:.2f}, grid_levels: {self.grid_levels}")
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        # Kiểm tra lệnh mở hiện tại
        open_orders = await safe_api_call(exchange.get_open_orders)
        existing_orders = {(o['side'], float(o['price'])) for o in open_orders}
        logging.info(f"Các lệnh mở hiện tại: {existing_orders}")

        # Kiểm tra cooldown
        now = time.time()
        if now - self.last_order_time < self.order_cooldown:
            await asyncio.sleep(self.order_cooldown - (now - self.last_order_time))
        self.last_order_time = time.time()

        # Hủy tất cả lệnh cũ trước khi đặt lệnh mới (chỉ gọi khi cần thiết)
        ...
        placed_buy_orders = set()  # Theo dõi các mức giá đã đặt lệnh mua
        placed_sell_orders = set()  # Theo dõi các mức giá đã đặt lệnh bán
        for buy_price, sell_price in self.grid_levels:
            try:
                ...
                # Đặt lệnh mua
                if buy_price < current_price and buy_price not in placed_buy_orders and ('BUY', buy_price) not in existing_orders:
                    ...
                    placed_buy_orders.add(buy_price)
                ...
                # Đặt lệnh bán
                if sell_price > current_price and sell_price not in placed_sell_orders and ('SELL', sell_price) not in existing_orders:
                    ...
                    placed_sell_orders.add(sell_price)
        ...
        # Đặt lệnh mua động nếu không có lệnh mua nào được đặt
        if not placed_buy_orders:
            try:
                buy_price = current_price * 0.99  # Đặt lệnh BUY thấp hơn 1% so với giá hiện tại
                if buy_price > 0 and ('BUY', buy_price) not in existing_orders:
                    buy_order = await safe_api_call(
                        exchange.place_order,
                        side='BUY',
                        order_type='LIMIT',
        ...
        # Đặt lệnh bán động nếu không có lệnh bán nào được đặt
        if not placed_sell_orders:
            try:
                sell_price = current_price * 1.01  # Đặt lệnh SELL cao hơn 1% so với giá hiện tại
                if sell_price > 0 and ('SELL', sell_price) not in existing_orders:
                    sell_order = await safe_api_call(
                        exchange.place_order,
                        side='SELL',
                        order_type='LIMIT',
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        try:
            open_orders = await safe_api_call(exchange.get_open_orders)
            logging.info(f"Danh sách lệnh mở trước khi hủy: {open_orders}")
            ...
            open_orders_after = await safe_api_call(exchange.get_open_orders)
            logging.info(f"Danh sách lệnh mở sau khi hủy: {open_orders_after}")
            if open_orders_after:
                logging.warning(f"Các lệnh vẫn còn sau khi hủy: {open_orders_after}")
        except Exception as e:
...
class EnhancedOrderManager:
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        try:
            open_orders = await safe_api_call(exchange.get_open_orders)
            logging.info(f"Danh sách lệnh mở trước khi hủy trong order manager: {open_orders}")
            for order in open_orders:
                try:
                    await safe_api_call(exchange.cancel_order, order['orderId'])
                    logging.info(f"Đã hủy lệnh {order['orderId']} tại giá {order['price']:.2f}")
            ...
            open_orders_after = await safe_api_call(exchange.get_open_orders)
            logging.info(f"Danh sách lệnh mở sau khi hủy trong order manager: {open_orders_after}")
            if open_orders_after:
                logging.warning(f"Các lệnh vẫn còn sau khi hủy trong order manager: {open_orders_after}")
        except Exception as e:
...
logging.error(f"Lỗi khi cập nhật trạng thái trên {state.exchange_name}: {str(e)}")
            send_telegram_alert(f"⚠️ Lỗi khi cập nhật trạng thái trên {state.exchange_name}: {str(e)}")
    return last_status_update

# ======== DASHBOARD ĐỂ THEO DÕI HIỆU SUẤT ========
def create_dashboard(states: Dict[str, BotState]) -> None:
    ...
    async def update_charts(n_intervals):
        price_fig = go.Figure()
        profit_fig = go.Figure()
        order_book_fig = go.Figure()

        for name, state in states.items():
            prices = list(state.grid_manager.price_history)
            times = [datetime.now() - timedelta(seconds=(len(prices) - i) * 60) for i in range(len(prices))]
            
            # Biểu đồ giá
            price_fig.add_trace(go.Scatter(x=times, y=prices, mode='lines', name=f'Price ({name})'))
            
            # Biểu đồ lợi nhuận
            equity = state.tracker.equity_curve
            profit_times = [datetime.now() - timedelta(seconds=(len(equity) - i) * 60) for i in range(len(equity))]
            profit_fig.add_trace(go.Scatter(x=profit_times, y=equity, mode='lines', name=f'Equity ({name})'))
            ...
            try:
                ...
                order_book_fig.add_trace(go.Bar(x=bid_volumes, y=bid_prices, orientation='h', name=f'Bids ({name})', marker_color='green'))
                order_book_fig.add_trace(go.Bar(x=ask_volumes, y=ask_prices, orientation='h', name=f'Asks ({name})', marker_color='red'))
            except Exception as e:
                logging.error(f"Lỗi khi lấy sổ lệnh cho dashboard trên {name}: {str(e)}")

        price_fig.update_layout(title='Price History', xaxis_title='Time', yaxis_title='Price')
        profit_fig.update_layout(title='Equity Curve', xaxis_title='Time', yaxis_title='Profit (USDT)')
        order_book_fig.update_layout(title='Order Book', xaxis_title='Volume', yaxis_title='Price', barmode='overlay')
        
        return price_fig, profit_fig, order_book_fig

    Thread(target=app.run_server, kwargs={'host': '0.0.0.0', 'port': 8050, 'debug': False}).start()
    logging.info("Dashboard đã được khởi động tại http://localhost:8050")
...
async def run_bot():
    ...
    tasks = []
    downtime_starts: Dict[str, Optional[float]] = {name: None for name in exchange_manager.exchanges}
    last_status_updates: Dict[str, float] = {name: time.time() for name in exchange_manager.exchanges}

    for name, exchange in exchange_manager.exchanges.items():
        if not await check_api_connectivity(exchange):
            logging.error(f"Không thể kết nối với {name}, bỏ qua sàn này")
            continue

        if not await check_balance_before_trading(exchange):
            logging.error(f"Số dư không đủ trên {name}, bỏ qua sàn này")
            continue
    ...
    while True:
        for name, state in states.items():
            ...
            try:
                if not await check_api_connectivity(exchange):
                    should_stop, downtime_starts[name] = await handle_exchange_downtime(state, downtime_starts[name])
                    if should_stop:
                        break
                    continue
                else:
                    downtime_starts[name] = None

                usdt_balance, btc_balance = await check_account_balance(exchange)
                if usdt_balance < config.initial_investment * 0.1:
                    logging.warning(f"Số dư USDT thấp trên {name}: {usdt_balance:.2f}")
                    send_telegram_alert(f"⚠️ Số dư USDT thấp trên {name}: {usdt_balance:.2f}")

                last_status_updates[name] = await update_status(state, exchange, last_status_updates[name])

                cpu_usage = psutil.cpu_percent(interval=1)
                memory_usage = psutil.virtual_memory().percent
                if cpu_usage > 90 or memory_usage > 90:
                    logging.warning(f"Hiệu suất hệ thống cao: CPU {cpu_usage}%, Memory {memory_usage}%")
                    send_telegram_alert(f"⚠️ Hiệu suất hệ thống cao: CPU {cpu_usage}%, Memory {memory_usage}%")

            except Exception as e:
                logging.error(f"Lỗi trong vòng lặp chính trên {name}: {str(e)}")
                send_telegram_alert(f"⚠️ Lỗi trong vòng lặp chính trên {name}: {str(e)}")
                await asyncio.sleep(60)
...
if __name__ == "__main__":
    try:
        ...
        send_telegram_alert("🛑 Bot đã dừng bởi người dùng")
    except Exception as e:
        logging.error(f"Lỗi nghiêm trọng: {str(e)}")
        send_telegram_alert(f"❌ Lỗi nghiêm trọng: {str(e)}")