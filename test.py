...
import redis
import random
import hmac  # Thêm thư viện cho WebSocket
...
class BotState:
    ...
    async def send_hourly_report(self):
        ...
        # === Thay đổi ở đây ===
        # Dùng đúng symbol và query trực tiếp open orders từ exchange
        symbol_clean = format_symbol(self.symbol)
        raw_open = await self.exchange.get_open_orders(symbol_clean)
        open_count = len([o for o in raw_open if isinstance(o, dict) and o.get("status") == "OPEN"])
        filled_count = len(self.order_manager.completed_orders)
        logging.info(f"[DEBUG] Open orders for {symbol_clean}: {raw_open}")  # optional debug
        # === Hết thay đổi ===
        ...
        embed = {
            ...
            "fields": [
                ...
                {"name": "! |📊| Lệnh mở", "value": str(open_count), "inline": True},
                {"name": "! |⭕️| Lệnh khớp", "value": str(filled_count), "inline": True},
...
class LiveFillMonitor:
    ...
    async def _handle_filled_order(self, order_id: str, executed_qty: float, avg_price: float):
        ...
        # Gửi thông báo
        msg = f"Lệnh {side} đã khớp @ {avg_price:.4f}, SL: {executed_qty:.6f}"
