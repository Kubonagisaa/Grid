...
class AdvancedConfig:
    def __init__(self):
        ...
        self.grid_rebalance_interval: int = int(os.getenv("GRID_REBALANCE_INTERVAL", 60))
...
class EnhancedProtectionSystem:
    def __init__(self):
        ...
        self.trailing_sell_price: float = 0.0
    ...
    def update_trailing_sell(self, current_price: float, initial_sell_price: float) -> Tuple[bool, float]:
        if not config.trailing_stop_enabled:
            return False, initial_sell_price
        if self.trailing_sell_price == 0.0:
            self.trailing_sell_price = initial_sell_price
        if current_price > self.trailing_sell_price:
            self.trailing_sell_price = current_price * (1 + config.base_take_profit_percent / 100)
            logging.info(f"Điều chỉnh giá trailing SELL lên: {self.trailing_sell_price:.2f}")
            return True, self.trailing_sell_price
        stop_price = self.trailing_sell_price * (1 - config.base_stop_loss_percent / 100)
        if current_price < stop_price:
            logging.info(f"Giá giảm xuống dưới ngưỡng trailing stop, giữ giá SELL tại: {self.trailing_sell_price:.2f}")
            return False, self.trailing_sell_price
        return False, self.trailing_sell_price
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.high_history: deque = deque(maxlen=config.volatility_window)
        self.low_history: deque = deque(maxlen=config.volatility_window)
        self.sell_orders: Dict[str, Tuple[float, float]] = {}
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface', bot_state: 'BotState' = None) -> None:
        ...
        for buy_price, sell_price in self.grid_levels:
            try:
                ...
                if sell_price > current_price:
                    if bot_state and bot_state.bought_quantity >= quantity:
                        ...
                        self.sell_orders[sell_order['id']] = (sell_price, quantity)
                        ...
                        logging.warning(f"Không đủ số lượng đã mua để đặt lệnh BÁN: Cần {quantity:.6f}, Có {bot_state.bought_quantity if bot_state else 0:.6f}")
                        send_telegram_alert(f"⚠️ Không đủ số lượng đã mua để đặt lệnh BÁN: Cần {quantity:.6f}, Có {bot_state.bought_quantity if bot_state else 0:.6f}")
                        continue
            except Exception as e:
    ...
    async def update_trailing_sell_orders(self, current_price: float, exchange: 'ExchangeInterface', protection: 'EnhancedProtectionSystem') -> None:
        if not self.order_ids:
            return
        open_orders = await safe_api_call(exchange.get_open_orders)
        sell_orders_to_update = [order for order in open_orders if order['side'] == 'SELL' and order['orderId'] in self.order_ids]
        for order in sell_orders_to_update:
            order_id = order['orderId']
            original_price = float(order['price'])
            quantity = float(order['origQty'])
            should_update, new_sell_price = protection.update_trailing_sell(current_price, original_price)
            if should_update and new_sell_price != original_price:
                try:
                    await safe_api_call(exchange.cancel_order, order_id)
                    self.order_ids.remove(order_id)
                    if order_id in self.sell_orders:
                        del self.sell_orders[order_id]
                    logging.info(f"Đã hủy lệnh BÁN cũ tại {original_price:.2f}")
                    ...
                    self.order_ids.append(new_order['id'])
                    self.sell_orders[new_order['id']] = (new_sell_price, quantity)
                    logging.info(f"Đã đặt lại lệnh BÁN tại {new_sell_price:.2f}, số lượng: {quantity:.6f}")
                    send_telegram_alert(f"📤 Đã cập nhật lệnh BÁN tại {new_sell_price:.2f}, số lượng: {quantity:.6f}")
                except Exception as e:
                    logging.error(f"Lỗi khi cập nhật lệnh BÁN: {str(e)}")
                    send_telegram_alert(f"⚠️ Lỗi khi cập nhật lệnh BÁN: {str(e)}")
...
class EnhancedOrderManager:
    def __init__(self):
        ...
        self.bought_quantity: float = 0.0
        ...
        try:
            ...
            for order in orders:
                if order['status'] == 'FILLED':
                    ...
                    if side == 'BUY':
                        self.bought_quantity += qty
                        logging.info(f"Lệnh BUY khớp, tăng số lượng đã mua: {self.bought_quantity:.6f}")
                    elif side == 'SELL':
                        if self.bought_quantity < qty:
                            logging.error(f"Không đủ số lượng để bán: Cần {qty:.6f}, Có {self.bought_quantity:.6f}")
                            send_telegram_alert(f"⚠️ Không đủ số lượng để bán: Cần {qty:.6f}, Có {self.bought_quantity:.6f}")
                            continue
                        self.bought_quantity -= qty
                        logging.info(f"Lệnh SELL khớp, giảm số lượng đã mua: {self.bought_quantity:.6f}")
    ...
    async def process_events(self) -> None:
        while True:
            ...
            for signal in signals:
                try:
                    if signal.side == "SELL" and self.bought_quantity < signal.quantity:
                        logging.warning(f"Không đủ số lượng đã mua để bán: Cần {signal.quantity:.6f}, Có {self.bought_quantity:.6f}")
                        send_telegram_alert(f"⚠️ Không đủ số lượng đã mua để bán: Cần {signal.quantity:.6f}, Có {self.bought_quantity:.6f}")
                        continue
                    order = await self.exchange.place_order(signal.side, "LIMIT", signal.quantity, signal.price)
                    if signal.side == "BUY":
                        self.bought_quantity += signal.quantity
            ...
            await self.grid_manager.update_trailing_sell_orders(self.current_price, self.exchange, self.protection)
...
async def check_protections(state: BotState, exchange: ExchangeInterface, current_price: float, current_volume: float, high: float, low: float) -> bool:
    ...
    if should_buy:
        ...
        await state.grid_manager.place_grid_orders(current_price, exchange, state)
    return False
    ...
    if time.time() - last_status_update > config.status_update_interval:
        try:
            ...
            message = (
                ...
                f"**Số Lượng Đã Mua**: {state.bought_quantity:.6f} BTC\n"
                f"**Chỉ Số Kỹ Thuật**:\n"
                f"- ATR: {atr:.2f}\n"
                f"- RSI: {rsi:.2f}\n"
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        if not await check_balance_before_trading(exchange):
            ...
            return
        state.bought_quantity = btc_balance
        state.order_manager.bought_quantity = btc_balance
        logging.info(f"Đồng bộ số lượng đã mua: {state.bought_quantity:.6f} BTC")
        try:
        ...
        await state.grid_manager.place_grid_orders(state.current_price, exchange, state)
        last_status_update = time.time()
        safe_mode = False
        downtime_start = None
        while True:
...
# Biến toàn cục để lưu trữ dữ liệu dashboard
dashboard_data = {
    "price_history": {},
    "profit_history": {},
    "open_orders": []
...
# Hàm bất đồng bộ để cập nhật dữ liệu dashboard
async def update_dashboard_data(exchange_manager: ExchangeManager, states: Dict[str, BotState]):
    ...
    while True:
        try:
            ...
            for exchange_name, exchange in exchange_manager.exchanges.items():
                if exchange_name not in states:
                    strategy = AdaptiveGridStrategy(EnhancedSmartGridManager())
                    states[exchange_name] = BotState(exchange, strategy)
                state = states[exchange_name]
                current_price = state.current_price if state.current_price else await safe_api_call(exchange.get_price)
                price_history = list(state.grid_manager.price_history)
                timestamps = [datetime.now() - timedelta(seconds=(len(price_history) - i) * 60) for i in range(len(price_history))]
                ...
                profit_timestamps = [datetime.now() - timedelta(seconds=(len(equity_curve) - i) * 60) for i in range(len(equity_curve))]
                profit_data[exchange_name] = {
                    "timestamps": profit_timestamps,
                    "profits": equity_curve
                }
                try:
                    open_orders = await safe_api_call(exchange.get_open_orders)
                    for order in open_orders:
                        all_open_orders.append(
                            f"{exchange_name.upper()}: {order['side']} tại {order['price']:.2f}, Số lượng: {order['origQty']:.6f}"
                        )
                except Exception as e:
                    logging.error(f"Lỗi khi lấy lệnh mở từ {exchange_name}: {str(e)}")
            dashboard_data["price_history"] = price_data
            dashboard_data["profit_history"] = profit_data
            dashboard_data["open_orders"] = all_open_orders
        except Exception as e:
            logging.error(f"Lỗi khi cập nhật dữ liệu dashboard: {str(e)}")
            send_telegram_alert(f"⚠️ Lỗi khi cập nhật dữ liệu dashboard: {str(e)}")
        await asyncio.sleep(60)
...
def update_dashboard(n):
    ...
    open_orders_text = "Không có dữ liệu lệnh mở"
    try:
        price_data = dashboard_data["price_history"]
        profit_data = dashboard_data["profit_history"]
        all_open_orders = dashboard_data["open_orders"]
        price_traces = []
        for exchange_name, data in price_data.items():
            price_traces.append(
                go.Scatter(
                    ...
                    y=data["prices"],
                    mode='lines',
                    name=f"Giá {exchange_name.upper()}"
                )
            )
        ...
        profit_traces = []
        for exchange_name, data in profit_data.items():
         profit_traces.append(
                        go.Scatter(
                            x=data["timestamps"],
                            y=data["profits"],
                            mode='lines',
                            name=f"Lợi nhuận {exchange_name.upper()}"
                        )
                    )
        profit_fig = go.Figure(data=profit_traces)
        profit_fig.update_layout(
            title="Biểu đồ Lợi nhuận",
            xaxis_title="Thời gian",
            yaxis_title="Lợi nhuận (USDT)",
            template="plotly_dark"
        )
        open_orders_text = html.Ul([html.Li(order) for order in all_open_orders]) if all_open_orders else "Không có lệnh mở"
    except Exception as e:
        logging.error(f"Lỗi khi cập nhật dashboard: {str(e)}")
        send_telegram_alert(f"⚠️ Lỗi khi cập nhật dashboard: {str(e)}")
        price_fig = go.Figure()
        profit_fig = go.Figure()
        price_fig.update_layout(title="Biểu đồ giá (Lỗi)", template="plotly_dark")
        profit_fig.update_layout(title="Biểu đồ Lợi nhuận (Lỗi)", template="plotly_dark")
    return price_fig, profit_fig, open_orders_text

# ======== KHỞI ĐỘNG BOT VÀ DASH APP ========
async def main():
    exchange_manager = ExchangeManager()
    states: Dict[str, BotState] = {}
    tasks = []
    try:
        # Khởi động bot cho từng sàn giao dịch
        for exchange in exchange_manager.exchanges.values():
            task = asyncio.create_task(run_bot_for_exchange(exchange, states))
            tasks.append(task)
        # Khởi động tác vụ cập nhật dashboard
        dashboard_task = asyncio.create_task(update_dashboard_data(exchange_manager, states))
        tasks.append(dashboard_task)
        # Chạy server Dash trong một thread riêng
        def run_dash():
            app.run_server(debug=False, host='0.0.0.0', port=8050)
        dash_thread = Thread(target=run_dash)
        dash_thread.start()
        logging.info("Đã khởi động Dash server tại http://0.0.0.0:8050")
        send_telegram_alert("🌐 Đã khởi động Dash server tại http://0.0.0.0:8050")
        # Chờ tất cả các tác vụ hoàn thành
        await asyncio.gather(*tasks)
    except KeyboardInterrupt:
        logging.info("Dừng bot bởi người dùng")
        send_telegram_alert("🛑 Bot đã bị dừng bởi người dùng")
    except Exception as e:
        logging.critical(f"Lỗi nghiêm trọng trong main: {str(e)}", exc_info=True)
        send_telegram_alert(f"🚨 Lỗi nghiêm trọng trong main: {str(e)}")
    finally:
        # Đóng tất cả các kết nối
        await exchange_manager.close_all()
        for state in states.values():
            await state.order_manager.cancel_all_orders(state.exchange)
        logging.info("Đã đóng tất cả kết nối và hủy lệnh mở")
...
def check_system_resources() -> bool:
    ...
    if cpu_usage > 90:
        ...
        send_telegram_alert(f"⚠️ CPU usage cao: {cpu_usage}%")
        return False
    if memory_usage > 90:
        logging.warning(f"Memory usage cao: {memory_usage}%")
        send_telegram_alert(f"⚠️ Memory usage cao: {memory_usage}%")
        return False
    logging.info(f"System resources: CPU {cpu_usage}%, Memory {memory_usage}%")
    return True

# ======== ĐIỂM VÀO CHÍNH ========
if __name__ == "__main__":
    if not check_system_resources():
        logging.error("Tài nguyên hệ thống không đủ, thoát chương trình")
        send_telegram_alert("❌ Tài nguyên hệ thống không đủ, bot không thể khởi động")
        exit(1)
    logging.info("Khởi động bot giao dịch lưới...")
    send_telegram_alert("🤖 Khởi động bot giao dịch lưới...")
    asyncio.run(main())