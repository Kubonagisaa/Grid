...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if current_time - self.last_price_update >= 5:
            try:
                self.current_price = safe_api_call(self.exchange.get_price)
                self.current_volume = safe_api_call(self.exchange.get_volume)
                self.current_high, self.current_low = safe_api_call(self.exchange.get_high_low)
