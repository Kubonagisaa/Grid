...
prices = list(self.grid_manager.price_history)
            ...
            if rsi < rsi_low:
                logging.info(f"RSI {rsi:.2f} < {rsi_low}, ưu tiên tín hiệu MUA")
            elif rsi > rsi_high:
                logging.info(f"RSI {rsi:.2f} > {rsi_high}, ưu tiên tín hiệu BÁN")
            else:
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.trailing_stops: Dict[Tuple[str, float], Any] = {}  # trailing stop for each order
            ...
            for order in orders:
                ...
                if order['status'] == 'FILLED':
                    ...
                    msg = (f"Lệnh {side} đã khớp\n"
                           ...
                           f"Phí: {fee:.4f} USDT")
                    logging.info(f"[PAPER] Lệnh {side.upper()} đã khớp: Giá {price:.2f},  Số lượng {qty:.6f}, Lợi nhuận {profit:.2f} USDT, Phí {fee:.4f} USDT")
                    self.completed_orders.append({"side": side.upper(), "price": price, "quantity": qty})
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if current_time - self.last_price_update >= 5:
            try:
                price_task = safe_api_call(self.exchange.get_price)
                volume_task = safe_api_call(self.exchange.get_volume)
                highlow_task = safe_api_call(self.exchange.get_high_low)
                self.current_price, self.current_volume, high_low = await asyncio.gather(price_task, volume_task, highlow_task)
...
class PaperTradingExchange(ExchangeInterface):
    ...
    async def run_paper_trading_live(self, state: BotState, interval: int = 1) -> None:
        ...
        try:
            while True:
                try:
                    await asyncio.wait_for(state.process_events(), timeout=20)
                except asyncio.TimeoutError:
                    logging.error("Vòng lặp xử lý quá thời gian, bỏ qua vòng này.")
                except Exception as e:
