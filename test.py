...
class AdvancedConfig:
    def __init__(self):
        ...
        # THAM SỐ MỚI: thời gian chờ (cooldown) không đặt lại lệnh cho cùng mức khi đã khớp
        self.order_cooldown_sec: int = int(os.getenv("ORDER_COOLDOWN_SECONDS", 180))

config = AdvancedConfig()

# ======== GỮI CẢNH BÁO TELEGRAM ========
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.high_history: deque = deque(maxlen=config.volatility_window)
        self.low_history: deque = deque(maxlen=config.volatility_window)
        # Theo dõi cooldown cho các mức lệnh
        self.grid_cooldown: Dict[Tuple[str, float], float] = {}
        # ---------- THÊM trailing riêng cho từng grid (đối với lệnh SELL) ----------
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        for i in range(num_levels):
            ...
            if buy_price <= 0 or sell_price <= 0:
                ...
                continue
            if buy_price < current_price * 0.8 or sell_price > current_price * 1.2:
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        # Hủy tất cả lệnh mở trước khi đặt lệnh mới
        await self.cancel_all_orders(exchange)
        await asyncio.sleep(1)  # Đợi sàn xử lý lệnh cancel
        ...
        for buy_price, sell_price in self.grid_levels:
            ...
            try:
                ...
                if buy_price < current_price and current_time >= cooldown_buy:
                    ...
                    logging.info(f"Bỏ qua lệnh BUY tại {buy_price:.2f} do cooldown.")
                ...
                if sell_price > current_price and current_time >= cooldown_sell:
                    ...
                    logging.info(f"Bỏ qua lệnh SELL tại {sell_price:.2f} do cooldown.")
    ...
    async def rebalance_if_needed(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        if time.time() - self.last_rebalance > config.grid_rebalance_interval:
            need_rebalance = True
        if low_grid is not None and current_price < low_grid:
            need_rebalance = True
        if high_grid is not None and current_price > high_grid:
            need_rebalance = True
        if need_rebalance:
            await self.calculate_adaptive_grid(current_price, exchange)
            await self.place_grid_orders(current_price, exchange)
            self.last_rebalance = time.time()
            logging.info(f"Đã cân bằng lại lưới với giá hiện tại {current_price:.2f}")
        ...
        try:
            ...
            for order in orders:
                if order['status'] == 'FILLED':
                    ...
                    for grid_level in grid_manager.grid_levels:
                        buy_price, sell_price = grid_level
                        if side.upper() == "BUY" and abs(price - buy_price) < 10 ** (-grid_manager._get_price_precision()):
                            grid_manager.set_cooldown("BUY", price)
                        if side.upper() == "SELL" and abs(price - sell_price) < 10 ** (-grid_manager._get_price_precision()):
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        # Xóa tất cả lệnh mở khi khởi động bot
        await state.order_manager.cancel_all_orders(exchange)
