    ...
    async def place_static_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            try:
                ...
                max_qty = float(filters.get('maxQty') or float('inf'))
                min_price = float(filters.get('minPrice') or 0.0)
                max_price = float(filters.get('maxPrice') or float('inf'))
                tick_size = float(filters.get('tickSize') or 0.00001)
                if tick_size <= 0:
                    tick_size = 0.00001
            except Exception as e:
...
class PaperTradingExchange(IExchange):
    ...
    async def get_symbol_filters(self) -> Dict:
        ...
        try:
            ...
            return {
                "minQty": float(amount_limits.get('min', 1.0)),
                "maxQty": float(amount_limits.get('max', 10000.0)),
                "stepSize": market.get('precision', {}).get('amount', 0.00001),
                "minPrice": float(price_limits.get('min', 0.00001)),
                "maxPrice": float(price_limits.get('max', 10000.0)),
