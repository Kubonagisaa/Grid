...
import pandas_ta as ta  # Thay talib bằng pandas-ta
...
class AdvancedConfig:
    def __init__(self):
        ...
        self.symbol: str = os.getenv("TRADING_SYMBOL", "DOGEUSDT").upper()  # Cập nhật thành DOGEUSDT
        self.initial_investment: float = float(os.getenv("INITIAL_INVESTMENT", 20))  # 20 USDT
        self.min_quantity: float = float(os.getenv("MIN_QUANTITY", 5))  # 5 DOGE
        ...
        self.min_grid_levels: int = int(os.getenv("MIN_GRID_LEVELS", 5))  # Tăng lên 5
        self.max_grid_levels: int = int(os.getenv("MAX_GRID_LEVELS", 7))  # Tăng lên 7
        self.base_grid_step_percent: float = float(os.getenv("BASE_GRID_STEP_PERCENT", 1.0))  # Giảm xuống 1.0%
        ...
        self.trailing_up_activation: float = float(os.getenv("TRAILING_UP_ACTIVATION", 0.7))  # Điều chỉnh cho DOGE
        self.trailing_down_activation: float = float(os.getenv("TRAILING_DOWN_ACTIVATION", 0.5))  # Điều chỉnh cho DOGE
        ...
        self.circuit_breaker_duration: int = int(os.getenv("CIRCUIT_BREAKER_DURATION", 180))  # Giảm xuống 180 giây
        ...
        self.max_open_orders: int = int(os.getenv("MAX_OPEN_ORDERS", 5))  # Giới hạn lệnh mở
        ...
        self.strategy_mode: int = int(os.getenv("STRATEGY_MODE", 2))  # Sử dụng grid dày
...
class EnhancedOrderManager:
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        try:
            ...
            logging.error(f"Lỗi khi hủy tấtan tất cả lệnh: {str(e)}")
...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        async with order_lock:
            ...
            quantity = max(5, dynamic_position_sizing(calculate_atr(list(self.high_history), list(self.low_history), list(self.price_history), config.atr_period)))
            quantity = min(quantity, filters['maxQty'])
            step_size = filters.get('stepSize', 0.00001)
            if step_size <= 0:
                step_size = 0.00001
            quantity = round(quantity - (quantity % step_size), 6)
            if quantity < 5 or quantity > filters['maxQty']:
...
class TrendDetector:
    ...
    async def detect_trend_strength(self) -> float:
        ...
        if len(highs) < 14 or len(lows) < 14 or len(closes) < 14:
            return 0.0
        # Đảm bảo các danh sách cùng độ dài trước khi tạo DataFrame
...
class BlackSwanProtection:
    ...
    async def monitor(self) -> None:
        ...
        if liquidity_ratio < 0.5:
            order_book = self.last_order_book
            if not order_book:
                try:
                    order_book = await self.state.exchange.get_order_book()
                except Exception as e:
                    logging.error(f"Không thể lấy order book để xác nhận Black Swan: {str(e)}")
                    return
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        if current_time - self.last_price_update >= 3:  # Giảm từ 5 giây xuống 3 giây
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        ...
        self.paper_orders = []
        self.current_market_price = 0.1  # Giá giả định cho DOGE/USDT
        self.current_volume = 0.0
    ...
    async def get_symbol_filters(self) -> Dict:
        return {'minQty': 5, 'maxQty': 100000, 'stepSize': 1}
    ...
    async def start_websocket(self):
        ...
        while True:
            try:
                async with websockets.connect("wss://wbs-api.mexc.com/ws") as websocket:
                    subscribe_msg = {
                        "method": "SUBSCRIPTION",
                        "params": [f"spot@public.ticker.v3.api@{self.symbol}"]
                    }
                    ...
                    while True:
                        ...
                        if "data" in data:
                            ticker = data["data"]
                            self.current_market_price = float(ticker["p"])
                            self.current_volume = float(ticker["v"])
                            self.current_high = float(ticker["h"])
                            self.current_low = float(ticker["l"])
            except Exception as e:
