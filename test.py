...
class EnhancedProtectionSystem:
    def __init__(self):
        ...
        self.trailing_sell_price: float = 0.0  # Giá trailing cho lệnh SELL
    ...
    def update_trailing_sell(self, current_price: float, initial_sell_price: float) -> Tuple[bool, float]:
        """
        Điều chỉnh giá của lệnh SELL theo cơ chế trailing.
        - Nếu giá tăng, tăng giá SELL để tối ưu lợi nhuận.
        - Nếu giá giảm quá ngưỡng, giữ giá SELL để bảo vệ lợi nhuận.
        """
        if not config.trailing_stop_enabled:
            return False, initial_sell_price

        # Khởi tạo giá trailing nếu chưa có
        if self.trailing_sell_price == 0.0:
            self.trailing_sell_price = initial_sell_price

        # Nếu giá hiện tại cao hơn giá trailing, tăng giá trailing
        ...
        # Nếu giá hiện tại giảm xuống dưới ngưỡng trailing stop, giữ giá SELL
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.high_history: deque = deque(maxlen=config.volatility_window)
        self.low_history: deque = deque(maxlen=config.volatility_window)
        self.sell_orders: Dict[str, Tuple[float, float]] = {}  # Lưu trữ các lệnh SELL: {order_id: (price, quantity)}
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        # Giới hạn volatility để tránh step_percent quá lớn
        ...
        # Giới hạn step_percent tối đa 2%
        ...
        for i in range(num_levels):
            ...
            # Đảm bảo giá không âm và không quá lớn
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface', bot_state: 'BotState' = None) -> None:
        ...
        # Tính toán số lượng cho mỗi mức lưới
        quantity_per_level = config.initial_investment / current_price / max(1, len(self.grid_levels))
        
        # Đảm bảo số lượng đáp ứng yêu cầu tối thiểu và bước giá
        ...
        # Đảm bảo luôn có ít nhất một lệnh BUY
        has_buy_order = False
        for buy_price, sell_price in self.grid_levels:
            try:
                # Kiểm tra giá mua và giá bán hợp lệ
                if buy_price <= 0 or sell_price <= 0:
                    logging.warning(f"Giá không hợp lệ: buy_price={buy_price}, sell_price={sell_price}. Bỏ qua mức lưới này.")
                    continue
                ...
                # Chỉ đặt lệnh SELL nếu có đủ số lượng đã mua
        ...
        # Nếu không có lệnh BUY nào được đặt, đặt một lệnh BUY ở mức thấp hơn current_price
        if not has_buy_order:
            try:
                buy_price = current_price * 0.99  # Đặt lệnh BUY thấp hơn 1% so với giá hiện tại
                if buy_price > 0:
                    buy_order = await safe_api_call(
                        exchange.place_order,
                        side='BUY',
    ...
    async def update_trailing_sell_orders(self, current_price: float, exchange: 'ExchangeInterface', protection: 'EnhancedProtectionSystem') -> None:
        ...
        # Lấy danh sách các lệnh đang mở từ sàn
        ...
        for order in sell_orders_to_update:
            ...
            # Tính toán giá mới theo cơ chế trailing
...
class EnhancedOrderManager:
    def __init__(self):
        ...
        self.bought_quantity: float = 0.0  # Theo dõi số lượng đã mua
        ...
        try:
            ...
            for order in orders:
                if order['status'] == 'FILLED':
                    ...
                    # Cập nhật số lượng đã mua khi lệnh khớp
                    if side == 'BUY':
    ...
    async def process_events(self) -> None:
        while True:
            ...
            for signal in signals:
                try:
                    # Kiểm tra số lượng trước khi đặt lệnh SELL
            ...
            # Cập nhật các lệnh SELL theo cơ chế trailing
...
class BinanceExchange(BaseExchange):
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        try:
            ...
            # Kiểm tra nếu là lệnh giới hạn, price không được là None
            ...
            if price is not None:
                ...
                # Làm tròn giá theo độ chính xác của sàn
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    try:
        ...
        # Đồng bộ số lượng đã mua với số dư BTC hiện có
        ...
        while True:
            ...
            # Cập nhật trạng thái ngay cả khi có lỗi trong process_events
...
def update_dashboard(n):
    ...
    try:
        # Lấy dữ liệu từ các sàn giao dịch
        exchange_manager = ExchangeManager()
        states = {}  # Lưu trữ trạng thái của bot cho từng sàn

        # Thu thập dữ liệu giá và lợi nhuận từ tất cả các sàn
        price_data = {}
        profit_data = {}
        all_open_orders = []

        for exchange_name, exchange in exchange_manager.exchanges.items():
            # Khởi tạo trạng thái bot nếu chưa có
            ...
            try:
                ...
                logging.error(f"Lỗi khi lấy giá từ {exchange_name}: {str(e)}")
                current_price = 0.0

            # Lấy lịch sử giá từ grid_manager
            ...
            # Thêm dữ liệu giá vào biểu đồ
            price_data[exchange_name] = go.Scatter(
                x=timestamps,
                y=price_history,
                mode='lines',
            ...
            # Thêm dữ liệu lợi nhuận vào biểu đồ
            profit_data[exchange_name] = go.Scatter(
                x=profit_timestamps,
                y=equity_curve,
                mode='lines',
        ...
        price_fig = go.Figure(data=list(price_data.values()))
        ...
        profit_fig = go.Figure(data=list(profit_data.values()))
        ...
        # Cập nhật danh sách lệnh mở
        if all_open_orders:
            open_orders_text = html.Ul([html.Li(order) for order in all_open_orders])
        else:
            open_orders_text = "Không có lệnh mở"
