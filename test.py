...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        if atr == 0:  # Nếu ATR = 0, sử dụng giá trị mặc định nhỏ (0.1% giá hiện tại)
        ...
        for i in range(num_levels):
            ...
            if buy_price <= 0 or sell_price <= 0:
                ...
                continue
            # Điều chỉnh giới hạn: chỉ cho phép mức BUY >= 90% giá hiện tại, SELL <= 110%
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        if num_levels == 0:
            ...
            return
        # Dành một nửa vốn cho BUY và một nửa cho SELL, chia đều cho số grid_levels
        ...
        for buy_price, sell_price in self.grid_levels:
            ...
            try:
                ...
                # Đặt lệnh BUY nếu giá thấp hơn giá hiện tại và không trong cooldown
                ...
                # Đặt lệnh SELL nếu giá cao hơn giá hiện tại và không trong cooldown
...
class AdaptiveGridStrategy(GridStrategy):
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        ...
        # Cập nhật grid ngay trước khi tạo tín hiệu dựa trên giá mới nhất
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        # Cập nhật grid dựa trên giá mới
