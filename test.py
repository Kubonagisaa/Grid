...
class AdvancedConfig:
    def __init__(self):
        ...
        self.base_stop_loss_percent: float = float(os.getenv("BASE_STOP_LOSS_PERCENT", 1.5))  # Giảm từ 2.0 xuống 1.5
        self.base_take_profit_percent: float = float(os.getenv("BASE_TAKE_PROFIT_PERCENT", 1.5))  # Giảm từ 2.0 xuống 1.5
        ...
        self.min_grid_levels: int = int(os.getenv("MIN_GRID_LEVELS", 5))  # Tăng từ 3 lên 5
        self.max_grid_levels: int = int(os.getenv("MAX_GRID_LEVELS", 10))
        self.base_grid_step_percent: float = float(os.getenv("BASE_GRID_STEP_PERCENT", 0.5))  # Giảm từ 1.0 xuống 0.5
        ...
        self.trailing_up_activation: float = float(os.getenv("TRAILING_UP_ACTIVATION", 1.0))  # % thay vì giá tuyệt đối
        self.trailing_down_activation: float = float(os.getenv("TRAILING_DOWN_ACTIVATION", 1.0))  # % thay vì giá tuyệt đối
        ...
        self.trailing_buy_activation_percent: float = float(os.getenv("TRAILING_BUY_ACTIVATION_PERCENT", 1.0))  # Giảm từ 1.5 xuống 1.0
        self.trailing_buy_distance_percent: float = float(os.getenv("TRAILING_BUY_DISTANCE_PERCENT", 0.5))  # Giảm từ 1.0 xuống 0.5
...
class EnhancedProfitTracker:
    ...
    def record_trade(self, profit: float, fee: float) -> None:
        ...
        # Kiểm tra nếu giao dịch trước đó quá gần (trong vòng 10 giây)
        if current_time - self.last_trade_time < 10:
            logging.warning(f"Giao dịch được ghi nhận quá sớm sau giao dịch trước đó: {current_time - self.last_trade_time:.2f}s")
            return
...
class EnhancedProtectionSystem:
    ...
    def update(self, price: float, high: float, low: float, volume: float) -> None:
        current_time = time.time()
        if current_time - self.last_checked_time < 5:  # Chỉ cập nhật mỗi 5 giây
            return
    ...
    def update_trailing_stop(self, price: float) -> Tuple[bool, float]:
        ...
        if price > self.trailing_stop_price and price > self.price_history[-1] * (1 + config.trailing_up_activation/100):
            self.trailing_stop_price = price * (1 - config.base_stop_loss_percent / 100)
        elif price < self.trailing_stop_price and price < self.price_history[-1] * (1 - config.trailing_down_activation/100):
            self.trailing_stop_price = price * (1 + config.base_stop_loss_percent / 100)
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.high_history: deque = deque(maxlen=config.volatility_window)
        self.low_history: deque = deque(maxlen=config.volatility_window)
        self.last_order_placement: float = 0.0
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        current_time = time.time()
        if current_time - self.last_order_placement < 30:  # Chỉ đặt lệnh mỗi 30 giây
            logging.info("Bỏ qua đặt lệnh do khoảng cách thời gian quá ngắn")
            return
            
        self.last_order_placement = current_time
        
        # Lấy danh sách lệnh đang mở
        open_orders = await safe_api_call(exchange.get_open_orders)
        existing_orders = {o['price']: o for o in open_orders}
        
        # Hủy các lệnh không còn trong grid levels
        for order_id in self.order_ids[:]:
            order = next((o for o in open_orders if o['orderId'] == order_id), None)
            if not order:
                continue
            if not any(abs(float(order['price']) - level[0] < 0.01 or 
                      abs(float(order['price']) - level[1] < 0.01 
        ...
        for buy_price, sell_price in self.grid_levels:
            try:
                ...
                # Kiểm tra nếu lệnh đã tồn tại
                if buy_price in existing_orders or sell_price in existing_orders:
                    continue
                    
                if buy_price < current_price:
                    if not any(o['side'] == 'BUY' and abs(float(o['price']) - buy_price) < 0.01 for o in open_orders):
                ...
                if sell_price > current_price:
                    if not any(o['side'] == 'SELL' and abs(float(o['price']) - sell_price) < 0.01 for o in open_orders):
        ...
        if not has_buy_order:
            try:
                ...
                if buy_price > 0 and not any(o['side'] == 'BUY' and abs(float(o['price']) - buy_price) < 0.01 for o in open_orders):
                    buy_order = await safe_api_call(
                        exchange.place_order,
                        side='BUY',
                        order_type='LIMIT',
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        for order_id in self.order_ids[:]:
            try:
                ...
                # Nếu lệnh không tồn tại, xóa khỏi danh sách
                self.order_ids.remove(order_id)
...
class EnhancedOrderManager:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface', profit_tracker: 'EnhancedProfitTracker') -> None:
        current_time = time.time()
        if current_time - self.last_check_time < 10:  # Chỉ kiểm tra mỗi 10 giây
            return
        ...
        try:
            ...
            # Kiểm tra trùng lặp - chỉ xử lý các lệnh duy nhất
            unique_orders = {}
            for order in filled_orders:
                key = (order['side'], order['price'], order['executedQty'])
...
class AdaptiveGridStrategy(GridStrategy):
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        ...
        current_time = time.time()
        if current_time - self.last_signal_time < 30:  # Chỉ tạo signal mỗi 30 giây
            return []
...
class BotState:
    ...
    async def process_events(self) -> None:
        current_time = time.time()
        if current_time - self.last_processed_time < 60:  # Chỉ xử lý mỗi phút
            return
