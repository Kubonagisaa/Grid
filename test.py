...
HEALTH_ALERT_COOLDOWN = 600  # 10 phút cooldown
...
class AdvancedConfig:
    def __init__(self):
        ...
        self.symbol: str = os.getenv("TRADING_SYMBOL", "DOGEUSDT").upper()
        self.initial_investment: float = float(os.getenv("INITIAL_INVESTMENT", 100))
        self.min_quantity: float = float(os.getenv("MIN_QUANTITY", 1))
        self.max_position: float = float(os.getenv("MAX_POSITION", 100))
        ...
        self.max_grid_levels: int = int(os.getenv("MAX_GRID_LEVELS", 7))
        ...
        self.trailing_up_activation: float = float(os.getenv("TRAILING_UP_ACTIVATION", 0.7))
        self.trailing_down_activation: float = float(os.getenv("TRAILING_DOWN_ACTIVATION", 0.5))
        ...
        self.trailing_buy_activation_percent: float = float(os.getenv("TRAILING_BUY_ACTIVATION_PERCENT", 2.0))
        ...
        self.circuit_breaker_duration: int = int(os.getenv("CIRCUIT_BREAKER_DURATION", 180))
        ...
        self.max_open_orders: int = int(os.getenv("MAX_OPEN_ORDERS", 5))
        ...
        self.order_cooldown_sec: int = int(os.getenv("ORDER_COOLDOWN_SECONDS", 60))
        self.paper_trading_enabled: bool = os.getenv("PAPER_TRADING", "True").lower() == "true"
        ...
        self.strategy_mode: int = int(os.getenv("STRATEGY_MODE", 2))
        ...
        self.min_profit_percent: float = float(os.getenv("MIN_PROFIT_PERCENT", 0.5))
...
class EnhancedProtectionSystem:
    ...
    def update_trailing_stop(self, price: float) -> Tuple[bool, float]:
        ...
        if price > self.trailing_stop_price and price > config.trailing_up_activation:
            self.trailing_stop_price = price - atr * 3  # ATR multiplier
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        try:
            ...
            for order in open_orders:
                if order is None or 'orderId' not in order:
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface',
        ...
        try:
            ...
            if orders is None:
                ...
                return
            for order in orders:
                if order is None or 'orderId' not in order or 'status' not in order:
                    logging.warning(f"Lệnh không hợp lệ: {order}")
                    continue
                if order['status'] == 'FILLED':
                    ...
                    if order.get('side', '').upper() == 'BUY' and order['orderId'] in grid_manager.order_ids:
                        grid_manager.order_ids.remove(order['orderId'])
        except Exception as e:
...
class EnhancedProfitTracker:
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        sharpe_ratio = 0.0  # Có thể thêm logic tính Sharpe Ratio
        return {
...
async def verify_order_state(exchange: 'ExchangeInterface', local_order_ids: List[str]) -> None:
    try:
        actual_orders = await safe_api_call(exchange.get_open_orders)
        if actual_orders is None:
            logging.error("Không lấy được danh sách lệnh mở để đồng bộ")
            return
        actual_ids = {order.get('orderId') for order in actual_orders if order and 'orderId' in order}
...
class EnhancedSmartGridManager:
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        best_bid = order_book['bids'][0][0] if order_book.get('bids') else current_price
        best_ask = order_book['asks'][0][0] if order_book.get('asks') else current_price
    ...
    def duplicate_order_exists(self, side: str, price: float, open_orders: List[Dict]) -> bool:
        ...
        for order in open_orders:
            if order is None or 'price' not in order or order['price'] is None:
                continue
    ...
    async def place_static_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        async with order_lock:
            ...
            if usdt_balance is None:
                logging.error("Không lấy được số dư, bỏ qua đặt lệnh")
                return
            ...
            for buy_price, sell_price in static_grid:
                if buy_price is None or sell_price is None:
                    logging.error(f"Giá lưới không hợp lệ: buy={buy_price}, sell={sell_price}")
                    continue
                try:
                    if buy_price <= 0 or sell_price <= 0:
                        continue
                    ...
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        ...
                        if buy_order is None or 'orderId' not in buy_order:
                            continue
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        async with order_lock:
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                ...
                try:
                    ...
                    if sell_price > current_price:
                        ...
                        if sell_order is None or 'orderId' not in sell_order:
                            continue
            ...
            has_buy_order = any(o['side'].upper() == 'BUY' for o in open_orders if o is not None)
            if not has_buy_order and len([o for o in open_orders if o is not None and o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > self.FALLBACK_COOLDOWN:
    ...
    def get_grid_range(self) -> Tuple[Optional[float], Optional[float]]:
        if not self.grid_levels:
            return None, None
        lows = [l for l, _ in self.grid_levels if l is not None]
        highs = [h for _, h in self.grid_levels if h is not None]
...
class TrendDetector:
    ...
    async def detect_trend_strength(self) -> float:
        ...
        adx_df = pd.DataFrame({'ADX_14': [0] * len(df)})  # Giả lập nếu pandas_ta không có
        try:
...
class NewsAnalyzer:
    ...
    def _analyze_sentiment(self, articles: Dict) -> float:
        if 'articles' not in articles:
            return 0.0
        sentiments = [TextBlob(a.get('title', '')).sentiment.polarity for a in articles['articles']]
...
class BlackSwanProtection:
    def __init__(self, state: 'BotState'):
        ...
        self.last_black_swan_time: float = 0.0
    ...
    async def monitor(self) -> None:
        ...
        if liquidity_ratio < 0.5:
            order_book = self.last_order_book or await self.state.exchange.get_order_book()
            ...
            if len(self.state.protection.price_history) >= 2:
                ...
                if last_price > 0:
                    price_drop = (last_price - self.state.current_price) / last_price
            abnormal_vol = self.state.protection.check_abnormal_activity(self.state.current_volume)
            if (price_drop > config.price_drop_threshold) or (spread > 0.05 and abnormal_vol):
    ...
    async def _calculate_liquidity(self) -> float:
        try:
            ...
            bid_liquidity = sum([bid[1] for bid in order_book['bids'][:10]] if 'bids' in order_book else [])
            ask_liquidity = sum([ask[1] for ask in order_book['asks'][:10]] if 'asks' in order_book else [])
...
class SimpleMLPredictor:
    ...
    def predict(self, features: np.ndarray) -> int:
        if not self.trained or len(features) == 0:
            return 0
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        features = [
            ...
            calculate_bollinger_bands(list(self.grid_manager.price_history), config.bb_period, config.bb_std_dev)[0],  # Lower band
            calculate_bollinger_bands(list(self.grid_manager.price_history), config.bb_period, config.bb_std_dev)[1],  # Upper band
        ]
        ...
        if self.ml_predictor.trained:
            prediction = self.ml_predictor.predict(np.array(features))
            if prediction == 1:
        ...
        capital = usdt_balance + coin_balance * self.current_price if self.current_price > 0 else 0.0
        ...
        async with order_lock:
            for signal in signals:
                try:
                    order = await safe_api_call(self.exchange.place_order, signal.side, "LIMIT", signal.quantity, signal.price)
                    if order is None or 'orderId' not in order:
        ...
        if config.telegram_enabled:
            ...
            if len(self.protection.price_history) > 1:
                ...
                price_change = abs(self.current_price - first_price) / first_price * 100 if first_price > 0 else 0
    ...
    async def handle_trailing_stops(self):
        ...
        for (side, entry_price), data in self.grid_manager.trailing_stops.items():
            ...
            target_price = entry_price + atr * 3  # ATR-based take profit
            if highest_price >= target_price:
                stop_price = highest_price - atr * 2  # ATR-based stop loss
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        ...
        logging.info(f"Khởi tạo số dư: {self.simulated_balance}")
        self.paper_orders = []
...
async def safe_api_call(func, *args, retries=3, delay=2, **kwargs):
    for attempt in range(retries):
        try:
            ...
            logging.warning(f"Lỗi API lần {attempt+1}/{retries}: {str(e)}")
...
if __name__ == "__main__":
    async def main():
        ...
        exchange.simulated_balance["USDT"] = config.initial_investment  # Reset rõ ràng
        exchange.simulated_balance[config.symbol.replace("USDT", "")] = 0.0
        logging.info(f"Reset số dư khi khởi động: {exchange.simulated_balance}")
