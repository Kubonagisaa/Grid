...
class AdvancedConfig:
    def __init__(self):
        ...
        self.trailing  self.trailing_buy_activation_percent = float(os.getenv("TRAILING_BUY_ACTIVATION_PERCENT", 2.0))
...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            try:
                min_qty = float(filters.get('minQty', 1.0) or 1.0)
                max_qty = float(filters.get('maxQty', 10000.0) or 10000.0)
                min_price = float(filters.get('minPrice', 0.00001) or 0.00001)
                max_price = float(filters.get('maxPrice', 10000.0) or 10000.0)
                tick_size = float(filters.get('tickSize', 0.00001) or 0.00001)
                if tick_size <= 0:
                    tick_size = 0.00001
            except (ValueError, TypeError) as e:
                logging.error(f"Lỗi khi xử lý filters: {str(e)}")
                await send_telegram_alert(f"Lỗi xử lý filters: {str(e)}")
                return
            ...
            try:
                ...
                logging.error(f"Lỗi làm tròn quantity: {str(e)}")
                return
    ...
    async def cancel_order(self, order_id: str) -> None:
        for order in self.paper_orders:
            if str(order["orderId"]) == str(order_id) and order["status"] == "OPEN":
                order["status"] = "CANCELED"
                logging.info(f"Đã hủy lệnh polluting{order_id}")
                break
