...
class BlackSwanProtection:
    def __init__(self, state: 'BotState'):
        self.state = state
        # Thêm thuộc tính lưu trữ dữ liệu gần đây để phân tích Black Swan
        self.last_order_book: Optional[Dict] = None
        self.last_black_swan_time: float = 0.0

    async def monitor(self) -> None:
        # Kiểm tra dữ liệu thị trường có mới hay không trước khi phân tích
        current_time = time.time()
        ...
        if liquidity_ratio < 0.5:
            # Tính toán các yếu tố bổ sung để xác nhận sự kiện Black Swan
            ...
            # Tính phần trăm giá giảm từ lần cập nhật trước
            price_drop = 0.0
    ...
    async def _calculate_liquidity(self) -> float:
        try:
            order_book = await self.state.exchange.get_order_book()
            # Lưu trữ order_book để dùng trong phân tích chi tiết
    ...
    async def _emergency_liquidation(self) -> None:
        ...
        if coin_balance > 0:
            try:
                # Trong trường hợp Paper Trading, thực hiện giảm số dư thay vì lệnh market thực
