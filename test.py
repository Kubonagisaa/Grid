...
class AdvancedConfig:
    def __init__(self):
        ...
        self.grid_rebalance_interval = int(os.getenv("GRID_REBALANCE_INTERVAL", 60))
...
class EnhancedProfitTracker:
    ...
    def record_trade(self, profit: float, fee: float, side: str) -> None:
        ...
        if self.consecutive_buys >= self.max_consecutive_trades:
            ...
            send_telegram_alert(f"⚠️ Too many consecutive BUY trades: {self.consecutive_buys}")
        if self.consecutive_sells >= self.max_consecutive_trades:
            ...
            send_telegram_alert(f"⚠️ Too many consecutive SELL trades: {self.consecutive_sells}")
...
class SmartGridManager:
    ...
    async def update_grid(self, current_price: float, exchange: 'ExchangeInterface', drawdown: float, profit_tracker: 'EnhancedProfitTracker') -> None:
        try:
            ...
            # Standard conditions for grid update
            should_update = (
                not self.trailing_enabled or
                self.last_price == 0.0 or
                abs(current_price - self.last_price) >= (self.grid_spacing * current_price * 0.5) or  # Lowered threshold to 50% of grid spacing
                time.time() - self.last_adjust_time > config.grid_rebalance_interval
            )
            
            # Force grid update if there are too many consecutive trades
            ...
            if self.adaptive_enabled:
                if stats["Consecutive Sells"] >= profit_tracker.max_consecutive_trades:
                    ...
                    force_update = True
                    profit_tracker.consecutive_sells = 0  # Reset counter after adjustment
                    ...
                    force_update = True
                    profit_tracker.consecutive_buys = 0  # Reset counter after adjustment
            ...
            if not (should_update or force_update):
                logging.debug(f"Grid update skipped: should_update={should_update}, force_update={force_update}, "
                              ...
                              f"threshold={self.grid_spacing * current_price * 0.5:.2f}, "
            ...
            try:
                ...
                if order:
                    ...
                    if abs(price - current_price) / current_price > 0.05:
                        ...
                        logging.info(f"Canceled order {order_id} at {price:.2f} (too far from current price {current_price:.2f})")
            except Exception as e:
                logging.error(f"Failed to check or cancel order {order_id}: {str(e)}")
...
async def safe_api_call(func, allow_empty: bool = False, **kwargs):
    try:
        ...
        send_telegram_alert(f"⚠️ API call failed: {func.__name__}, error: {str(e)}")
...
def update_dashboard(n_intervals):
    bot = BotState(exchange, strategy)  # This needs to be properly initialized
...
async def main():
    ...
    # Cancel all existing orders at startup
...
if __name__ == "__main__":
    ...
    try:
        ...
        send_telegram_alert(f"⚠️ Unexpected error: {str(e)}")
        # Cancel all orders on unexpected error
