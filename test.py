...
class AdvancedConfig:
    def __init__(self):
        ...
        # THÊM biến cấu hình paper trading; true nếu muốn chạy chế độ mô phỏng giao dịch (không thay đổi số dư thật)
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        # Đặt tên sàn là "paper_trading" và không dùng API key thật
        super().__init__("paper_trading", "", "")
        self.symbol = symbol
        # Tạo tài khoản ảo với số dư ban đầu (cho cặp BTCUSDT, tài khoản chứa USDT và BTC)
    ...
    async def get_price(self) -> float:
        if self.current_market_price is None:
            self.current_market_price = 100.0  # giá mặc định nếu chưa cập nhật
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        if order_type.upper() == "LIMIT" and (price is None or price <= 0):
            raise ValueError("Lệnh giới hạn yêu cầu giá hợp lệ")
        ...
        order = {
            ...
            "status": "FILLED",  # Giả sử khớp ngay khi đặt lệnh nếu điều kiện đáp ứng
            "timestamp": time.time(),
    ...
    async def get_open_orders(self) -> List[Dict]:
        return [o for o in self.paper_orders if o["status"] not in ("FILLED", "CANCELED")]
...
# ======== THỰC THI GIAO DỊCH THỰC (ví dụ Binance và MEXC) ========
...
async def main():
    ...
    if config.paper_trading_enabled:
        logging.info("Chạy chế độ paper trading: sử dụng tài khoản ảo")
        paper_exchange = PaperTradingExchange(config.symbol)
        await run_simulated_backtest(paper_exchange)
        bot_task = asyncio.create_task(run_bot_for_exchange(paper_exchange, bot_states))
...
if __name__ == "__main__":
    try:
        ...
        logging.critical(f"Lỗi thực thi main: {str(e)}")