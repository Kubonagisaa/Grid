...
class DistributedLockManager:
    ...
    def release_lock(self, lock_name, identifier):
        ...
        try:
            pipe.watch(lock_key)
            current_id = pipe.get(lock_key)
            if current_id and current_id.decode() == identifier:
                pipe.multi()
...
class PreTradeGateway:
    ...
    async def validate_order(self, side: str, quantity: float, price: float) -> bool:
        """Validate order against risk parameters"""
        # 1. Check exposure limits
        exposure = await self.calculate_exposure(side, quantity, price)
        if exposure > config.max_exposure_per_trade:
            logging.warning(f"Order rejected: Exposure {exposure:.2f} > limit {config.max_exposure_per_trade}")
            return False
            
        # 2. Check available balance
        usdt_balance, coin_balance = await safe_api_call(self.state.exchange.get_balance)
        if side == 'BUY':
            required = quantity * price
            if required > usdt_balance * 0.95:  # 5% buffer
                logging.warning(f"Order rejected: Insufficient USDT {usdt_balance:.2f} < required {required:.2f}")
                return False
        else:  # SELL
            if quantity > coin_balance * 0.95:  # 5% buffer
                logging.warning(f"Order rejected: Insufficient coin {coin_balance:.6f} < required {quantity:.6f}")
                return False
                
        # 3. Check volatility filter
        volatility = await self.calculate_volatility()
        if volatility > config.max_volatility_for_trade:
            logging.warning(f"Order rejected: Volatility {volatility:.2%} > limit {config.max_volatility_for_trade:.2%}")
            return False
            
        return True
        
    async def calculate_exposure(self, side: str, quantity: float, price: float) -> float:
        """Calculate exposure in USDT"""
        return quantity * price
        
    async def calculate_volatility(self) -> float:
        """Calculate current market volatility"""
        if len(self.state.grid_manager.price_history) < 20:
            return 0.0
            
        prices = list(self.state.grid_manager.price_history)[-20:]
        returns = [math.log(prices[i]/prices[i-1]) for i in range(1, len(prices))]
        return np.std(returns) * math.sqrt(365)  # Annualized volatility

# ======== Exposure Monitor ========
class ExposureMonitor:
    def __init__(self, state: 'BotState'):
        self.state = state
        self.max_daily_drawdown = -float(os.getenv("MAX_DAILY_DRAWDOWN_PERCENT", 2.0))
        self.daily_high_equity = config.initial_investment
        if state.tracker.equity_curve:
            self.daily_high_equity = max(state.tracker.equity_curve)
        self.last_reset_time = time.time()
        
    async def update(self, realized_pnl: float) -> None:
        """Update exposure monitor with new PNL"""
        current_equity = await self.state._get_current_capital()
        
        # Reset daily tracking at midnight
        if datetime.now().hour == 0 and time.time() - self.last_reset_time > 86400:
        ...
        daily_drawdown = (current_equity - self.daily_high_equity) / self.daily_high_equity * 100
        
        # Activate kill switch if drawdown threshold breached
        if daily_drawdown < self.max_daily_drawdown:
            logging.critical(f"KILL SWITCH ACTIVATED! Daily drawdown {daily_drawdown:.2f}%")
    ...
    async def activate_kill_switch(self):
        ...
        _, coin_balance = await safe_api_call(self.state.exchange.get_balance)
        if coin_balance > 0:
            await safe_api_call(
                self.state.exchange.place_order, 
                "SELL", 
                "MARKET", 
                coin_balance
            )
        ...
        # 3. Shutdown bot
        await discord_notifier.send_alert(" KILL SWITCH ACTIVATED! Bot shutdown", is_error=True)
...
class RealisticFillSimulator:
    ...
    async def simulate_fill(self, order: Dict) -> Dict:
        ...
        order_book = await safe_api_call(self.exchange.get_order_book)
        current_price = await safe_api_call(self.exchange.get_price)
        
        # Calculate realistic fill parameters
        if order_type == 'MARKET':
            filled_price, filled_qty = self.simulate_market_order(side, quantity, order_book)
        else:  # LIMIT order
            filled_price, filled_qty = self.simulate_limit_order(side, price, quantity, order_book, current_price)
            
        # Update order status
        order['status'] = 'FILLED' if filled_qty >= quantity * 0.95 else 'PARTIALLY_FILLED'
        order['executedQty'] = filled_qty
        order['avgPrice'] = filled_price
        
        return order
        
    def simulate_market_order(self, side: str, quantity: float, order_book: Dict) -> Tuple[float, float]:
    ...
    def simulate_limit_order(self, side: str, price: float, quantity: float, 
                             order_book: Dict, current_price: float) -> Tuple[float, float]:
        """Simulate limit order fill with partial fills"""
        # Calculate fill probability based on historical data
        fill_probability = self.calculate_fill_probability(side, price, current_price)
        
        # Simulate fill based on probability
        if random.random() < fill_probability:
            # Partial fill simulation
            fill_ratio = min(1.0, max(0.3, random.gauss(0.8, 0.2)))  # 30-100% fill
            filled_qty = quantity * fill_ratio
            
            # Add slight price improvement
            price_improvement = 0.0001 * current_price * (-1 if side == 'BUY' else 1)
            filled_price = price + price_improvement
            
            return filled_price, filled_qty
        return 0.0, 0.0
        
    def calculate_fill_probability(self, side: str, price: float, current_price: float) -> float:
        """Calculate fill probability based on distance to market"""
        price_distance = abs(price - current_price) / current_price
        
        # Base probability curve
        if side == 'BUY':
            probability = 1.0 - min(1.0, price_distance * 50)  # 2% distance = 0% probability
        else:  # SELL
            probability = 1.0 - min(1.0, price_distance * 50)
            
        # Adjust based on volatility
        volatility = 0.05  # Placeholder - should use actual volatility
        probability *= max(0.5, 1.0 - volatility * 10)
...
class MarketRegimeClassifier:
    def __init__(self):
        self.volatility_state = "NORMAL"  # LOW/MEDIUM/HIGH/EXTREME
        self.liquidity_state = "HEALTHY"  # WARNING/DANGER
        self.trend_strength = 0.0
        
    async def update(self, state: 'BotState'):
        """Classify current market regime"""
        # 1. Volatility classification
        await self.classify_volatility(state)
        
        # 2. Liquidity assessment
        await self.assess_liquidity(state)
        
        # 3. Trend strength
        self.trend_strength = await state.trend_detector.detect_trend_strength()
        
        # 4. Black swan detection
        if await state.black_swan_protection.monitor():
            self.volatility_state = "EXTREME"
            self.liquidity_state = "DANGER"
            
        logging.info(f"Market Regime: Volatility={self.volatility_state}, Liquidity={self.liquidity_state}, Trend={self.trend_strength:.1f}")
        
    async def classify_volatility(self, state: 'BotState'):
        """Classify volatility regime"""
        if len(state.protection.price_history) < 50:
            return
            
        prices = list(state.protection.price_history)[-50:]
        returns = [math.log(prices[i]/prices[i-1]) for i in range(1, len(prices))]
        volatility = np.std(returns) * math.sqrt(365)  # Annualized
    ...
    async def assess_liquidity(self, state: 'BotState'):
        """Assess market liquidity conditions"""
        try:
            order_book = await safe_api_call(state.exchange.get_order_book)
            if not order_book:
                return
                
            # Calculate bid/ask liquidity ratio
            bid_liquidity = sum([bid[1] for bid in order_book.get('bids', [])[:5]])
            ask_liquidity = sum([ask[1] for ask in order_book.get('asks', [])[:5]])
            
            if bid_liquidity == 0 or ask_liquidity == 0:
                return
                
            liquidity_ratio = min(bid_liquidity, ask_liquidity) / max(bid_liquidity, ask_liquidity)
            
            # Classify liquidity state
            if liquidity_ratio < 0.3:
                self.liquidity_state = "DANGER"
            elif liquidity_ratio < 0.6:
                self.liquidity_state = "WARNING"
            else:
                self.liquidity_state = "HEALTHY"
                
        except Exception as e:
            logging.error(f"Liquidity assessment error: {str(e)}")
...
class SmartOrderRouter:
    def __init__(self, exchange: 'IExchange'):
        ...
        self.order_cooldown = 2  # seconds
        
    async def place_order(self, side: str, quantity: float, price: float = None) -> Dict:
        """Intelligently route orders based on market conditions"""
        current_time = time.time()
        if current_time - self.last_trade_time < self.order_cooldown:
            await asyncio.sleep(self.order_cooldown - (current_time - self.last_trade_time))
            
        # Determine optimal order type
        if price is None:
            order_type = "MARKET"
        else:
            # Use limit orders in normal conditions, market in volatile
            order_type = "LIMIT" if self.exchange.regime_classifier.volatility_state in ["LOW", "NORMAL"] else "MARKET"
...
class DiscordNotifier:
    ...
    async def send_alert(self, message: str, is_error: bool = False):
        ...
        embed = {
            "title": " Cnh Báo" if is_error else " Thông Báo",
            "description": message,
        ...
        try:
            ...
            logging.error(f"Li gi thông báo Discord: {str(e)}")
...
class AdvancedConfig:
    def __init__(self):
        ...
        # Thêm tham s PRO
        self.max_exposure_per_trade = float(os.getenv("MAX_EXPOSURE_PER_TRADE", 1e6))
        self.max_volatility_for_trade = float(os.getenv("MAX_VOLATILITY_FOR_TRADE", 0.05))
        self.realized_pnl_tracking = os.getenv("REALIZED_PNL_TRACKING", "True").lower() == "true"
        self.distributed_lock_enabled = os.getenv("DISTRIBUTED_LOCK_ENABLED", "False").lower() == "true"
        self.max_daily_drawdown = float(os.getenv("MAX_DAILY_DRAWDOWN_PERCENT", 2.0))
        self.micro_slippage_enabled = os.getenv("MICRO_SLIPPAGE_ENABLED", "True").lower() == "true"
        self.volatility_regime_enabled = os.getenv("VOLATILITY_REGIME_ENABLED", "True").lower() == "true"

config = AdvancedConfig()

logging.info(f"Chọn chiến lược {config.strategy_mode}: {'Grid thưa (bước ~2-3%, có RSI)' if config.strategy_mode == 1 else 'Grid dày (bước ~0.5%, không dùng RSI)'}")
logging.info(f"Bước lời cơ bản: {config.base_grid_step_percent:.2f}%, Mức grid tối thiểu: {config.min_grid_levels}, tối đa: {config.max_grid_levels}")
...
def format_symbol(symbol: str) -> str:
    """Chuyển đổi symbol từ dạng 'DOGEUSDT' sang 'DOGE/USDT'"""
    if "USDT" in symbol:
...
async def monitor_system_health() -> None:
    ...
    if (cpu > 85 or memory > 80) and (current_time - last_health_alert_time > HEALTH_ALERT_COOLDOWN):
        await discord_notifier.send_alert(f"! Cảnh báo: CPU {cpu}% | RAM {memory}%", is_error=True)
...
class EnhancedProtectionSystem:
    ...
    async def check_circuit_breaker(self, price: float) -> bool:
        ...
        if price_change > config.circuit_breaker_threshold:
            ...
            await discord_notifier.send_alert(f"Circuit breaker: biến động giá quá lớn ({price_change:.2%}), tạm dừng giao dịch", is_error=True)
            return True
        return False
...
# ======== Enhanced Order Manager (đã nâng cấp) ======== 
class EnhancedOrderManager:
    def __init__(self, state: 'BotState'):
        ...
        self.state = state
        self.pre_trade_gateway = PreTradeGateway(state)  # Thêm cổng kiểm tra
        self.smart_router = SmartOrderRouter(state.exchange)  # Thêm bộ định tuyến thông minh
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        ...
        if not await self.pre_trade_gateway.validate_order(side, quantity, price):
            return None
            
        # 2. Use distributed lock for atomic operations
        lock_id = None
        if config.distributed_lock_enabled:
            lock_name = f"order_{side}_{self.state.symbol}"
            lock_id = lock_manager.acquire_lock(lock_name)
            if not lock_id:
                logging.error(f"Failed to acquire lock for {lock_name}")
                return None
                
        try:
            # 3. Smart order routing
            order = await self.smart_router.place_order(side, quantity, price)
            
            # 4. Realistic fill simulation for paper trading
            if config.paper_trading_enabled and config.micro_slippage_enabled:
                order = await self.state.exchange.fill_simulator.simulate_fill(order)
                
            return order
        finally:
            if lock_id:
                lock_manager.release_lock(lock_name, lock_id)
            ...
            for order in orders:
                ...
                if order['status'] == 'FILLED':
                    ...
                    if order['orderId'] in grid_manager.static_order_ids and side == 'BUY':
                        ...
                        logging.info(f"Đã khớp lệnh static BUY tại {price:.6f}, qty {qty:.6f}")
                        await discord_notifier.send_alert(f"Static BUY filled @ {price:.6f}, qty {qty:.6f}")
...
async def load_state(states: Dict[str, 'BotState']) -> None:
    try:
        ...
        for symbol, state in states.items():
            if symbol in state_data:
                ...
                await verify_order_state(state.exchange, state.grid_manager.order_ids)
...
# ======== Grid Manager (đã nâng cấp) ======== 
class EnhancedSmartGridManager:
    def __init__(self, exchange=None):  # Sửa: Thêm exchange làm tham số
        self.exchange = exchange  # Sửa: Gán exchange
        self.grid_levels = []
        ...
        self.trailing_stops = {}
        self.grid_cooldown = {}
        self.price_precision = 6  # Tăng độ chính xác giá
        ...
        self.regime_classifier = MarketRegimeClassifier()  # Thêm phân loại thị trường

    async def calibrate_grid(self, current_price: float):
        """Calibrate grid based on market regime"""
        # Điều chỉnh tham số dựa trên trạng thái thị trường
        if self.regime_classifier.volatility_state == "EXTREME":
            config.base_grid_step_percent *= 2.0
            config.max_grid_levels = max(3, config.max_grid_levels // 2)
            config.min_quantity *= 0.5
            logging.warning("Volatility EXTREME: Adjusting grid parameters")
            
        elif self.regime_classifier.volatility_state == "HIGH":
            config.base_grid_step_percent *= 1.5
            config.max_grid_levels = max(5, config.max_grid_levels - 2)
            config.min_quantity *= 0.7
            logging.info("Volatility HIGH: Adjusting grid parameters")
            
        # Gọi phương thức tính toán grid thích ứng
        await self.calculate_adaptive_grid(current_price, self.exchange)  # Sửa: Dùng self.exchange
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        order_book = await safe_api_call(self.exchange.get_order_book, alert_on_failure=True)
    ...
    async def place_static_orders(self, current_price: float, exchange: 'IExchange') -> None:
        ...
        async with order_lock:
            ...
            # Lấy thông tin filters và kiểm tra kỹ lượng
            ...
            for i, (buy_price, sell_price) in enumerate(static_grid, 1):
                ...
                if profit_percent < config.min_profit_percent:
                    logging.info(f"Mức lưới {i} không đạt lợi nhuận tối thiểu: {profit_percent:.2f}% < {config.min_profit_percent}%")
                    continue
                ...
                if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                    try:
                        ...
                        if buy_order and isinstance(buy_order, dict) and 'orderId' in buy_order:
                            ...
                            logging.info(f"(Static) Đã đặt lệnh MUA tại {buy_price_rounded:.6f}, SL: {quantity:.6f}")
                        else:
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                ...
                if buy_price < current_price:
                    ...
                    if not self.duplicate_order_exists("BUY", buy_price, open_orders):
                        ...
                        if buy_order and isinstance(buy_order, dict) and 'orderId' in buy_order:
                            self.order_ids.append(buy_order['orderId'])
                            open_count += 1
                            logging.info(f"(Dynamic) Đã đặt lệnh MUA tại {buy_price_rounded:.6f}, SL: {quantity:.6f}")
                        else:
                ...
                if sell_price > current_price:
                    ...
                    if sell_order and isinstance(sell_order, dict) and 'orderId' in sell_order:
                        ...
                        open_count += 1
                        logging.info(f"(Dynamic) Đã đặt lệnh BÁN tại {sell_price_rounded:.6f}, SL: {quantity:.6f}")
                    else:
            ...
            if not has_buy_order and len([o for o in open_orders if isinstance(o, dict) and o['side'].upper() == 'BUY']) < 2 and current_time - self.last_fallback_time > self.FALLBACK_COOLDOWN:
                ...
                if (buy_price_rounded > 0 and 
                    ...
                    if buy_order and isinstance(buy_order, dict) and 'orderId' in buy_order:
                        ...
                        logging.info(f"(Fallback) Đã đặt lệnh MUA tại {buy_price_rounded:.6f}, SL: {quantity:.6f}")
                    else:
...
class BotState:
    def __init__(self, exchange: 'IExchange', strategy: IGridStrategy, symbol: str):
        ...
        # Sửa: Truyền exchange vào Grid Manager
        self.grid_manager = EnhancedSmartGridManager(exchange)
        self.order_manager = EnhancedOrderManager(self)
        ...
        self.ml_features = []
        self.ml_labels = []
        self.exposure_monitor = ExposureMonitor(self)
        self.pre_trade_gateway = PreTradeGateway(self)
    ...
    async def process_events(self) -> None:
        try:
            ...
            # Cập nhật phân loại thị trường
            if config.volatility_regime_enabled:
                await self.grid_manager.regime_classifier.update(self)
                await self.grid_manager.calibrate_grid(self.current_price)
            ...
            # REALIZED PNL TRACKING - Cốt lõi của risk management
            if config.realized_pnl_tracking:
                await self.track_realized_pnl()
                
            # Cập nhật exposure monitor
            await self.exposure_monitor.update(self.realized_pnl)
            ...
            # Generate and process trade signals
            data = MarketData(
            ...
            signals = await self.strategy.generate_signals(data)
            async with order_lock:
                for signal in signals:
                    order = await self.order_manager.place_order(signal.side, "LIMIT", signal.quantity, signal.price)
    ...
    async def track_realized_pnl(self):
        """Theo dõi REALIZED PNL chính xác"""
        # Tính tổng lợi nhuận từ các lệnh đã hoàn thành (filled)
        new_pnl = 0.0
        for order in self.order_manager.completed_orders:
            if 'profit' in order:  # Giả sử mỗi order hoàn thành có ghi 'profit'
                new_pnl += order['profit']
        
        # Cập nhật nếu có thay đổi
        if abs(new_pnl - self.realized_pnl) > 0.001:
            self.realized_pnl = new_pnl
            logging.info(f"Realized PNL updated: {self.realized_pnl:.2f} USDT")
            ...
            if highest_price >= target_price:
                ...
                if self.current_price <= stop_price and self.current_price >= entry_price:
                    ...
                    if self.exchange.simulated_balance[base_asset] < quantity:
                        ...
                        logging.info(f"Chốt lời: Bán {quantity:.6f} {base_asset} @ {sell_price:.6f}, lợi nhuận {profit:.2f}")
                        await discord_notifier.send_alert(f"Chốt lời: Bán {quantity:.6f} {base_asset} @ {sell_price:.6f}, lợi nhuận {profit:.2f}")
    ...
    async def _get_current_capital(self) -> float:
        """
        Trả về tổng vốn hiện tại: USDT + coin_balance * giá hiện tại
        """
        # Lấy số dư USDT và coin (ví dụ tuple (usdt, coin))
        usdt_balance, coin_balance = await self.exchange.get_balance()
        # Lấy giá hiện tại của symbol, giả sử trả về float
        price = await self.exchange.get_price()
        return usdt_balance + coin_balance * price

    async def send_hourly_report(self):
        now = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
        profit = self.tracker.total_profit

        # Lấy danh sách lệnh mở và phân loại
        raw_open = await self.exchange.get_open_orders()
        open_orders = [o for o in raw_open if isinstance(o, dict) and o.get("status") == "OPEN"]
        open_buy_count = len([o for o in open_orders if o.get('side') == 'BUY'])
        open_sell_count = len([o for o in open_orders if o.get('side') == 'SELL'])

        capital = await self._get_current_capital()

        # Định dạng giá dựa trên symbol
        ...
        embed = {
            ...
            "fields": [
                ...
                {"name": "! |📈| Giá hiện tại", "value": format_price(self.current_price, self.symbol), "inline": True},
                {"name": "! |💳/💸| Lời/Lỗ", "value": f"{profit:.2f} USDT", "inline": True},
                {"name": "! |📊| Lệnh MUA mở", "value": str(open_buy_count), "inline": True},
                {"name": "! |📊| Lệnh BÁN mở", "value": str(open_sell_count), "inline": True},
                {"name": "! |⭕️| Tổng lệnh khớp", "value": str(self.tracker.trade_count), "inline": True},
                {"name": "! |⏰️| Thời gian", "value": now, "inline": False},
            ],
            "footer": {"text": "Bot by KuboNagisa | Auto Notify"}
        }
...
# ======== Paper Trading Exchange (đã nâng cấp) ========
class PaperTradingExchange(IExchange):
    def __init__(self, symbol: str):
        ...
        self.fill_simulator = RealisticFillSimulator(self)  # Thêm mô phỏng khớp lệnh
        self.regime_classifier = MarketRegimeClassifier()  # Phân loại thị trường
    ...
    async def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        ...
        if notional < min_notional:
            logging.warning(f"Notional {notional:.6f} < minNotional {min_notional:.6f}. Lệnh bị bỏ qua.")
            return {}
        ...
        # Sử dụng fill simulator cho paper trading
        if config.paper_trading_enabled and config.micro_slippage_enabled:
            order = await self.fill_simulator.simulate_fill(order)
        ...
        for order in self.paper_orders:
            ...
            if order["side"] == "BUY" and current_price <= order_price:
                ...
                if self.simulated_balance["USDT"] >= cost:
                    ...
                    logging.info(f"Khớp lệnh MUA tại {order_price:.6f}, SL: {order_qty:.6f}, Balance: {self.simulated_balance}")
                    await discord_notifier.send_alert(f"Khớp lệnh MUA tại {order_price:.6f}, SL: {order_qty:.6f}")
                else:
                ...
                if self.simulated_balance[base_asset] >= order_qty:
                    ...
                    order["status"] = "FILLED"
                    order["executedQty"] = order_qty
                    logging.info(f"Khớp lệnh BÁN tại {order_price:.6f}, SL: {order_qty:.6f}, Balance: {self.simulated_balance}")
                    await discord_notifier.send_alert(f"Khớp lệnh BÁN tại {order_price:.6f}, SL: {order_qty:.6f}")
                else:
...
# ======== New Modules ========
# 1. Live Fill Monitor (WebSocket Order Updates)
class LiveFillMonitor:
    def __init__(self, state: BotState):
        self.state = state
        self.ws_url = "wss://wbs.mexc.com/ws"  # WS spot endpoint MEXC mới
    ...
    async def start(self):
        """Kết nối WebSocket MEXC và đăng ký cập nhật lệnh"""
        while True:
            try:
                async with websockets.connect(self.ws_url, ping_interval=15) as ws:
                    self.ws_connected = True
                    logging.info("WebSocket connected to MEXC")
                    ...
                    if self.api_key and self.api_secret:
                        ...
                        auth_payload = {
                            "method": "login",
                            "params": [self.api_key, expires, signature],
                            "id": 1
                        }
                        await ws.send(json.dumps(auth_payload))
                    
                    # Đăng ký cập nhật lệnh cá nhân
                    subscribe_payload = {
                        "method": "sub.personal.order",
                        "params": [self.state.symbol],
                        "id": 2
                    }
                    await ws.send(json.dumps(subscribe_payload))
                    logging.info("Subscribed to personal order stream")
    ...
    async def _process_message(self, message: str):
        ...
        try:
            ...
            # Kiểm tra xem có phải cập nhật lệnh không
            if 'd' in data and 'o' in data['d']:
                order_data = data['d']['o']
                order_id = order_data.get('i')
                status = order_data.get('s')
                executed_qty = float(order_data.get('z', 0))
                avg_price = float(order_data.get('p', 0))
                
                # Xử lý các trạng thái lệnh khác nhau
                if status == "FILLED":
                    await self._handle_filled_order(order_id, executed_qty, avg_price)
                elif status == "PARTIALLY_FILLED":
                    await self._handle_partial_fill(order_id, executed_qty)
                elif status == "CANCELED":
                    await self._handle_canceled_order(order_id)
    ...
    async def _handle_filled_order(self, order_id: str, executed_qty: float, avg_price: float):
        """Xử lý lệnh đã khớp hoàn toàn"""
        # Tìm lệnh trong hệ thống
        order = next((o for o in self.state.order_manager.open_orders if o.get('orderId') == order_id), None)
        if not order:
            return
        ...
        # Ghi nhận giao dịch
        side = order['side'].upper()
        profit = (self.state.current_price - avg_price) * executed_qty if side == 'BUY' else (avg_price - self.state.current_price) * executed_qty
        fee = executed_qty * avg_price * (config.maker_fee if order.get('type') == 'LIMIT' else config.taker_fee)
        self.state.tracker.record_trade(profit, fee)
        
        # Gửi thông báo
        msg = f"Lệnh {side} đã khớp @ {avg_price:.6f}, SL: {executed_qty:.6f}"
        logging.info(f"[WS] {msg}")
        await discord_notifier.send_alert(msg)
        
    async def _handle_partial_fill(self, order_id: str, executed_qty: float):
        """Xử lý lệnh khớp một phần"""
        order = next((o for o in self.state.order_manager.open_orders if o.get('orderId') == order_id), None)
        if order:
            order['executedQty'] = executed_qty
            logging.info(f"[WS] Lệnh {order_id} khớp một phần: {executed_qty}")
            
    async def _handle_canceled_order(self, order_id: str):
        """Xử lý lệnh đã hủy"""
        order = next((o for o in self.state.order_manager.open_orders if o.get('orderId') == order_id), None)
        if order:
            order['status'] = 'CANCELED'
            logging.info(f"[WS] Lệnh {order_id} đã hủy")
            
    async def _handle_rejected_order(self, order_id: str):
        """Xử lý lệnh bị từ chối"""
        order = next((o for o in self.state.order_manager.open_orders if o.get('orderId') == order_id), None)
        if order:
            order['status'] = 'REJECTED'
            logging.warning(f"[WS] Lệnh {order_id} bị từ chối")
            await discord_notifier.send_alert(f"Lệnh bị từ chối: {order_id}", is_error=True)

# 2. Auto-Rebalance Grid
class GridRebalancer:
    def __init__(self, grid_manager: EnhancedSmartGridManager):
        self.grid_manager = grid_manager
        self.last_rebalance_time = 0
        
    async def rebalance_grid(self, current_price: float, exchange: 'IExchange'):
        """Tự động cân bằng grid dựa trên điều kiện thị trường"""
        current_time = time.time()
        if current_time - self.last_rebalance_time < config.grid_rebalance_interval:
            return
            
        # Tính khoảng cách từ giá hiện tại đến biên grid
        grid_low, grid_high = self.grid_manager.get_grid_range()
        if grid_low is None or grid_high is None:
            return
            
        distance_low = abs(current_price - grid_low) / current_price if current_price > 0 else 0
        distance_high = abs(grid_high - current_price) / current_price if current_price > 0 else 0
        
        # Kiểm tra nếu giá đã di chuyển quá xa grid
        if distance_low > 0.08 or distance_high > 0.08:
            logging.info("Rebalancing grid due to price drift")
            await self.grid_manager.cancel_all_orders(exchange)
            await self.grid_manager.calculate_adaptive_grid(current_price, exchange)
            await self.grid_manager.place_grid_orders(current_price, exchange)
...
class PartialLiquidationEngine:
    def __init__(self, state: BotState):
        ...
        self.liquidation_cooldown = 3600  # 1 hour
        
    async def check_and_liquidate(self):
        """Kiểm tra và thực hiện thanh lý một phần nếu cần"""
        current_time = time.time()
        if current_time - self.last_liquidation_time < self.liquidation_cooldown:
            return
            
        # Tính toán lợi nhuận thả nổi
        _, coin_balance = await safe_api_call(self.state.exchange.get_balance)
        current_value = coin_balance * self.state.current_price
        entry_value = sum([qty * price for (_, price), (_, qty) in self.state.grid_manager.trailing_stops.items()])
        
        if entry_value == 0 or current_value == 0:
            return
            
        floating_pnl = (current_value - entry_value) / entry_value * 100
        
        # Kiểm tra ngưỡng thanh lý
        if floating_pnl < -15:  # 15% lỗ thả nổi
            logging.warning(f"Partial liquidation triggered: {floating_pnl:.2f}% floating loss")
            await self._execute_partial_liquidation(coin_balance)
            self.last_liquidation_time = current_time
            
    async def _execute_partial_liquidation(self, coin_balance: float):
        """Bán một phần tài sản để giảm rủi ro"""
        # Bán 30% lượng coin nắm giữ
        liquidate_qty = coin_balance * 0.3
        
        if liquidate_qty < config.min_quantity:
            return
            
        # Đặt lệnh bán thị trường
        ...
        if order:
            logging.info(f"Partial liquidation executed: Sold {liquidate_qty} coins")
            await discord_notifier.send_alert(f"Partial liquidation: Sold {liquidate_qty} coins to reduce exposure")
            
            # Tái lập grid positions
            await self.state.grid_manager.calculate_adaptive_grid(self.state.current_price, self.state.exchange)
            await self.state.grid_manager.place_grid_orders(self.state.current_price, self.state.exchange)

# 4. Enhanced Order State Machine
class OrderStateManager:
    def __init__(self, state: BotState):
        self.state = state
        
    async def handle_order_update(self, order_id: str, status: str, filled_qty: float, remaining_qty: float):
        """Xử lý cập nhật trạng thái lệnh"""
        if status == "PARTIALLY_FILLED":
            await self._handle_partial_fill(order_id, filled_qty, remaining_qty)
        elif status == "REJECTED":
            await self._handle_rejected_order(order_id)
        elif status == "CANCELED":
            await self._handle_canceled_order(order_id)
            
    async def _handle_partial_fill(self, order_id: str, filled_qty: float, remaining_qty: float):
        """Xử lý lệnh khớp một phần"""
        # Tìm lệnh trong danh sách mở
        open_orders = await safe_api_call(self.state.exchange.get_open_orders)
        order = next((o for o in open_orders if o.get('orderId') == order_id), None)
        
        if not order:
            return
            
        # Hủy lệnh hiện tại và đặt lệnh mới với số lượng còn lại
        if order['side'] == 'BUY' and remaining_qty > 0:
            await safe_api_call(self.state.exchange.cancel_order, order_id)
            await safe_api_call(
                self.state.exchange.place_order,
                "BUY",
                "LIMIT",
            ...
            logging.info(f"Updated BUY order {order_id} to qty {remaining_qty}")
            
    async def _handle_rejected_order(self, order_id: str):
        """Xử lý lệnh bị từ chối"""
        # Xóa khỏi danh sách theo dõi
        if order_id in self.state.grid_manager.order_ids:
            self.state.grid_manager.order_ids.remove(order_id)
            
        # Thông báo
        await discord_notifier.send_alert(f"Order {order_id} rejected", is_error=True)
        
    async def _handle_canceled_order(self, order_id: str):
        """Xử lý lệnh đã hủy"""
        # Đặt lại lệnh nếu là lệnh static
        if order_id in self.state.grid_manager.static_order_ids:
            # Lấy thông tin lệnh
            all_orders = await safe_api_call(self.state.exchange.get_all_orders)
            order = next((o for o in all_orders if o.get('orderId') == order_id), None)
            
            if order and order['side'] == 'BUY':
                # Đặt lại lệnh static
                price = float(order['price'])
                quantity = float(order['origQty']) - float(order.get('executedQty', 0))
                
                if quantity > 0:
                    new_order = await safe_api_call(
                        self.state.exchange.place_order,
                        "BUY",
                        "LIMIT",
                        quantity,
                    ...
                    if new_order and 'orderId' in new_order:
                        self.state.grid_manager.static_order_ids.append(new_order['orderId'])
                        logging.info(f"Replaced static order {order_id} with {new_order['orderId']}")

# 5. API Rate Limit Guard
class RateLimiter:
    ...
    async def wait(self):
        """Chờ nếu vượt quá giới hạn rate limit"""
        now = time.time()
        
        # Xóa các dấu thời gian cũ
...
class NewsEventKillSwitch:
    ...
    async def monitor(self):
        """Giám sát tin tức và kích hoạt kill switch nếu cần"""
        if abs(self.news_analyzer.sentiment) > 0.8:  # Tâm lý cực đoan
            logging.critical("News Kill Switch activated due to extreme sentiment")
            self.triggered = True
    ...
    async def _activate_kill_switch(self):
        ...
        # Hủy tất cả lệnh
        # Ghi chú: Cần truy cập vào instance exchange
        # Triển khai phụ thuộc vào kiến trúc
        
        # Gửi cảnh báo
        await discord_notifier.send_alert("NEWS KILL SWITCH ACTIVATED! Bot shutdown", is_error=True)
...
async def run_live_loop():
    try:
        ...
        for symbol in symbols:
            ...
            # Tạo state trước
            state = BotState(exchange, None, symbol)
            
            # Tạo strategy dựa trên grid_manager của state
            if config.strategy_mode == 1:
                state.strategy = AdaptiveGridStrategy(state.grid_manager)
            else:
                state.strategy = DenseGridStrategy(state.grid_manager)
                
            states[symbol] = state

        # Load state and initialize
        await load_state(states)
        
        # Cancel all orders at startup
        for state in states.values():
            logging.info(f"Hủy tất cả lệnh mở trước khi khởi động bot cho {state.symbol}")
        ...
        tasks = []
        for state in states.values():
            tasks.append(state.exchange.run_paper_trading_live(state, interval=1))
            tasks.append(rebalance_loop(state))
            
            # Start WebSocket monitors
            if config.websocket_enabled:
                ws_monitor = LiveFillMonitor(state)
                tasks.append(ws_monitor.start())
                
            # Initialize additional modules
            partial_liquidator = PartialLiquidationEngine(state)
            order_state_manager = OrderStateManager(state)
            grid_rebalancer = GridRebalancer(state.grid_manager)
            
            # Add periodic checks
            tasks.append(partial_liquidator.check_and_liquidate())
            tasks.append(grid_rebalancer.rebalance_grid(state.current_price, state.exchange))
        ...
        await discord_notifier.send_alert(f"Bot crashed: {str(e)}", is_error=True)
...
async def run_backtest():
    ...
    for symbol, state in states.items():
        ...
        for idx, data in df.iterrows():
            ...
            signals = await state.strategy.generate_signals(data)
            async with order_lock:
                for signal in signals:
                    order = await state.order_manager.place_order(signal.side, "LIMIT", signal.quantity, signal.price)
                    if order and 'orderId' in order:
        ...
        plt.save(f'equity_curve_{symbol}.png')
