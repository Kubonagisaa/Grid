...
class AdvancedConfig:
    def __init__(self):
        ...
        self.max_grid_levels = int(os.getenv("MAX_GRID_LEVELS", 8))  # Giảm từ 12 xuống 8
        self.base_grid_step_percent = float(os.getenv("BASE_GRID_STEP_PERCENT", 0.2))  # Đảm bảo là 0.2
...
class SmartGridManager:
        ...
        try:
            ...
            if self.adaptive_enabled:
                ...
                if stats["Consecutive Sells"] >= profit_tracker.max_consecutive_trades:
                    ...
                    logging.info(f"Reduced grid spacing to {self.grid_spacing:.2%} due to too many consecutive SELL trades")
                elif stats["Consecutive Buys"] >= profit_tracker.max_consecutive_trades:
                    self.grid_spacing = max(self.grid_spacing * 0.8, config.base_grid_step_percent / 100)
                    logging.info(f"Reduced grid spacing to {self.grid_spacing:.2%} due to too many consecutive BUY trades")
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        # Kiểm tra số lượng lệnh mở trước khi đặt lệnh mới
        ...
        # Hủy các lệnh quá xa giá hiện tại
        ...
        # Đặt lệnh BUY khẩn cấp nếu không có lệnh BUY nào dưới giá hiện tại
