...
logging.basicConfig(
    level=logging.DEBUG,  # Thay đổi level thành DEBUG để có thêm log chi tiết
...
def send_telegram_alert(message: str) -> None:
    ...
    if not telegram_limiter.can_send():
        ...
        return
    try:
        if "error" in message.lower() and "API call failed" in message:
            logging.warning("Skipping Telegram alert with error content")
            return
...
class SmartGridManager:
    ...
    async def update_grid(self, current_price: float, exchange: 'ExchangeInterface', drawdown: float, profit_tracker: 'EnhancedProfitTracker') -> None:
        try:
            ...
            logging.debug(f"Should update grid: {should_update}, trailing_enabled={self.trailing_enabled}, "
                          f"last_price={self.last_price}, current_price={current_price}, "
            ...
            if self.last_price == 0.0 or should_update or force_update:
                ...
                logging.info(f"Updating grid with {len(new_levels)} levels")
                ...
                logging.debug("Grid update skipped due to conditions not met")
    ...
    async def cancel_order(self, order_id: str) -> None:
        try:
            ...
            if "Unknown order sent" in str(e):
                logging.warning(f"Order {order_id} not found, possibly already canceled: {str(e)}")
            else:
                logging.error(f"Failed to cancel order {order_id}: {str(e)}")
    ...
    async def close(self) -> None:
        if self.client:
            try:
                ...
                logging.error(f"Failed to close exchange connection: {str(e)}")
            finally:
                self.client = None
...
async def safe_api_call(func, allow_empty: bool = False, **kwargs):
    try:
        ...
        if func.__name__ == "get_price":
            return 0.0
        elif func.__name__ == "get_volume":
            return 0.0
        elif func.__name__ == "get_high_low":
            return (0.0, 0.0)
        elif func.__name__ == "get_open_orders" or func.__name__ == "get_all_orders":
            return [] if allow_empty else None
        elif func.__name__ == "get_balance":
            return (0.0, 0.0)
        elif func.__name__ == "get_symbol_filters":
            return {'minQty': 0.0001, 'maxQty': 1000.0, 'stepSize': 0.0001}
        return None
