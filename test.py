...
class AdvancedConfig:
    def __init__(self):
        ...
        self.trailing_threshold: float = float(os.getenv("TRAILING_THRESHOLD", 0.005))  # Ngưỡng để kích hoạt trailing (0.5%)
        self.min_price_distance_percent: float = float(os.getenv("MIN_PRICE_DISTANCE_PERCENT", 0.001))  # Khoảng cách tối thiểu 0.1%
...
class EnhancedSmartGridManager:
    def __init__(self):
        ...
        self.high_history: deque = deque(maxlen=config.volatility_window)
        self.low_history: deque = deque(maxlen=config.volatility_window)
        self.reference_price: float = 0.0  # Giá tham chiếu để trailing
        self.sell_match_count: Dict[float, int] = {}  # Theo dõi số lần khớp lệnh SELL tại cùng một mức giá
    ...
    async def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        ...
        # Khởi tạo giá tham chiếu nếu chưa có
        if self.reference_price == 0.0:
            self.reference_price = current_price

        # Kiểm tra xem giá có thay đổi đủ lớn để kích hoạt trailing không
        price_change_percent = abs(current_price - self.reference_price) / self.reference_price
        if price_change_percent < config.trailing_threshold and self.grid_levels:
            # Nếu giá thay đổi nhỏ hơn ngưỡng, giữ nguyên lưới hiện tại
            return

        # Cập nhật giá tham chiếu
        self.reference_price = current_price

        # Tính toán lưới mới dựa trên giá hiện tại
        ...
        logging.info(f"Đã tính toán lưới thích ứng: {num_levels} mức, bước {step_percent:.2%}, giá tham chiếu: {self.reference_price:.2f}")
        grid_levels_str = "\n".join([f"  Mức {i+1}: Mua tại {buy:.2f}, Bán tại {sell:.2f}" for i, (buy, sell) in enumerate(self.grid_levels)])
        logging.info(f"Các mức lưới:\n{grid_levels_str}")
        send_telegram_alert(f"🔄 Cập nhật lưới:\n{grid_levels_str}")
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        for buy_price, sell_price in self.grid_levels:
            try:
                # Đảm bảo vùng mua/bán không quá gần giá hiện tại
                if buy_price >= current_price * (1 - config.min_price_distance_percent):
                    logging.info(f"Giá mua {buy_price:.2f} quá gần giá hiện tại {current_price:.2f}, bỏ qua")
                    continue
                if sell_price <= current_price * (1 + config.min_price_distance_percent):
                    logging.info(f"Giá bán {sell_price:.2f} quá gần giá hiện tại {current_price:.2f}, bỏ qua")
                    continue
                ...
                if sell_price > current_price:
                    # Kiểm tra số lần khớp lệnh SELL tại mức giá này
                    if sell_price in self.sell_match_count and self.sell_match_count[sell_price] >= 3:
                        logging.info(f"Đã khớp lệnh SELL tại {sell_price:.2f} quá nhiều lần (3 lần), tạm dừng đặt lệnh")
                        continue
        ...
        try:
            ...
            for order in orders:
                if order['status'] == 'FILLED':
                    ...
                    # Ghi nhận số lần khớp lệnh SELL
                    if side == 'SELL':
                        grid_manager.sell_match_count[price] = grid_manager.sell_match_count.get(price, 0) + 1
                    ...
                    msg = (f"✅ Lệnh {side} đã khớp\n"
                           f"▪ Giá: {price:.2f}\n"
                           f"▪ Giá thị trường hiện tại: {current_price:.2f}\n"
                           f"▪ Số lượng: {qty:.6f}\n"
...
async def rebalance_grid_if_needed(state: BotState, exchange: ExchangeInterface, current_price: float) -> None:
    # Luôn kiểm tra và cập nhật lưới nếu giá thay đổi đáng kể
