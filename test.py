    ...
    async def initialize(self):
        for exchange in self.exchanges.values():
            try:
                ...
                logging.error(f"Lỗi khi kiểm tra sàn {exchange.exchange_name}: {str(e)}")
...
async def run_bots():
    while True:
        ...
        if int(time.time()) % 60 == 0:
            stats_summary = "\n".join(
                f"{name}: Profit {state.tracker.get_stats()['Total Profit']:.2f} USDT, Trades {state.tracker.get_stats()['Trade Count']}"
                for name, state in bot_states.items()
            )
...
# ======== FIX: XỬ LÝ TÍN HIỆU (SIGNAL HANDLER) ========
def signal_handler(sig, frame):
    logging.info("Đã nhận tín hiệu dừng, đang tắt bot.")
    try:
        loop = asyncio.get_event_loop()
    except Exception as e:
        logging.error(f"Lỗi khi lấy event loop: {str(e)}")
        loop = None
    if loop:
        for state in bot_states.values():
            # Chỉ fix lỗi: thay vì await trực tiếp, ta sử dụng asyncio.run_coroutine_threadsafe
            asyncio.run_coroutine_threadsafe(state.order_manager.cancel_all_orders(state.exchange), loop)
            asyncio.run_coroutine_threadsafe(state.exchange.close(), loop)
    save_state(bot_states)
    sys.exit(0)
