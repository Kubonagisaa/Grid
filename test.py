...
class EnhancedSmartGridManager:
    ...
    async def place_static_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        ...
        # Khôi phục kiểm tra RSI từ mã gốc
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        async with order_lock:
            ...
            if num_levels == 0:
                ...
                return
            # Reinvest profits: use current balance instead of initial investment (từ mã mới)
            ...
            for i, (buy_price, sell_price) in enumerate(self.grid_levels[self.static_levels:], start=self.static_levels):
                # Update current balances each iteration
                usdt_balance, coin_balance = await safe_api_call(exchange.get_balance)
                # Check minimum profit for this level (từ mã mới)
                profit_percent = (sell_price - buy_price) / buy_price * 100 if buy_price > 0 else 0
                if profit_percent < config.min_profit_percent:
                    continue
                try:
                    ...
                    if buy_price < current_price:
                        ...
                        if current_time < self.grid_cooldown.get(price_key, 0):
                            continue
                        # Capital check before placing buy order
                        if usdt_balance < buy_price * quantity:
...
# ======== TRẠIING STOP VÀ CƠ CHẾ TỰ ĐỘNG ========
@dataclass
class TradeSignal:
    side: str
    price: float
...
class AdaptiveGridStrategy(GridStrategy):
    async def generate_signals(self, data: MarketData) -> List[TradeSignal]:
        signals = []
        # Khôi phục tín hiệu RSI từ mã gốc
        ...
        # Giữ logic vượt phạm vi lưới từ mã mới
...
class DenseGridStrategy(GridStrategy):
    async def generate_signals(self, data: MarketData) -> List[TradeSignal]:
        signals = []
        # Simple example: No additional signals in dense mode
        return signals
...
class BotState:
    ...
    async def process_events(self) -> None:
        ...
        current_time = time.time()
        alert_messages = []  # Collect messages to send as a single alert
        ...
        if current_time - self.last_price_update >= 5:
            ...
            if not await check_internet_connection():
                ...
                alert_messages.append("Không có kết nối Internet, bot tạm dừng lấy dữ liệu mới")
                logging.warning(f"Sử dụng giá trị gần nhất để tiếp tục: "
                               f"price={self.current_price}, volume={self.current_volume}, "
                               f"high={self.current_high}, low={self.current_low}")
            else:
                new_price = None
                new_volume = None
                new_high_low = None
                for attempt in range(config.api_retry_count):
                    try:
                        async with asyncio.timeout(20):  # Tăng timeout lên 20 giây
        ...
        if await check_protections(self, self.exchange, self.current_price, self.current_volume, self.current_high, self.current_low):
            if self.protection.check_pump_protection(self.current_price):
                alert_messages.append("Pump protection: biến động giá quá lớn, tạm dừng giao dịch")
            if self.protection.check_abnormal_activity(self.current_volume):
                alert_messages.append("Khối lượng giao dịch bất thường, tạm dừng giao dịch")
            if alert_messages:
                await send_telegram_alert("\n".join(alert_messages))
            return

        # Kiểm tra sức khỏe sàn trước khi giao dịch
        health = await self.exchange.health_check()
        if health['status'] != 'healthy':
            logging.warning(f"Sàn không ổn định: {health['status']}")
            alert_messages.append(f"Sàn không ổn định: {health['status']}")
            if alert_messages:
                await send_telegram_alert("\n".join(alert_messages))
            return
        ...
        # Gửi báo cáo Telegram theo tần suất tùy biến nếu cần
...
TELEGRAM_RATE_LIMIT_SECONDS = 5  # Increased from 2 to 5 seconds to avoid rate limiting
last_update_id = 0
...
class PaperTradingExchange(ExchangeInterface):
    def __init__(self, symbol: str):
        ...
        self.paper_orders = []
        self.current_market_price = 100.0  # Fallback price
        self.current_volume = 0.0
        self.current_high = 101.0  # Fallback high
        self.current_low = 99.0    # Fallback low
    ...
    def _validate_symbol(self):
        ...
        try:
            ...
            if symbol_str not in markets:
                raise ValueError(f"Symbol {symbol_str} is not supported on MEXC")
        except Exception as e:
            logging.error(f"Symbol validation failed: {str(e)}")
            raise
        finally:
            mexc.close()
    ...
    async def get_price(self) -> float:
        ...
        try:
            ...
            return self.current_market_price  # Return last known price
            ...
            # Simulate a small price movement as a fallback
            if self.current_market_price > 0:
                simulated_price = self.current_market_price * (1 + np.random.uniform(-0.001, 0.001))
                self.current_market_price = simulated_price
                self.current_high = simulated_price * 1.01
                self.current_low = simulated_price * 0.99
                self.current_volume = max(self.current_volume, 100.0)  # Ensure volume isn't 0
                return simulated_price
            else:
                # If no valid price exists, use a default
    ...
    async def get_volume(self) -> float:
        await self.get_price()  # Ensure data is updated
    ...
    async def fetch_historical_data(self, timeframe: str, limit: int) -> pd.DataFrame:
        try:
            binance = ccxt_async.binance({'enableRateLimit': True})  # Fixed: Changed ccxt to ccxt_async
