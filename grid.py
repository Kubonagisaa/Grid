...
class EnhancedSmartGridManager:
        ...
        for buy_price, sell_price in self.grid_levels:
            ...
            try:
                if buy_price < current_price:
                    buy_order = safe_api_call(exchange.place_order, side='BUY', order_type='LIMIT', quantity=quantity, price=buy_price)
                    self.order_ids.append(buy_order['id'])
                    logging.info(f"Placed BUY order at {buy_price:.2f}, qty: {quantity}")
                if sell_price > current_price:
                    sell_order = safe_api_call(exchange.place_order, side='SELL', order_type='LIMIT', quantity=quantity, price=sell_price)
    ...
    def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        for order_id in self.order_ids:
            try:
                safe_api_call(exchange.cancel_order, order_id)
            except Exception as e:
...
class EnhancedOrderManager:
    ...
    def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface', profit_tracker: 'EnhancedProfitTracker') -> None:
        try:
            orders = safe_api_call(exchange.get_all_orders)
            for order in orders:
    ...
    async def fetch_historical_data(self, timeframe: str, limit: int) -> pd.DataFrame:
        try:
            ...
            if not ohlcv or len(ohlcv) == 0:
                logging.warning(f"No historical data returned for {self.symbol}")
...
async def handle_orders(state: BotState, exchange: ExchangeInterface, current_price: float) -> None:
    state.order_manager.check_and_handle_orders(current_price, exchange, state.tracker)
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    ...
    if config.websocket_enabled:
        websocket_task = asyncio.create_task(exchange.start_websocket(state))
...
async def main_async():
    ...
    for exchange_name, exchange in exchange_manager.exchanges.items():
        backtest_result = await run_simulated_backtest(exchange)
        logging.info(f"Simulated backtest on {exchange_name}: {backtest_result}")
        send_telegram_alert(
            f"📊 Simulated backtest on {exchange_name}:\n"
            f"Total Profit: {backtest_result['Total Profit']:.2f}\n"
            f"Trades: {backtest_result['Trade Count']}\n"
            f"Win Rate: {backtest_result['Win Rate']:.2f}%\n"
            f"Max Drawdown: {backtest_result['Max Drawdown']:.2f}%\n"
            f"Sharpe Ratio: {backtest_result['Sharpe Ratio']:.2f}\n"
            f"Total Fees: {backtest_result['Total Fees']:.2f}"
        )
        
        historical_data = await safe_api_call(exchange.fetch_historical_data, timeframe='1h', limit=100)
        real_backtest_result = await backtest_strategy(exchange, historical_data)
        logging.info(f"Real backtest on {exchange_name}: {real_backtest_result}")
        send_telegram_alert(
            f"📈 Real backtest on {exchange_name}:\n"
            f"Total Profit: {real_backtest_result['Total Profit']:.2f}\n"
            f"Trades: {real_backtest_result['Trade Count']}\n"
            f"Win Rate: {real_backtest_result['Win Rate']:.2f}%\n"
            f"Max Drawdown: {real_backtest_result['Max Drawdown']:.2f}%\n"
            f"Sharpe Ratio: {real_backtest_result['Sharpe Ratio']:.2f}\n"
            f"Total Fees: {real_backtest_result['Total Fees']:.2f}"
        )
