...
class AdvancedConfig:
    def __init__(self):
        ...
        # Lấy trực tiếp API key và API secret từ .env, không mã hóa
...
class EnhancedProfitTracker:
    ...
    def record_trade(self, profit: float, fee: float) -> None:
        """
        Ghi lại thông tin giao dịch, cập nhật các thống kê.

        Args:
            profit (float): Lợi nhuận từ giao dịch.
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        Returns:
            Dict[str, float]: Thống kê bao gồm tổng lợi nhuận, số giao dịch, tỷ lệ thắng, v.v.
        """
...
class EnhancedProtectionSystem:
    ...
    def update(self, price: float, high: float, low: float, volume: float) -> None:
        """
        Cập nhật hệ thống bảo vệ với dữ liệu thị trường mới.
    ...
    def check_pump_protection(self, price: float) -> bool:
        """
        Kiểm tra nếu có điều kiện bảo vệ tăng giá đột biến.

        Args:
            price (float): Giá hiện tại.

        Returns:
            bool: True nếu bảo vệ được kích hoạt, False nếu không.
        """
    ...
    def check_circuit_breaker(self, price: float) -> bool:
        """
        Kiểm tra nếu có điều kiện ngắt mạch.
    ...
    def check_circuit_breaker_status(self) -> bool:
        """
        Kiểm tra trạng thái của ngắt mạch.

        Returns:
            bool: True nếu ngắt mạch đang hoạt động, False nếu không.
        """
    ...
    def check_abnormal_activity(self, volume: float) -> bool:
        """
        Kiểm tra hoạt động bất thường dựa trên khối lượng giao dịch.

        Args:
            volume (float): Khối lượng giao dịch hiện tại.

        Returns:
            bool: True nếu phát hiện hoạt động bất thường, False nếu không.
        """
    ...
    def update_trailing_stop(self, price: float) -> Tuple[bool, float]:
        """
        Cập nhật giá trailing stop và kiểm tra nếu điều kiện dừng được kích hoạt.

        Args:
            price (float): Giá hiện tại.

        Returns:
            Tuple[bool, float]: (Điều kiện dừng có được kích hoạt không, giá trailing stop mới).
        """
    ...
    def update_trailing_buy(self, price: float) -> Tuple[bool, float]:
        """
        Cập nhật giá trailing buy và kiểm tra nếu điều kiện mua được kích hoạt.

        Args:
            price (float): Giá hiện tại.

        Returns:
            Tuple[bool, float]: (Điều kiện mua có được kích hoạt không, giá trailing buy mới).
        """
...
class EnhancedSmartGridManager:
    ...
    def calculate_adaptive_grid(self, current_price: float, exchange: Optional['ExchangeInterface'] = None) -> None:
        """
        Tính toán lưới giao dịch thích ứng dựa trên độ biến động thị trường.

        Args:
            current_price (float): Giá thị trường hiện tại.
            exchange (Optional[ExchangeInterface]): Giao diện sàn giao dịch, nếu cần.
        """
    ...
    def _create_static_grid(self, current_price: float) -> List[Tuple[float, float]]:
        """
        Tạo lưới tĩnh với các mức cố định.

        Args:
            current_price (float): Giá thị trường hiện tại.

        Returns:
            List[Tuple[float, float]]: Danh sách các cặp (giá mua, giá bán).
        """
        grid = []
    ...
    async def place_grid_orders(self, current_price: float, exchange: 'ExchangeInterface') -> None:
        """
        Đặt lệnh lưới dựa trên các mức lưới hiện tại.

        Args:
            current_price (float): Giá thị trường hiện tại.
            exchange (ExchangeInterface): Giao diện sàn giao dịch để đặt lệnh.
        """
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        """
        Hủy tất cả lệnh lưới đang mở.

        Args:
            exchange (ExchangeInterface): Giao diện sàn giao dịch để hủy lệnh.
        """
...
class EnhancedOrderManager:
    ...
    async def cancel_all_orders(self, exchange: 'ExchangeInterface') -> None:
        """
        Hủy tất cả lệnh mở trên sàn giao dịch.
    ...
    async def check_and_handle_orders(self, current_price: float, exchange: 'ExchangeInterface', profit_tracker: 'EnhancedProfitTracker') -> None:
        """
        Kiểm tra và xử lý các lệnh đã được khớp, ghi lại lợi nhuận và phí.

        Args:
            current_price (float): Giá thị trường hiện tại.
            exchange (ExchangeInterface): Giao diện sàn giao dịch để kiểm tra lệnh.
            profit_tracker (EnhancedProfitTracker): Trình theo dõi lợi nhuận để ghi lại giao dịch.
        """
        try:
...
class GridStrategy(ABC):
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        """
        Tạo tín hiệu giao dịch dựa trên dữ liệu thị trường.

        Args:
            data (MarketData): Dữ liệu thị trường hiện tại.

        Returns:
            List[Signal]: Danh sách các tín hiệu giao dịch.
        """
        pass
...
class AdaptiveGridStrategy(GridStrategy):
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        """
        Tạo tín hiệu giao dịch bằng chiến lược lưới thích ứng.

        Args:
            data (MarketData): Dữ liệu thị trường hiện tại.

        Returns:
            List[Signal]: Danh sách tín hiệu mua/bán.
        """
...
class BotState:
    ...
    async def process_events(self) -> None:
        """
        Xử lý các sự kiện từ hàng đợi sự kiện, tạo và thực thi tín hiệu.
        """
        while True:
...
class ExchangeInterface(ABC):
    def __init__(self, exchange_name: str, api_key: str, api_secret: str):
        ...
        self.client: Any = None  # Sẽ được thiết lập bởi các lớp con
    ...
    async def health_check(self) -> Dict[str, Any]:
        """
        Kiểm tra sức khỏe kết nối với sàn giao dịch.

        Returns:
            Dict[str, Any]: Trạng thái sức khỏe bao gồm kết nối WebSocket và độ trễ API.
        """
        try:
            start_time = time.time()
            await self.get_price()
            latency = (time.time() - start_time) * 1000  # Độ trễ tính bằng mili giây
            return {
...
class BaseExchange(ExchangeInterface):
    ...
    async def get_volume(self) -> float:
        """
        Lấy khối lượng giao dịch hiện tại của cặp giao dịch.

        Returns:
            float: Khối lượng hiện tại.
        """
        try:
            ticker = await self.client.fetch_ticker(self.symbol)
    ...
    async def get_high_low(self) -> Tuple[float, float]:
        """
        Lấy giá cao nhất và thấp nhất của cặp giao dịch.

        Returns:
            Tuple[float, float]: Giá cao và thấp.
        """
        try:
            ticker = await self.client.fetch_ticker(self.symbol)
...
class ExchangeManager:
    ...
    async def check_price_disparity(self) -> None:
        """
        Kiểm tra chênh lệch giá giữa các sàn giao dịch.
        """
        prices = {}
...
async def safe_api_call(func, *args, max_retries: int = 3, **kwargs) -> Any:
    """
    Gọi API một cách an toàn với cơ chế thử lại.

    Args:
        func: Hàm API cần gọi.
        max_retries (int): Số lần thử lại tối đa.
        *args, **kwargs: Các tham số truyền vào hàm.

    Returns:
        Any: Kết quả của cuộc gọi API.
    """
...
class RSICalculator:
    ...
    def update(self, price: float, prev_price: float) -> float:
        """
        Cập nhật giá trị RSI với giá mới.

        Args:
            price (float): Giá hiện tại.
            prev_price (float): Giá trước đó.

        Returns:
            float: Giá trị RSI đã cập nhật.
        """
...
def calculate_rsi(prices: List[float], period: int) -> float:
    """
    Tính chỉ số RSI (Relative Strength Index) cho một chuỗi giá.

    Args:
        prices (List[float]): Danh sách giá.
...
def calculate_atr(highs: List[float], lows: List[float], closes: List[float], period: int) -> float:
    """
    Tính toán ATR (Average True Range) cho một chuỗi giá.

    Args:
        highs (List[float]): Danh sách giá cao nhất.
        lows (List[float]): Danh sách giá thấp nhất.
        closes (List[float]): Danh sách giá đóng cửa.
...
def calculate_bollinger_bands(prices: List[float], period: int, std_dev: float) -> Tuple[float, float, float]:
    """
    Tính toán Bollinger Bands cho một chuỗi giá.

    Args:
        prices (List[float]): Danh sách giá.
        period (int): Chu kỳ tính toán.
        std_dev (float): Hệ số độ lệch chuẩn.

    Returns:
        Tuple[float, float, float]: Dải giữa, dải trên, và dải dưới.
    """
...
def calculate_sharpe_ratio(returns: List[float], risk_free_rate: float = 0.02) -> float:
    """
    Tính toán tỷ lệ Sharpe hàng năm cho một chuỗi lợi nhuận.

    Args:
        returns (List[float]): Danh sách lợi nhuận hàng ngày.
        risk_free_rate (float, optional): Tỷ lệ không rủi ro, mặc định là 0.02 (2%).

    Returns:
        float: Tỷ lệ Sharpe hàng năm.
    """
    if len(returns) < 2:
        return 0.0
...
def calculate_max_drawdown(equity_curve: List[float]) -> float:
    """
    Tính toán mức sụt giảm tối đa từ đường cong vốn.

    Args:
        equity_curve (List[float]): Danh sách giá trị vốn theo thời gian.

    Returns:
        float: Mức sụt giảm tối đa (tính bằng %).
    """
...
async def run_simulated_backtest(exchange: ExchangeInterface) -> Dict[str, float]:
    """
    Chạy kiểm tra mô phỏng trên dữ liệu lịch sử.

    Args:
        exchange (ExchangeInterface): Giao diện sàn giao dịch để lấy dữ liệu.

    Returns:
        Dict[str, float]: Thống kê kiểm tra.
    """
...
async def backtest_strategy(exchange: ExchangeInterface, historical_data: pd.DataFrame) -> Dict[str, float]:
    """
    Chạy kiểm tra thực tế trên dữ liệu lịch sử.

    Args:
        exchange (ExchangeInterface): Giao diện sàn giao dịch.
        historical_data (pd.DataFrame): Dữ liệu thị trường lịch sử.
...
async def check_account_balance(exchange: ExchangeInterface) -> Tuple[float, float]:
    """
    Kiểm tra số dư tài khoản trên sàn giao dịch.

    Args:
        exchange (ExchangeInterface): Giao diện sàn giao dịch.

    Returns:
        Tuple[float, float]: Số dư USDT và BTC.
    """
    try:
...
async def handle_exchange_downtime(state: BotState, downtime_start: Optional[float]) -> Tuple[bool, Optional[float]]:
    """
    Xử lý khi sàn giao dịch bị gián đoạn bằng cách chuyển sang chế độ an toàn.

    Args:
        state (BotState): Trạng thái bot.
        downtime_start (Optional[float]): Thời điểm bắt đầu gián đoạn.

    Returns:
        Tuple[bool, Optional[float]]: (Có ở chế độ an toàn không, thời điểm bắt đầu gián đoạn mới).
    """
...
async def check_protections(state: BotState, exchange: ExchangeInterface, current_price: float, current_volume: float, high: float, low: float) -> bool:
    """
    Kiểm tra các cơ chế bảo vệ và quyết định xem có nên tạm dừng hoặc dừng bot không.

    Args:
        state (BotState): Trạng thái bot.
        exchange (ExchangeInterface): Giao diện sàn giao dịch.
        current_price (float): Giá hiện tại.
        current_volume (float): Khối lượng hiện tại.
        high (float): Giá cao nhất.
        low (float): Giá thấp nhất.

    Returns:
        bool: True nếu bot nên dừng, False nếu không.
    """
...
async def handle_orders(state: BotState, exchange: ExchangeInterface, current_price: float) -> None:
    """
    Xử lý việc kiểm tra và cập nhật lệnh.
...
async def rebalance_grid_if_needed(state: BotState, exchange: ExchangeInterface, current_price: float) -> None:
    """
    Cân bằng lại lưới nếu đã đến thời gian cần thiết.
...
async def update_status(state: BotState, exchange: ExchangeInterface, last_status_update: float) -> float:
    """
    Cập nhật trạng thái bot và gửi cảnh báo định kỳ.

    Args:
        state (BotState): Trạng thái bot.
        exchange (ExchangeInterface): Giao diện sàn giao dịch.
        last_status_update (float): Thời điểm cập nhật trạng thái cuối cùng.

    Returns:
        float: Thời điểm cập nhật trạng thái mới.
    """
...
async def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    """
    Chạy bot giao dịch cho một sàn giao dịch cụ thể.

    Args:
        exchange (ExchangeInterface): Giao diện sàn giao dịch.
        states (Dict[str, BotState]): Từ điển trạng thái bot.
    """
...
def update_charts(n: int) -> Tuple[Dict, Dict]:
    """
    Cập nhật biểu đồ trên bảng điều khiển với dữ liệu giá và lợi nhuận.

    Args:
        n (int): Số lần cập nhật.

    Returns:
        Tuple[Dict, Dict]: Biểu đồ giá và lợi nhuận.
    """
    global bot_states
    price_data = []
    profit_data = []
...
class TestIndicators(unittest.TestCase):
    def test_calculate_rsi(self):
        ...
        self.assertAlmostEqual(rsi, 100.0, places=2)  # Giá tăng liên tục, RSI gần 100

        prices = [100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85]
        rsi = calculate_rsi(prices, period=14)
        self.assertAlmostEqual(rsi, 0.0, places=2)  # Giá giảm liên tục, RSI gần 0
    ...
    def test_calculate_atr(self):
        ...
        self.assertAlmostEqual(atr, 13.0, places=2)  # Giá trị ATR dự kiến: (12 + 14) / 2 = 13
...
async def main():
    """
    Điểm vào chính của bot giao dịch.
    """
