...
# ======== BẢO MẬT API KEYS (Đã sửa - Tắt mã hóa để tránh lỗi InvalidToken) ========
def get_encrypted_credentials(exchange: str) -> Tuple[str, str]:
    # Bỏ qua mã hóa, sử dụng trực tiếp API key và secret từ .env
...
# ======== CẤU HÌNH NÂNG CAO (Đã sửa min_quantity) ========
class AdvancedConfig:
    def __init__(self):
        ...
        self.min_quantity = 0.0005  # Đã sửa để phù hợp với LOT_SIZE của BTCUSDT trên Testnet
...
class BinanceExchange(ExchangeInterface):
    ...
    def get_symbol_filters(self) -> Dict:
        try:
            ...
            return {'minQty': 0.0005, 'maxQty': 1000, 'stepSize': 0.0001}  # Đã sửa minQty để phù hợp với BTCUSDT
...
def calculate_max_drawdown(equity_curve: List[float]) -> float:
    ...
    for value in equity_curve:
        peak = max(peak, value)
        dd = (peak - value) / peak if peak != 0 else 0.0  # Đã sửa để tránh ZeroDivisionError
...
class GridCalculator:
    ...
    def calculate_grid_parameters(self, current_price: float, investment: float, price_history: List[float], highs: List[float], lows: List[float], exchange: ExchangeInterface) -> Dict:
        ...
        try:
            ...
            return {'levels': 5, 'spacing': 0.005, 'quantity': 0.0005, 'min_price': current_price * 0.95, 'max_price': current_price * 1.05}  # Đã sửa quantity
