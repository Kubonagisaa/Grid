...
class ExchangeManager:
    ...
    async def check_price_disparity(self) -> None:
        ...
        if max_diff / min(prices.values()) > 0.01:
            ...
            send_telegram_alert(f" Large price disparity: {max_diff:.2f} between {prices}")
...
async def safe_api_call(func, *args, max_retries=3, **kwargs):
    for attempt in range(max_retries):
        try:
            ...
            send_telegram_alert(f" Exchange unavailable: {str(e)}")
...
def calculate_max_drawdown(equity_curve: List[float]) -> float:
    ...
    for value in equity_curve:
        if peak == 0:  # ã sa li ZeroDivisionError
            continue
        peak = max(peak, value)
...
async def run_simulated_backtest(exchange: ExchangeInterface) -> Dict:
    ...
    if historical_data.empty:
        ...
        return {"PnL": 0.0, "Trades": 0, "Sharpe Ratio": 0.0, "Max Drawdown": 0.0, "Total Fees": 0.0}
...
async def check_account_balance(exchange: ExchangeInterface) -> Tuple[float, float]:
    ...
    if usdt_balance < config.initial_investment:
        ...
        send_telegram_alert(f" Insufficient USDT balance on {exchange.exchange_name}: {usdt_balance:.2f} < {config.initial_investment}")
    return usdt_balance, btc_balance
...
async def handle_exchange_downtime(state: BotState, downtime_start: Optional[float]) -> Tuple[bool, Optional[float]]:
    if downtime_start is None:
        ...
        send_telegram_alert(f" Exchange {state.exchange_name} unavailable, entering safe mode")
...
async def check_protections(state: BotState, exchange: ExchangeInterface, current_price: float, current_volume: float, high: float, low: float) -> bool:
    if state.protection.check_pump_protection(current_price):
        ...
        send_telegram_alert(f" Pump protection triggered on {state.exchange_name}")
    ...
    if state.protection.check_abnormal_activity(current_volume):
        ...
        send_telegram_alert(f" Abnormal activity detected on {state.exchange_name}: Volume {current_volume}")
        state.order_manager.is_paused = True
        return False
...
def main():
    ...
    for exchange_name, exchange in exchange_manager.exchanges.items():
        loop.run_until_complete(run_simulated_backtest(exchange))
    
    for exchange_name, exchange in exchange_manager.exchanges.items():
        historical_data = loop.run_until_complete(safe_api_call(exchange.fetch_historical_data, timeframe='1h', limit=100))
        backtest_result = loop.run_until_complete(backtest_strategy(exchange, historical_data))
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for exchange in exchange_manager.exchanges.values():
            executor.submit(lambda ex=exchange: loop.run_until_complete(run_bot_for_exchange(ex, bot_states)))
