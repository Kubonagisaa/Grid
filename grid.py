        ...
        for buy_price, sell_price in self.grid_levels:
            ...
            try:
                if buy_price < current_price:
                    ...
                    send_telegram_alert(f"📥 Đã đặt lệnh MUA trên {exchange.exchange_name} tại {buy_price:.2f}, số lượng: {quantity}")
                if sell_price > current_price:
                    ...
                    send_telegram_alert(f"📤 Đã đặt lệnh BÁN trên {exchange.exchange_name} tại {sell_price:.2f}, số lượng: {quantity}")
            except Exception as e:
...
class AdaptiveGridStrategy(GridStrategy):
    ...
    async def generate_signals(self, data: MarketData) -> List[Signal]:
        # Kiểm tra xem dữ liệu có đầy đủ không trước khi thêm vào deque
...
def calculate_sharpe_ratio(returns: List[float], risk_free_rate: float = 0.02) -> float:
    ...
    # Tránh chia cho độ lệch chuẩn quá nhỏ
    if std_return < 1e-6:  # Nếu độ lệch chuẩn nhỏ hơn 0.000001
        return 0.0
...
async def run_simulated_backtest(exchange: ExchangeInterface) -> Dict[str, float]:
    ...
    # Kiểm tra và loại bỏ các hàng có giá trị thiếu
...
async def update_status(state: BotState, exchange: ExchangeInterface, last_status_update: float) -> float:
    if time.time() - last_status_update > config.status_update_interval:
        ...
        # Tạo thông báo Telegram chi tiết
        message = (
...
async def main():
    ...
    for exchange_name, exchange in exchange_manager.exchanges.items():
        # Chạy kiểm tra mô phỏng
        sim_stats = await run_simulated_backtest(exchange)
        logging.info(f"Kết quả kiểm tra mô phỏng trên {exchange_name}: {sim_stats}")
        send_telegram_alert(
            f"📉 Kết quả kiểm tra mô phỏng trên {exchange_name}:\n"
            f"PnL={sim_stats['Total Profit']:.2f}, Giao Dịch={sim_stats['Trade Count']}, "
            f"Sharpe={sim_stats['Sharpe Ratio']:.2f}, MaxDD={sim_stats['Max Drawdown']:.2f}%, Phí={sim_stats['Total Fees']:.2f}"
        )

        # Chạy kiểm tra thực tế
        historical_data = await exchange.fetch_historical_data(timeframe='1h', limit=100)
        real_stats = await backtest_strategy(exchange, historical_data)
        logging.info(f"Kết quả kiểm tra thực tế trên {exchange_name}: {real_stats}")
        send_telegram_alert(
            f"📈 Kết quả kiểm tra thực tế trên {exchange_name}:\n"
            f"PnL={real_stats['Total Profit']:.2f}, Giao Dịch={real_stats['Trade Count']}, "
            f"Sharpe={real_stats['Sharpe Ratio']:.2f}, MaxDD={real_stats['Max Drawdown']:.2f}%, Phí={real_stats['Total Fees']:.2f}"
        )

        # Thêm task chạy bot
        tasks.append(asyncio.create_task(run_bot_for_exchange(exchange, bot_states)))

    # Chạy kiểm tra chênh lệch giá giữa các sàn
    price_disparity_task = asyncio.create_task(exchange_manager.check_price_disparity())

    # Chạy Dash app trong luồng riêng
    def run_dash():
        app.run_server(debug=False, host='0.0.0.0', port=8050)

    dash_thread = Thread(target=run_dash, daemon=True)
    dash_thread.start()
    logging.info("Đã khởi động Dash app tại http://localhost:8050")

    # Chờ tất cả các task hoàn thành
    await asyncio.gather(*tasks, price_disparity_task)
...
if __name__ == "__main__":
    try:
        ...
        logging.info("Chương trình đã bị dừng bởi người dùng")
    except Exception as e:
