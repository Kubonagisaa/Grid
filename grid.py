...
class BinanceExchange(ExchangeInterface):
    ...
    def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        return self.client.create_order(
            ...
            price=str(round(price, self.price_precision)) if price else None,
            recvWindow=5000
        )
    ...
    def get_all_orders(self) -> List[Dict]:
        return self.client.get_all_orders(symbol=self.symbol, limit=50)
    ...
    def get_balance(self) -> Tuple[float, float]:
        ...
        btc_balance = float(next(asset['free'] for asset in account['balances'] if asset['asset'] == base_asset))
        return usdt_balance, btc_balance

    def get_order_book(self) -> Dict:
        return self.client.get_order_book(symbol=self.symbol, limit=50)
    ...
    def fetch_historical_data(self, timeframe: str, limit: int) -> pd.DataFrame:
        ...
        return pd.DataFrame(klines, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_volume', 'trades', 'taker_buy_base', 'taker_buy_quote', 'ignore']).astype(float)
    ...
    async def start_websocket(self, state: BotState):
        ...
        def message_handler(message):
            if 'data' in message and 'p' in message['data']:
                state.current_price = float(message['data']['p'])
        ...
        self.ws_client.ticker(symbol=self.symbol, callback=message_handler)
        logging.info(f"WebSocket khởi động trên {self.exchange_name}")

        while True:
            if not self.websocket_connected:
                logging.warning(f"WebSocket {self.exchange_name} ngắt kết nối, thử lại sau {config.websocket_reconnect_interval}s")
                self.ws_client.stop()
...
class MEXCExchange(ExchangeInterface):
    ...
    def place_order(self, side: str, order_type: str, quantity: float, price: float = None) -> Dict:
        order_type = 'limit' if order_type in [ORDER_TYPE_LIMIT, ORDER_TYPE_LIMIT_MAKER] else 'market'
        return self.client.create_order(
            symbol=self.symbol,
            side=side.lower(),
            type=order_type,
...
def safe_api_call(func, *args, max_retries=3, **kwargs):
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(300)
        except (BinanceAPIException, ccxt.NetworkError, ccxt.ExchangeError) as e:
    ...
    def place_grid_orders(self, current_price: float, exchange: ExchangeInterface) -> None:
        ...
        for price in self.grid_levels:
            ...
            if any(o['price'] == str(price) and o['side'] == side for o in open_orders):
                continue
            try:
                order = safe_api_call(exchange.place_order, side=side, order_type=ORDER_TYPE_LIMIT, quantity=quantity, price=price)
...
def check_protections(state: BotState, exchange: ExchangeInterface, current_price: float, current_volume: float, high: float, low: float) -> bool:
    ...
    if trailing_stop_price:
        ...
        if state.tracker.total_btc_bought > 0:
            safe_api_call(exchange.place_order, side=SIDE_SELL, order_type=ORDER_TYPE_MARKET, quantity=state.tracker.total_btc_bought)
...
def run_bot_for_exchange(exchange: ExchangeInterface, states: Dict[str, BotState]) -> None:
    ...
    try:
        ...
        if config.websocket_enabled and exchange.exchange_name == "binance" and exchange.ws_client:
            exchange.ws_client.stop()
        elif config.websocket_enabled and exchange.exchange_name in ["mexc", "okx"]:
